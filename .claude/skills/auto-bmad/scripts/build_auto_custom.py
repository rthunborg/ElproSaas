...
def _run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert "Claude Code — model `opus`; Codex — model `gpt-5.6-sol`, reasoning effort `xhigh`; otherwise" in sec, sec
        ...
        assert "codex exec -m gpt-5.6-luna -c model_reasoning_effort=xhigh" in cm["instruction"], cm["instruction"]
        ...
        cfg_blank = _mk(root, _CFG_FULL.replace("  security_layer: critical\n", "  security_layer: \"\"\n"), "blank.yaml")
        ...
        cfg_missing = _mk(root, _CFG_FULL.replace("  security_layer: critical\n", "  security_layer: ab-nope\n"), "missing.yaml")
        ...
        cfg_noprof = _mk(root, _CFG_FULL.replace("  cross_model_layer: diverse_review\n", ""), "noprof.yaml")
        try:
        ...
        sec_models_ok = {"claude_model": "opus", "codex_model": "gpt-5.6-sol", "codex_effort": "xhigh"}
        ...
        assert old_cm.startswith("Run an external reviewer using the configured diverse-review model/profile. Do this yourself, in this order:\n\n1. Save the review content")
        ...
        assert new_cm.startswith("Run an external reviewer using the configured diverse-review model/profile. Do this yourself, in this order:\n\n1. Run exactly this one shell command")
