...
**Never pass a spec path to build-auto in stories mode** — not for the plan run, not for the build run, not for a follow-up/final-convergence review pass. Folder+id routes by the on-disk story file's status, which is exactly the resume behaviour the pipeline wants. The **`build-plan`**, **`build-run`**, **`followup-review`** and **`final-convergence`** entries of `delegation.md` carry or reference the same *Stories mode variant* — use that fence verbatim. The invocation intent it hands build-auto is always:
