...
| Browser execution | CI uses Playwright's configured production web server. | Retain the current focused Roles/Admin browser result; do not substitute `next dev`. |
...
- `docs/quality/epic-11-nfr-evidence-runbook.md:59` — `## Existing operational evidence and remaining decisions`: confines operational follow-up to owner-approved targets and safe environments.

The R-1108 and R-1109 commands executed with `SUPABASE_TEST_REQUIRED=1`; their raw output remains under ignored `tmp/private/`. The restore rehearsal remains pending a passing scoped restore, and the browser callback proof executed RED with the implicit-fragment defect above. A skipped or mocked result is not evidence.
