...
The corrected required probe at current worktree revision `c885185` asserted the approved exact profile: Tenant A 120 active memberships, Tenant B 24, five roles with 24 primary assignments each, 40 secondary-role members, and 160 assignments. Across 25 measured iterations after five warmups, Admin read p95 was **45.98 ms**, synthetic bulk effective-permissions p95 **2.00 ms**, and the projection used exactly three authenticated RLS requests per iteration. The final local unit suite passed **1,780 tests with zero failures and zero skips**; typechecking, focused lint, and review-order validation passed, and independent final review found no finding. This is a local regression observation, not a rendering, production-capacity, or production-SLO result. Final CI for the current changes remains pending.
...
| --- | --- | --- |
| Required DB execution | PR #62, merged PR #63, and post-merge main CI each passed the required database/RLS suite with zero skips. | Retain executed/failed/skipped counts with any new gate run. |
| Browser execution | PR #62, merged PR #63, and post-merge main CI passed 138 browser tests with four explicit skips and zero failures. The #62 Production deployment is ready; bounded unauthenticated release probes returned `/login` 200, `/auth/invite/confirm` 307 to the canonical completion route, and that page 200. | Run the authorized authenticated deployed callback/login journey; do not substitute `next dev`. |
