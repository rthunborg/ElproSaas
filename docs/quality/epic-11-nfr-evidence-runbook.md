...
Status: local R-1108 pilot, local R-1109 Auth-to-Mailpit smoke, a scoped local logical restore rehearsal, and controlled hosted invitation/recovery delivery journeys executed on 2026-09-14. The owner approved pilot limits and operating criteria on the same date. The hosted recovery callback reached password update; the required local browser journey verifies password update and fresh sign-in. Monitoring history and hosted backup/recovery evidence remain pending.
...
The invitation message was spam-classified and had SPF `permerror`, although DKIM and DMARC passed. The owner inspected the existing `enhancior.se` Cloudflare records and replaced its dead SPF include target with the Google Workspace provider form `v=spf1 include:_spf.google.com ~all`, retaining the soft-fail policy. The saved record was confirmed through authoritative DNS and 1.1.1.1. A controlled recovery delivery then arrived in Gmail at 2026-09-14T19:08:55Z with SPF, DKIM, and DMARC passing, and its unmodified hosted callback reached the password-update page. This is a bounded post-change sample, not a general deliverability, spam-placement, or availability result. The required local Auth-to-Mailpit browser journey performs the password update and verifies a fresh sign-in; no further personal hosted exercise is needed. Pinned private-fixture cleanup ended two memberships, revoked no invitations, and deleted no Auth users; an independent tenant-scoped read verified only two ended memberships and no active or invited membership. Auth users and records were retained, with no existing demo data cleanup. Cloudflare verified the owner Gmail destination. Free sender operations cannot be configured in the UI without an apex-MX change, so the owner has been asked to choose the existing `thunborg.se` sender on Free or paid operations; no DNS or paid-plan change was made. Sender name Kopplas is intentional future naming; no app, repository, or configuration rename is in scope. No recipient, token, URL, or message content is recorded in this runbook.
...
| Browser execution | PR #65 CI 34925739157 passed 138 browser tests with four explicit skips and zero failures at its PR head `141d3df`; post-merge main CI 34926333592 passed at `93d4901`. The required Auth-to-Mailpit recovery case verifies password update and fresh sign-in. | No further personal hosted password exercise is needed. |
...
A private approved Workspace Drive folder was created and `BACKUP_DRIVE_FOLDER_ID` is configured as a GitHub secret; its identifier is not recorded. Google Cloud OAuth is blocked at the organizational identity check and owner sign-in is requested. An owner-only local GPG recovery key exists outside the repository; `BACKUP_GPG_PASSPHRASE` was set at 2026-09-15T09:33:24Z, and a fresh GitHub secret listing confirms all four required backup secrets. Independent owner escrow remains pending. Database URL remains absent, so the operational workflow remains disabled. Its current controls use stable pre/post Storage inventory plus restore-shaped checksums and allow only the exact direct/shared-session pooler target, rejecting query overrides. The monthly Codex heartbeat `elpro-monthly-recovery-check` is configured for the first day of each month at 09:00 local time to inspect backup freshness, recovery evidence, monitoring gaps, and the 100 SEK/month cap; no execution is yet evidence. These controls do not replace a hosted restore exercise, a 30-day monitoring history, or alert-delivery proof.
...
GitHub scheduled monitoring failed the five-minute pilot criterion: two runs occurred over approximately eight hours after activation. The approved free Cloudflare monitoring setup is in implementation and has no monitoring-history evidence yet. This is a consequential monitoring gap. It prevents an availability or monitoring PASS even though the manual sample and bounded alert delivery passed.

At 2026-09-14T19:40:46Z, the owner-approved mailbox received the GitHub failure notification for that isolated simulation. This is a bounded alert-delivery PASS only: it does not prove scheduler history, availability, an error-rate denominator, or alert latency. The repository now has server-only `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` secrets for the exact demo project. A private approved Drive folder and the `BACKUP_DRIVE_FOLDER_ID` GitHub secret are configured without recording the identifier. Google Cloud OAuth is blocked at organizational identity check and owner sign-in is requested. An owner-only local GPG recovery key exists outside the repository; `BACKUP_GPG_PASSPHRASE` is configured and fresh secret listing confirms the four required backup secrets. Independent owner escrow remains pending. Database URL remains absent; `PILOT_BACKUP_ENABLED` stays false and no encrypted upload, restore, or enablement has occurred.
...
Historical context: the stacked callback repair needed the same required CI evidence before it could be retargeted to `main`. The trigger remained an explicit named repair-branch allowlist; it did not widen verification to a branch pattern or alter CI jobs, permissions, guards, or concurrency.
