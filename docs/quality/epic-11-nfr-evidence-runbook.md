...
The initial browser callback journey is retained as historical **RED** evidence: a real invite link reached `/login` because the server callback received neither `code` nor `token_hash` from the implicit fragment. The candidate callback remediation in [PR #62](https://github.com/rthunborg/ElproSaas/pull/62), candidate commit `12bb90b`, changes that handoff. A production-build browser run from `2026-09-14T13:16:05` to `13:16:18` passed **2/2** with zero skips in 12.4 seconds: a real Auth-to-Mailpit invitation set the browser cookie and activated the database membership, and password recovery reached password update followed by a fresh sign-in. Independent focused review found no consequential finding. This is candidate **GREEN** local application evidence only while PR #62 CI, merge, and deployment remain pending.
...
| --- | --- | --- |
| Required DB execution | CI starts the local stack, resets from empty, sets `SUPABASE_TEST_REQUIRED=1`, and runs the integration/RLS suite. | Retain the current exact run URL and executed/failed/skipped counts for the revision being assessed. |
| Browser execution | CI uses Playwright's configured production web server; candidate PR #62 has a 2/2, zero-skip local production-build Auth callback journey. | Retain PR #62 CI/merge/deployment evidence and verify hosted Auth configuration/login; do not substitute `next dev`. |
