...
**IN — 2026-09-14 authenticated dashboard inspection:** the demo project's Supabase Auth Site URL is `https://elpro-saas.vercel.app` and its sole redirect URL is `https://elpro-saas.vercel.app/auth/invite/confirm`. The invite and recovery preview links use `{{ .ConfirmationURL }}`. Custom SMTP is off, so these are platform-default delivery templates; confirm email is on, while anonymous sign-ins and manual linking are off. These settings were saved and re-navigation confirmed them. They are configuration evidence only: a deployed application callback/login journey and external mailbox delivery still need their own controlled proof. No template subject or content was changed or recorded.
