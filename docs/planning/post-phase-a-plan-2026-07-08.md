...
- **Story 12.1 authority detail (Decisions 1–7C, current 2026-09-19):** `provision_tenant` remains the sole SECURITY DEFINER surface and may only perform initial provisioning, record requested/unknown/failed, record reconciliation/attempt-generation results, and audit transitions; Epic 11 acceptance may mark `ready`. The initial authorised dispatch and every explicit resend each generate a fresh 32-byte CSPRNG application token in memory, persist only SHA-256 bound to invitation/tenant/membership/normalized email/`tenant_admin`/expiry, and never expose raw material to storage, operator/browser output, logs, or audits. Replay/reconciliation never rotate or dispatch. After reconciliation, explicit `retry_first_admin_invite` uses the sole RPC to atomically revoke the prior hash and reserve the fresh hash/attempt before at most one provider call, invalidating old links; `unknown` never auto-resends. One approved snapshot permits three total dispatches; a fourth requires fresh preview/approval and begins a new invitation generation. V1 has no raw-token escrow, deterministic derivation, or provider/callback redesign. Decision 7C supersedes the earlier same-token reuse across dispatch attempts. `TENANT_PROVISIONING_BASELINES` entries are immutable ID/monotonic-version/content-hash records. Email uses trim/NFC, IDNA+lowercase domain, lowercase local, preserved plus/dots/tags, and strict syntax rejection; VAT is `SE` + matching ten-digit organisation number + `01`. `ready` requires completed DB provisioning, recorded baseline ID/version/hash, active membership, non-null matching Auth identity, and no unresolved definitive failure. `Platform.Operator.Access` is `scope: platform`, `tenantGrantable: false`, `tenantRoles: []`; `is_platform_operator()` remains exclusive authority and tenant consumers exclude the row.
...
5. ~~Business model / per-company pricing & provisioning flow (Roadmap 7 — "gemensamt beslut efter möte").~~ **Resolved:** N-2 (2026-07-26) fixes internal operator provisioning/no self-serve and subscription terms as data; the Story 12.1 owner contract (2026-09-17, supplemented 2026-09-19) fixes strict-v1 identity/schema and exact canonicalization, immutable baseline catalogue, stateless hash-bound approval, dual idempotency, sole narrow RPC authority, hashed Epic 11 token lifecycle, reconciliation-first bounded attempts, exact readiness, and non-granting platform capability. **(owner)**
