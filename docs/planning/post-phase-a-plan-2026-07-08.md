...
- **Multi-company delivery:** the product must remain deliverable to many independent eligible companies (multi-tenant SaaS); Phase B must cover tenant provisioning/onboarding and full RBAC. Story 12.1's 2026-09-17 and 2026-09-19 owner contract limits provisioning v1 to Swedish non-personal legal entities (no individuals or `enskild firma`) while leaving CRM end customers unrestricted; fixes strict email/VAT canonicalization, immutable baseline ID/version/hash, write-nothing approval, dual idempotency, the sole narrow provisioning RPC, hash-only Epic 11 invitation tokens, reconciliation-first bounded provider attempts, exact readiness, and a non-granting platform permission row; and leaves no live owner question for planning.
- **Story 12.1 authority detail (Decisions 1–8A, current 2026-09-19):** `provision_tenant` is the sole public authenticated SECURITY DEFINER writer. Server provisioning invokes it under the operator's normal Supabase JWT; the RPC derives current allow-listed `auth.uid()` and verifies a ≤2-minute domain-separated HMAC-SHA-256 attestation on every mutation. Its separate 256-bit app/Vault key uses the repository's length-prefixed Node/Postgres pattern, key IDs, and current/previous overlap bounded to that TTL; it is never stored, logged, audited, browser-returned, or committed. A dedicated least-privilege `NOLOGIN NOINHERIT` owner has only required table and filtered Vault access; `authenticated` gets exact RPC execution, while PUBLIC/anon/authenticator/service_role get no provisioning execution/DML. Remove the legacy delegate/fallback and broad service-role DML; add no custom JWT or second callable writer. The authoritative baseline catalogue is migration-owned, insert-only DB data with immutable ID/version/canonical content/hash; TypeScript is only the coherence-tested preview/build mirror. Initial provision and every Decision 7C resend use DB commit → signed `reserve_dispatch` installing only a fresh token SHA-256/current generation and returning durable provider identity facts → at most one Auth call → signed sanitized exact-reservation outcome. Replay/reconciliation never rotate or dispatch; caller retry identity, stale outcomes, and trusted `fresh_approval` booleans are forbidden. Three dispatches fit one approved snapshot; dispatch four requires fresh preview/approval with signed monotonic approval generation. Existing identity/canonicalization, token binding/expiry, exact readiness, no four-eyes rule, and platform capability constraints remain. Rollout order is app/Vault key, compatible signer/server, enforcement/removal migration.
...
5. ~~Business model / per-company pricing & provisioning flow (Roadmap 7 — "gemensamt beslut efter möte").~~ **Resolved:** N-2 (2026-07-26) fixes internal operator provisioning/no self-serve and subscription terms as data; the Story 12.1 owner contract (2026-09-17, supplemented through Decision 8A on 2026-09-19) fixes strict v1, canonicalization, migration-owned catalogue/TypeScript mirror, write-nothing preview, dual idempotency, normal-JWT/server-HMAC sole-RPC authority, least-privilege owner, signed reservation/outcome, fresh hash-only tokens, bounded attempts/monotonic reapproval, exact readiness, and non-granting platform capability. **(owner)**
