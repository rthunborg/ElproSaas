...
- **Story 12.1 authority detail (current 2026-09-19):** `provision_tenant` remains the sole SECURITY DEFINER surface and may only perform initial provisioning, record requested/unknown/failed, record reconciliation, and audit transitions; Epic 11 acceptance may mark `ready`. Tokens are cryptographically random, hash-only, bound to invitation/tenant/membership/normalized email/`tenant_admin`/expiry, never exposed to operator/log/audit, reused during reconciliation, and revoked/replaced only by explicit resend. Replay reconciles without provider work; explicit `retry_first_admin_invite` makes at most one call per execution and three attempts per invitation, then fresh preview/approval rotates the token. `TENANT_PROVISIONING_BASELINES` entries are immutable ID/monotonic-version/content-hash records. Email uses trim/NFC, IDNA+lowercase domain, lowercase local, preserved plus/dots/tags, and strict syntax rejection; VAT is `SE` + matching ten-digit organisation number + `01`. `ready` requires completed DB provisioning, recorded baseline ID/version/hash, active membership, non-null matching Auth identity, and no unresolved definitive failure. `Platform.Operator.Access` is `scope: platform`, `tenantGrantable: false`, `tenantRoles: []`; `is_platform_operator()` remains exclusive authority and tenant consumers exclude the row.
