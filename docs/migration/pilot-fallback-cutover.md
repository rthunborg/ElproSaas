...
| VAT | **`blocking`** real-pilot | `A.2` (öppen (möte) — moms-sats) + `2.1` (answered — display) | Accounting | Settings / Pricing, Calculations, Quote Versions | The VAT RATE (`A.2`) is `möte`-open; the DISPLAY rule (`2.1`: private always incl-VAT, company togglable) is decided. Blocks real-pilot accounting-finality; `UNRESOLVED_VAT` surfaces as a warning meanwhile. |
