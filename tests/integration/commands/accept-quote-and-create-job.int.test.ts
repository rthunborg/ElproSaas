 ...
 * HIGHEST-STAKES red-phase suite of Epic 7: the idempotent, transactional multi-record write that
 * records the acceptance, flips the quote version `sent → accepted`, creates the minimal job, and
 * writes quote/job/audit events — all in ONE transaction, idempotent on retry + concurrent-accept,
 * fully rolled back on any mid-transaction failure.
 *
 * ════════════════════════════════════════════════════════════════════════════════════════════════
 * RED PHASE (ATDD, Story 7.2 create-story). `src/server/commands/quotes/accept-and-create-job.ts`
 * (the `acceptQuoteAndCreateJob` command), the `accept_quote_and_create_job` RPC migration, the
 * `jobs`/`job_events` read-back factory helpers, and the affordance-gating do NOT yet exist. This
 * ENTIRE suite is `describe.skip(...)` so the gate is GREEN-but-pending until dev implements the
 * transaction, at which point the `.skip` is removed and every assertion must hold. Each test
 * asserts the EXPECTED post-implementation behaviour (TDD red phase) — do NOT weaken an assertion
 ...
 *     + anonymous (⇒ UNAUTHENTICATED) rejection; generic error, no existence disclosure.
 ...
 *
 * ── DEV: HELPERS TO ADD (tests/factories/tenants.ts) ──────────────────────────────────────────────
 * These read-back helpers do NOT yet exist and must be authored ALONGSIDE the transaction (mirror
 * `adminSelectQuoteAcceptanceRow` / `adminSelectAcceptancesForVersion` / `adminSelectQuoteEventsFor
 * Version`), then imported here:
 *   - `adminSelectJobRow(id)` / `adminSelectJobsForAcceptance(acceptanceId)` (count + source-ref proof)
 *   - `adminSelectJobEventsForJob(jobId)` (job `created` event proof)
 *   - `adminSelectQuoteEventsForVersion(versionId)` (REUSE — already exists) filtered to `accepted`
 * Until then the imports below are placeholders (marked TODO) and the suite stays `.skip`.
 *
 ...
 *  supabase/migrations/20260707120000_quote_version_sent_lock.sql (mark_quote_version_sent pattern);
 *  20260708120000_quote_new_version.sql (multi-insert + find-or-create idempotent guard);
 *  20260709120000_acceptance_to_job_model.sql (the 7.1 uniqueness backstops the RPC relies on);
 *  src/server/commands/quotes/{mark-sent,accept}.ts (the command patterns 7.2 fuses)]
 */
// RED-PHASE: seed vars below feed the assertion bodies that are commented out until 7.2 lands
// (removed with the `.skip` markers in the GREEN pass). Silence the "prepared but not yet consumed"
// warnings so the scaffold is clean in the interim without weakening the eventual assertions.
...
import {
  ...
  // TODO(7.2 dev): add these `jobs`/`job_events` read-back helpers to tests/factories/tenants.ts
  //   (mirror adminSelectQuoteAcceptanceRow / adminSelectAcceptancesForVersion), then uncomment:
  // adminSelectJobRow,
...
// TODO(7.2 dev): export `acceptQuoteAndCreateJob` from src/server/commands/quotes/index.ts,
//   then import it here (this import is the RED gate — it does not resolve until the command lands):
// import { acceptQuoteAndCreateJob } from "@/server/commands/quotes";
...
// ──────────────────────────────────────────────────────────────────────────────────────────────
// RED PHASE: the whole suite is `.skip` until `acceptQuoteAndCreateJob` + its RPC + the job/event
// read-back helpers land. Remove `.skip` (and uncomment the command import + job helpers) in the
// 7.2 GREEN pass; do NOT weaken any assertion.
// ──────────────────────────────────────────────────────────────────────────────────────────────
describe.skip("acceptQuoteAndCreateJob — atomic accept + create job (AC1)", () => {
  it("[P0] 7.2-INT-01: happy path — records the acceptance, flips `sent → accepted`, creates the minimal job with immutable source refs, writes a `quote_events` accepted + a `job_events` created + ONE audit row, all committed together", async (testCtx) => {
    ...
    // if (!res.ok) return;
    // const { acceptanceId, jobId } = res.data; // shape: { targetId: acceptanceId, jobId, acceptanceId }
    ...
    // const acc = await adminSelectQuoteAcceptanceRow(acceptanceId);
    // expect(String(acc?.accepted_price_ore)).toBe(String(SOURCE_SENT_TOTAL_ORE));
    // expect(String(acc?.source_sent_total_ore)).toBe(String(SOURCE_SENT_TOTAL_ORE));
    ...
    // // 3) Minimal job created with immutable source refs + carried customer/facility/contact.
    ...
    // // 4) A `quote_events` accepted row (occurred_at = the accepted instant).
    ...
    // expect(JSON.stringify(audits[0]?.metadata)).not.toMatch(/125000|ore|customer/i);
    expect.fail("RED: acceptQuoteAndCreateJob not implemented (Story 7.2)");
  });
  ...
  it("[P0] 7.2-INT-01: the accepted instant is the EXPLICIT input (H1), NOT the injected command clock", async (testCtx) => {
    ...
    // const res = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
    // expect(res.ok).toBe(true);
    // if (!res.ok) return;
    // const acc = await adminSelectQuoteAcceptanceRow(res.data.acceptanceId);
    // const acceptedAt = acc?.accepted_at instanceof Date ? acc.accepted_at.toISOString() : String(acc?.accepted_at);
...
describe.skip("acceptQuoteAndCreateJob — retry idempotency (AC2 — the headline)", () => {
  it("[P0] 7.2-INT-02: a second identical call returns the SAME (acceptance_id, job_id) and creates NO second acceptance/job/lifecycle-transition/event/audit", async (testCtx) => {
    ...
    // const first = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
    // expect(first.ok).toBe(true);
    // if (!first.ok) return;
    // const second = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
  ...
  it("[P0] 7.2-INT-02: an idempotent re-entry writes NO fresh audit row (R-710 — the retry produced no state change)", async (testCtx) => {
    ...
    // expect(first.ok).toBe(true);
    // const second = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: retryCorrelation });
    // expect(second.ok).toBe(true);
...
describe.skip("acceptQuoteAndCreateJob — DB duplicate-prevention constraints (AC2)", () => {
  it("[P0] 7.2-INT-03: a DIRECT second `quote_acceptances` INSERT for the same version violates `unique (quote_version_id)` — the DB backstop below the command", async (testCtx) => {
    ...
    // First acceptance via the transaction (or a seeded acceptance).
    ...
    // A DIRECT second acceptance for the SAME version must violate the unique constraint.
    await expect(
      adminInsertQuoteAcceptance({
        tenant_id: fixture.tenantA.id,
        quote_id: quoteId,
  ...
  it("[P0] 7.2-INT-03: a DIRECT second `jobs` INSERT for the same acceptance violates `unique (quote_acceptance_id)` — one job per acceptance", async (testCtx) => {
    ...
    // A DIRECT second job for the SAME acceptance must violate the unique constraint.
    await expect(
      adminInsertJob({
        tenant_id: fixture.tenantA.id,
        quote_acceptance_id: acceptanceId,
...
describe.skip("acceptQuoteAndCreateJob — atomicity / no partial state (AC3, NFR20)", () => {
  it("[P0] 7.2-INT-04: an injected mid-transaction failure (job insert / event write) leaves NO acceptance, NO lifecycle change, NO job, NO event — behavioral rollback proof", async (testCtx) => {
    if (skipUnlessStack(testCtx, stackUp)) return;
    const { versionId } = await seedSentVersion(fixture.tenantA.id, a);

    // DEV: drive a fault at a step boundary INSIDE the transaction. Options (pick the one the RPC
    // exposes): a fault-injection hook parameter on the command/RPC; OR craft an input the RPC
    // accepts past validation but the job/event INSERT rejects (e.g. a constraint the job row
    // violates mid-txn). The END STATE must be empty — the whole txn rolled back.
    // const res = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: { ...acceptInput(versionId), __faultInject: "job-insert" as never },
    //   clock: fixedClock, correlationId: crypto.randomUUID() });
    // expect(res.ok).toBe(false);

    // // NOTHING committed: no acceptance, no lifecycle flip, no job, no event, no audit.
  ...
  it("[P0] 7.2-INT-04: a fault at the EVENT-write boundary also rolls the whole transaction back (no orphaned acceptance/job)", async (testCtx) => {
    ...
    const { versionId } = await seedSentVersion(fixture.tenantA.id, a);
    // const res = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: { ...acceptInput(versionId), __faultInject: "event-write" as never },
    //   clock: fixedClock, correlationId: crypto.randomUUID() });
    // expect(res.ok).toBe(false);
    // expect((await adminSelectAcceptancesForVersion(versionId)).length).toBe(0);
...
describe.skip("acceptQuoteAndCreateJob — cross-tenant + anon rejection (AC4)", () => {
  it("[P0] 7.2-INT-05: a foreign-tenant (Tenant B) sent version id ⇒ TENANT_ACCESS_DENIED before execute; no existence disclosure; Tenant-B untouched", async (testCtx) => {
    ...
    // const res = await runCommand(acceptQuoteAndCreateJob, { client: a as never, // adminA on a B id
    //   input: acceptInput(bVersionId), clock: fixedClock, correlationId: crypto.randomUUID() });
    // expect(res.ok).toBe(false);
    // if (res.ok) return;
  ...
  it("[P0] 7.2-INT-05: an ANONYMOUS caller ⇒ UNAUTHENTICATED (anon must NOT execute the RPC — revoke-from-public)", async (testCtx) => {
    if (skipUnlessStack(testCtx, stackUp)) return;
    const { versionId } = await seedSentVersion(fixture.tenantA.id, a);
    // DEV: build an unauthenticated anon-key client (no session) and attempt the command through it.
    // const anon = makeAnonServerClient(); // TODO(7.2 dev): the anon (no-session) client helper
    // const res = await runCommand(acceptQuoteAndCreateJob, { client: anon as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
    // expect(res.ok).toBe(false);
    // if (res.ok) return;
  ...
  it("[P0] 7.2-INT-05: a non-sent (draft/accepted/rejected/expired/superseded) version ⇒ a user-safe lifecycle rejection (VALIDATION_FAILED), no leak, no write", async (testCtx) => {
    if (skipUnlessStack(testCtx, stackUp)) return;
    // A draft version (never flipped to sent) — acceptance is legal ONLY on status='sent' (AC4).
    const customerId = await adminInsertCustomer({
      tenant_id: fixture.tenantA.id, customer_type: "company",
      display_name: `acj-draft-${crypto.randomUUID().slice(0, 8)}`,
    });
    const calcId = await adminInsertCalculation({
      tenant_id: fixture.tenantA.id, customer_id: customerId,
    ...
    const versionId = await adminInsertQuoteVersion({
      tenant_id: fixture.tenantA.id, quote_id: quoteId, calculation_id: calcId, status: "draft",
      accepted_price_ore: SOURCE_SENT_TOTAL_ORE,
    });
    // const res = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
...
describe.skip("acceptQuoteAndCreateJob — concurrent accept (AC2 — row-lock proof)", () => {
  it("[P0] 7.2-INT-06: two PARALLEL Promise.all accepts of the same sent version ⇒ exactly ONE job; the other idempotent-returns or fails COMMAND_CONFLICT/ACCEPTANCE_ALREADY_RECORDED; NEVER two jobs (sleep-free)", async (testCtx) => {
    ...
    //   runCommand(acceptQuoteAndCreateJob, { client: a as never, input: acceptInput(versionId),
    //     clock: fixedClock, correlationId: crypto.randomUUID() }),
    //   runCommand(acceptQuoteAndCreateJob, { client: a as never, input: acceptInput(versionId),
    //     clock: fixedClock, correlationId: crypto.randomUUID() }),
    // ]);
    ...
    // // Every OK result points at the SAME single acceptance + job (no forked commitment).
...
describe.skip("acceptQuoteAndCreateJob — event/audit completeness + no-duplicate-on-retry (AC1/AC2)", () => {
  it("[P1] 7.2-INT-07: on success every expected row is present (acceptance, job, quote `accepted` event, job `created` event, ONE audit); a retry adds NO duplicate of any of them", async (testCtx) => {
    ...
    // expect((await adminSelectAcceptancesForVersion(versionId)).length).toBe(1);
    // expect((await adminSelectJobsForAcceptance(first.data.acceptanceId)).length).toBe(1);
    // expect((await adminSelectQuoteEventsForVersion(versionId)).filter((e) => e.event_type === "accepted").length).toBe(1);
    // expect((await adminSelectJobEventsForJob(first.data.jobId)).filter((e) => e.event_type === "created").length).toBe(1);
    // // Retry adds no duplicate of any row.
    // const retry = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
...
describe.skip("acceptQuoteAndCreateJob — lifecycle edges (AC2, P2)", () => {
  it("[P2] 7.2-INT-08: accepting an ALREADY-accepted version (its acceptance + job already exist) idempotent-returns the EXISTING records, no error-path duplication", async (testCtx) => {
    ...
    // const repeat = await runCommand(acceptQuoteAndCreateJob, { client: a as never,
    //   input: acceptInput(versionId), clock: fixedClock, correlationId: crypto.randomUUID() });
...
// Referenced so the placeholder helper import + input builder are not flagged unused while the
// assertion bodies remain commented in the RED phase. (Removed in the GREEN pass.)
void acceptInput;
void adminSelectQuoteAcceptanceRow;
void adminSelectQuoteVersionRow;
void adminSelectQuoteEventsForVersion;
void adminSelectAuditEvents;
