...
describe("Admin user management RLS and wrapper boundary (Story 11.3 ATDD RED)", () => {
  test.skip("[P0] authenticated users cannot direct-DML membership lifecycle or operation records; independent admin readback is unchanged", async () => {
  ...
  test.skip("[P0] a tenant-A Admin cannot read or mutate tenant-B membership operations through RLS or hardened wrappers", async () => {
    const { createTwoTenantFixture, cleanupFixture, makeAuthedServerClient } = await import("../../factories/tenants");
    const { adminQuery } = await import("../../factories/admin-sql");
    const fixture = await createTwoTenantFixture();
    try {
      ...
      expect(independent[0]?.count).toBeGreaterThanOrEqual(0);
    } finally {
      await cleanupFixture(fixture);
    }
  });
