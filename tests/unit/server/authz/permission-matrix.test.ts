...
test.skip("[P0] 11.1-UNIT-001 legacy tenant_admin remains a role when membership_roles has no rows", () => {
  assert.equal(
...
test.skip("[P0] 11.1-UNIT-002 permission decisions are an order-independent union of held roles", () => {
...
test.skip("[P0] 11.1-UNIT-003 omitted, empty, unknown, or malformed matrix inputs deny by default", () => {
  for (const input of [
...
test.skip("[P0] 11.1-UNIT-004 a sensitive field is withheld only when no held role grants it", () => {
