/**
 * Story 2.3 — PURE-LOGIC acceptance tests for the audit METADATA SANITIZER
 ...
 *
 * GREEN as of Story 2.3 dev-story (`sanitizeAuditMetadata` implemented).
 *
...
test("R-010: a planted fake secret/api key is DROPPED from the persisted metadata", async () => {
...
test("R-010: an `.env`-looking blob is never persisted", async () => {
  const { sanitizeAuditMetadata } = await load();
  const out = sanitizeAuditMetadata({
...
test("R-010: raw file contents / a full request body are dropped (not a safe field)", async () => {
  const { sanitizeAuditMetadata } = await load();
  const out = sanitizeAuditMetadata({
...
test("R-010: a long free-text PII blob is rejected/dropped (allow-list is scalar safe fields only)", async () => {
  const longPii =
...
test("R-010: pre-approved safe scalar fields PASS unchanged (before/after hashes, enum reason, scoped ids)", async () => {
  const { sanitizeAuditMetadata } = await load();
  const out = sanitizeAuditMetadata({
...
test("R-010: an empty / no-metadata input yields a safe empty object (never null/undefined leakage)", async () => {
  const { sanitizeAuditMetadata } = await load();
  const out = sanitizeAuditMetadata({});
