...
test("13.4-UNIT-SHAPE-04: the `active` set reproduces exactly the 40 tenant tables (pinned, non-circular)", async () => {
  ...
  assert.equal(tables.length, 40, "the active tenant-table union must total exactly 40 (no dup, no gap)");
