...
test("13.1-UNIT-DERIVE-05: TENANT_TABLES derives as the union of active modules' tenantTables == the pinned 32", async () => {
  ...
  assert.equal(derived.length, 32, "the derived tenant-table union must total exactly 32");
  assert.deepEqual(
    sortedUnique(derived),
    sortedUnique(PINNED_TENANT_TABLES),
    "the manifest-derived tenant tables must equal exactly the 32 authored H4-enrolled tables (no drift)",
  );
});
