...
test("13.4-UNIT-DERIVE-05: TENANT_TABLES derives as the union of active modules' tenantTables == the pinned 41", async () => {
  ...
  assert.equal(derived.length, 41, "the derived tenant-table union must total exactly 41");
  assert.deepEqual(
    sortedUnique(derived),
    sortedUnique(PINNED_TENANT_TABLES),
    "the manifest-derived tenant tables must equal exactly the 41 authored H4-enrolled tables (no drift)",
  );
});
