...
test.describe("Story 13.4 E-postinställningar — authenticated journey", () => {
  test("[P0] a user can persist a non-essential email opt-out after the server admits synthetic sandbox delivery", async ({ page }) => {
    ...
    // The component re-reads the authenticated preference after saving. Waiting
    // for that rendered state proves persistence without coupling this journey
    // to the route or HTTP method used by the authorized write path.
    await expect(emailPreference).not.toBeChecked();
    await page.reload(); await expect(page.getByRole("switch", { name: `${category} e-post`, exact: true })).not.toBeChecked();
  });

  test("[P1] an essential email category remains visibly required while a non-essential category is editable", async ({ page }) => {
    await signIn(page, fixture().emailActivation.preferenceUser); await page.goto("/settings/notifications");
