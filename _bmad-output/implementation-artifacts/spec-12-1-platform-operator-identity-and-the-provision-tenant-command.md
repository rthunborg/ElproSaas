...
- Every authorised explicit `retry_first_admin_invite` resend is a new dispatch generation. After reconciliation, the server generates another fresh 32-byte CSPRNG token in memory, and the sole narrowly scoped `provision_tenant` RPC atomically revokes the prior token hash, reserves the next attempt/generation, and records only the new SHA-256 hash. The provider is then called at most once, and its sanitized outcome is recorded through the approved narrow variant. Rotation invalidates every older link immediately; acceptance continues through Epic 11 validation and membership activation using only the current non-revoked binding and verified Auth identity.
...
- Persist the dispatch-attempt counter/generation and sanitized outcome. One approved provisioning snapshot authorises at most three total provider dispatch attempts: the initial dispatch plus at most two explicit resends, each with its own fresh token. A fourth dispatch requires a fresh hash-bound preview and explicit approval and begins a new invitation generation. Every result returns tenant ID, current provisioning state, attempt number, and reconciliation action.
...
| Auth timeout/failure retry | Committed tenant in unknown, requested, or failed handoff state plus explicit `retry_first_admin_invite` | Reconcile by normalized email without provider work, then atomically revoke the prior hash and reserve a fresh 32-byte-token/SHA-256 dispatch generation; make at most one provider call and persist the attempt/sanitized outcome | `unknown` never auto-resends; the approved snapshot permits three total dispatches, and a fourth requires fresh preview/approval and a new invitation generation |
...
- Given provider acceptance, timeout, definitive failure, lost response, replay, or explicit retry, when the post-commit handoff runs, then the sole RPC's narrow action variant persists/audits only the approved transition, reconciliation precedes every decision, replay/reconciliation never rotate or dispatch, `unknown` never auto-resends, each explicit resend reserves a fresh dispatch generation before at most one provider call, the attempt number/sanitized outcome are persisted and returned, and dispatch 4 requires fresh preview/approval plus a new invitation generation.
