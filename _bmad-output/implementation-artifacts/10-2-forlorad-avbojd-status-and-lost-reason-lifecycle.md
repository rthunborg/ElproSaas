...
<!-- Added by auto-bmad review triage 2026-07-19. Order: Decision → Patch → Defer. Severity tag after the type tag drives the orchestrator gate (untagged = Critical/High). Sources: primary Acceptance-Auditor (thin Tier-A) + auto-bmad-local security review (0 findings). No failed layers; blind/edge deliberately not run in Tier A. -->

- [x] [Review][Decision][Low] RESOLVED won't-fix (auto-resolved per triage recommendation, epic mode): Frozen Phase-A acceptance-gate validators relaxed from exact `=== 24` to a `>= 24` floor — `tests/unit/docs/acceptance-gate-report-validators.test.ts` (9.5-SCOPE-01) and `tests/unit/docs/migration-runbook-validators.test.ts` (9.1-CLASS-01) changed `assert.equal(tables.length, 24)` to `assert.ok(tables.length >= 24)`. Stop Condition #3 asked to reconcile-or-escalate a surfaced 10.1-guardrail drift; the dev reconciled in-place, weakening a previously-exact guard on frozen Phase-A assertions. Defensible: Phase B is additive-only, the exact live count is re-pinned at 25 in the manifest shape/derivation tests, and the Phase-A report still correctly cites 24 as a historical snapshot. Recommended: dismiss: accept the `>= 24` floor as the correct additive-only reconciliation — the exact live count stays pinned at 25 in `manifest-shape`/`manifest-derivations` tests and the Phase-A gate report's 24-table claim remains a correct historical snapshot; no code change needed.
- [x] [Review][Patch][Med] RESOLVED 2026-07-19: the standing R-1015 privacy scan now covers the quotes golden estate through the SAME shared authority. Added `GOLDEN_QUOTES_DIR` + `listGoldenQuotesFixtureFiles()` (generalized the glob backstop as `listGoldenFixtureFiles(dir)`) to `tests/support/anonymization-scan.ts`, and a new standing test in `tests/unit/fixtures/golden/lovable/lovable-privacy-scan.test.ts` that runs `scanFixtureData` over every `tests/fixtures/golden/quotes/**` fixture and asserts zero violations — so `lost-lifecycle.json` (and any future quotes golden) fails CI on real PII via the standing control, not only the weaker fixture-local `PII_PATTERNS`. — Standing CI PII/secret scan (10.x-UNIT-01 / R-1015) not extended to the new lost-reason golden fixture [tests/support/anonymization-scan.ts] — Task 6.4 + the Dev Notes P0 non-negotiable + test-design R-1015 (a CI-gated blocker) require the standing privacy scan to cover the lost-reason fixture, but the scan globs only `tests/fixtures/golden/lovable/**` (`GOLDEN_LOVABLE_DIR`) and the new fixture `tests/fixtures/golden/quotes/lost-lifecycle.json` sits outside it (the diff touches neither `anonymization-scan.ts` nor `lovable-privacy-scan.test.ts`). PII coverage is only the weaker fixture-local `PII_PATTERNS` block inside `tests/unit/features/quotes/lost-version-golden.test.ts`, not the standing R-1015 control. Fix: widen the standing scan glob (`anonymization-scan.ts` / `lovable-privacy-scan.test.ts`) to also cover `tests/fixtures/golden/quotes/**` (or all `golden/**`).
- [x] [Review][Patch][Med] RESOLVED 2026-07-19: removed `describe.skip` and re-labelled the suite green ("quote_lost_reasons INSERT-ONLY RLS (GREEN — Story 10.2 landed)") + updated the stale header note; the two own-tenant insert-only negatives (UPDATE + DELETE rejected) now RUN against the local stack instead of shipping as a dead skipped test. — Insert-only RLS scaffold shipped as `describe.skip` though the feature landed [tests/integration/rls/quote-lost-reasons.rls.test.ts:87] — still declares `describe.skip("quote_lost_reasons INSERT-ONLY RLS (RED — Story 10.2 not implemented)", ...)`; the file's own header says "Remove `.skip` after the migration + enrolment land" and both have landed, contradicting the Change Log claim "All ATDD scaffolds unskipped and green." Coverage is not actually lost (an active own-tenant-UPDATE→42501 assertion exists in `tests/integration/commands/mark-quote-version-lost.int.test.ts` as 10.2-RLS-01), so this is a dead skipped test shipped as if it were coverage. Fix: remove `.skip` and re-label green (or fold into the shared inventory suite and delete the standalone file).
- [x] [Review][Patch][Low] RESOLVED 2026-07-19: deleted the redundant skipped scaffold `tests/unit/server/commands/mark-quote-version-lost-validation.test.ts` (throwing `notYetImplemented()` placeholder). The active `tests/unit/server/commands/mark-lost-validation.test.ts` is the retained, real 10.2-UNIT-02 coverage. — Duplicate RED-phase validator scaffold with a throwing placeholder committed and skipped [tests/unit/server/commands/mark-quote-version-lost-validation.test.ts:66] — declares `describe(..., { skip: "ATDD red phase ..." })` and uses a local `notYetImplemented()` placeholder that throws if ever run; it duplicates the active real validator test `tests/unit/server/commands/mark-lost-validation.test.ts` (the only mark-lost validator test in the File List). Dead code contradicting the "all scaffolds unskipped and green" claim. Fix: delete the redundant skipped scaffold (the active `mark-lost-validation.test.ts` is the retained 10.2-UNIT-02 coverage).
