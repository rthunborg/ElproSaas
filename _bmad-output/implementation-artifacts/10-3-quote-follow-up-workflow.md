...
- [x] **Task 1 — Migration: `quote_follow_ups` table + partial unique index + full RLS (AC1, AC3, AC4)**
  - [x] 1.1 New migration file `supabase/migrations/<ts>_quote_follow_ups.sql` (timestamp AFTER `20260719120000` — the 10.2 migration). Do NOT edit any frozen migration. Purely additive.
  - [x] 1.2 Create `public.quote_follow_ups` (see Dev Notes → "The `quote_follow_ups` table" for the verbatim shape): direct `tenant_id` (FK → `tenants` on delete cascade), `quote_id` + `quote_version_id` with **composite same-tenant FKs** to `quotes(id,tenant_id)` / `quote_versions(id,tenant_id)` (on delete cascade), `due_date date not null`, nullable `note`, `status text not null default 'open' check (status in ('open','completed'))`, nullable `outcome`, `created_at`, nullable `completed_at`, plus the fail-closed `check (status <> 'completed' or completed_at is not null)`.
  - [x] 1.3 **Partial unique index** `create unique index quote_follow_ups_one_open_per_quote on public.quote_follow_ups (quote_id) where status = 'open';` (the UXB-A6 one-open rule — R-1030). Add `quote_follow_ups_tenant_id_idx (tenant_id)`.
  - [x] 1.4 Grants: `grant select, insert, update on public.quote_follow_ups to authenticated;` (**NO delete** — archive-over-delete) and `grant select, insert, update, delete on public.quote_follow_ups to service_role;` (repo convention, test-only). Anon: nothing.
  - [x] 1.5 `enable row level security` + **`force row level security`**. Policies (3): `quote_follow_ups_select_own` (SELECT `using (public.is_tenant_admin(tenant_id))`), `quote_follow_ups_insert_own` (INSERT `with check (public.is_tenant_admin(tenant_id))`), `quote_follow_ups_update_own` (UPDATE `using (...) with check (...)`). **NO delete policy.**
  - [x] 1.6 `comment on table` documenting: tenant-owned, MANY rows per tenant, at most ONE open per quote (partial unique index), UPDATE-able (open→completed), NO delete/updated_at-trigger, NO money/öre column, NO Fortnox/supplier/sync/portal column, NO notification/email column (E13 owns reminders).
- [x] **Task 2 — Enrol the new table (H4 + scope manifest) IN THE SAME PR (AC4, FR129)**
  - [x] 2.1 Add `"quote_follow_ups"` to the `quotes` module `tenantTables` in `src/scope/manifest.ts` (the module comment at ll.134-137 already reserves this for 10.3). Manifest-derived `TENANT_TABLES` count 25 → 26; the 10.1 coherence + derivation tests must stay green. **Manifest-governed activation (ADR-B003 §5.5): the table may exist only when its module change lands in the same PR** — the H4 gate fails CI otherwise. The `quotes` module is already `active`/wave `A` — this is a table joining an existing active module, NOT a new pending module.
  - [x] 2.2 Bump the live-count pins in `tests/unit/scope/manifest-shape.test.ts` + `manifest-derivations.test.ts` from 25 → 26 (they were bumped 24 → 25 by 10.2). **Do NOT re-touch the frozen Phase-A gate validators** (`acceptance-gate-report-validators.test.ts` / `migration-runbook-validators.test.ts`) — 10.2 already relaxed them to a `>= 24` floor; they stay as floors (Phase B only adds).
...
- [x] **Task 3 — Commands + validators: plan / complete / annotate (AC1, AC3, AC4)**
  ...
  - [x] 3.2 New command file `src/server/commands/quotes/follow-ups.ts` with three `defineCommand`s (envelope, RLS client, no RPC — mirror the envelope shape of `lifecycle.ts`, minus the RPC):
  ...
  - [x] 3.3 Register all three in `src/server/commands/quotes/index.ts` (export commands + validators/types).
- [x] **Task 4 — Pure due/overdue + chip/badge selection logic (AC2)**
  - [x] 4.1 `src/features/quotes/follow-up-dates.ts`: a pure `classifyFollowUp(dueDateISO, now, timeZone='Europe/Stockholm')` returning `{ isDue, isOverdue }` (or a small status enum). Compute "today in Europe/Stockholm" from the injected `now` via an explicit `Intl.DateTimeFormat('sv-SE', { timeZone: 'Europe/Stockholm' })` (never the host default, never `Date.now()`). Overdue = open AND `due_date < todayStockholm`; due-today = open AND `due_date == todayStockholm`. Completed follow-ups are never due/overdue.
  - [x] 4.2 In the same module (or `src/features/quotes/follow-up-view.ts`): a pure `selectNextOpenFollowUp(followUps)` (pick the single open follow-up for the chip) + a `followUpChipState(followUp, now)` (the chip/overdue-badge presentation state). Keep this client-island logic on the fast unit gate (the epic-9/10 "extract client-island logic to a pure unit" lesson).
- [x] **Task 5 — UI: plan dialog, completion sheet + jumps, detail chip, list filters + badge (AC1, AC2, AC3)**
  - [x] 5.1 New client component `src/components/quotes/PlanFollowUpButton.tsx` (the `Planera uppföljning` dialog): a due-date input + a note textarea + explicit confirm. Render it on a **sent** version in `QuoteDetailView.tsx`, alongside the accept / mark-lost affordances.
  - [x] 5.2 New client component `src/components/quotes/FollowUpSheet.tsx` (the `Klarmarkera` completion sheet): an outcome-note field + confirm; after completion offers `planera nästa` (re-open `PlanFollowUpButton`) and the decide-here jumps to `Markera som förlorad/avböjd` (reuse the shipped 10.2 `MarkLostButton`, passing the open follow-up id) and `Ny version` (reuse the existing new-version affordance).
  - [x] 5.3 Detail header: render the **next-follow-up chip** (from `selectNextOpenFollowUp` + `followUpChipState`) using the `ConnectionChip`/`StatusBadge` contract (UX-BDR17/BDR4); overdue escalates visually.
  - [x] 5.4 New action-state file `src/features/quotes/follow-up-action-state.ts` (mirror `lost-action-state.ts`), and server actions in `src/features/quotes/actions.ts`: `planQuoteFollowUpAction`, `completeQuoteFollowUpAction`, `annotateQuoteFollowUpAction`. Each revalidates BOTH `/quotes/[quoteId]` AND the `/quotes/[quoteId]/versions/[versionId]` subroute after success (the 6.2 subroute-revalidation lesson).
  - [x] 5.5 **The auto-complete-on-lost seam:** extend `markQuoteVersionLostAction` to read an OPTIONAL hidden `follow_up_id` form field; when present AND the lost flip succeeds, call `completeQuoteFollowUp(follow_up_id, outcome=<the chosen förlorad/avböjd>)` (lost-flip FIRST — see Dev Notes). The standalone (non-follow-up) lost dialog omits the field, so 10.2's behavior is byte-unchanged when no follow-up id is carried.
  - [x] 5.6 Quote list (`src/components/quotes/QuoteList.tsx` + `src/features/quotes/read.ts`): add the `Har uppföljning` and `Försenad uppföljning` filters (functional, correct rows) + an overdue badge on rows with an overdue open follow-up. Surface the open-follow-up + overdue flags from the list read projection. **Keep the read-model/aggregation OUT — that is Story 10.4.**
- [x] **Task 6 — Tests (all ACs; map to test-design-epic-10 IDs) — see Dev Notes → Testing**
  - [x] 6.1 UNIT (`node --test`): 10.3-UNIT-01 (due/overdue date-boundary logic, Europe/Stockholm, injected instant — due-today, just-overdue, completed-excluded, empty, overdue-escalation flag); 10.3-UNIT-02 (next-follow-up-chip + overdue-badge selection logic); the plan/complete/annotate validators.
  - [x] 6.2 INT (Vitest, local Supabase): 10.3-INT-01 (one-open partial unique index — positive plan; a second open rejected with the clear `VALIDATION_FAILED` message; a NEW open allowed after the prior is completed); 10.3-INT-02 (migration + isolation: direct `tenant_id`, force RLS, the 3 policies, `anon → none`, `TENANT_TABLES` enrolment; plan/complete/annotate commands each write exactly ONE audit row); 10.3-INT-03 (complete-with-outcome + `planera nästa` reopen allowed; the lost-from-follow-up jump auto-completes the follow-up with the chosen outcome and both actions are audited — the 10.3↔10.2 coupling).
  - [x] 6.3 RLS (Vitest): 10.3-RLS-01 (cross-tenant read/write + anon rejected; own-tenant UPDATE ALLOWED but cross-tenant UPDATE hidden/zero-rows; H4 green with the table enrolled) — via the `TENANT_TABLES` enrolment, not a hand-written ad-hoc suite.
  - [x] 6.4 Extend `tests/integration/rls/migration-reset.int.test.ts` (EXACT policy enumeration) with the 3 `quote_follow_ups` policies.
  - [x] 6.5 E2E (Playwright): 10.3-E2E-01 (`Planera uppföljning` with due date + note → next-follow-up chip; overdue badge; `Klarmarkera` with outcome note → sheet offers `planera nästa` / `Markera som förlorad/avböjd` / `Ny version`) — two-tenant fixture, `crypto.randomUUID()` seeds.
  - [x] 6.6 DOCS: 10.3-DOCS-01 (record the legacy follow-up delta P18 + the recorded parallel-open-follow-ups reversal STOP — see Dev Notes → "Oracle / legacy delta").
...
Opus 4.8 (1M) — auto-bmad dev-story delegate.

### Debug Log References

- Full `supabase db reset` against the LOCAL stack applied `20260719130000_quote_follow_ups.sql` cleanly (used a complete TEMP CLI profile with `dashboard_url` for CLI 2.109.1; the committed `cli-profile.yaml` was NOT changed — Task 6.7).
- Gates: `tsc --noEmit` exit 0; `eslint` 0 errors (1 pre-existing unrelated warning in `tests/unit/lib/money/vat.test.ts`); `next build` green; UNIT `node --test` 1487/1488 (the 1 failure is a pre-existing Windows CRLF artifact in `tests/fixtures/golden/lovable/*.json`, eol=lf-pinned so green on CI Linux — unrelated to 10.3); full Vitest INT/RLS 765/765; Playwright E2E 4/4.

### Completion Notes List

- **Delivered:** the `quote_follow_ups` migration (UPDATE-able, one-open-per-quote partial unique index `(quote_id) WHERE status='open'`, composite same-tenant FKs, force-RLS + 3 own-tenant SELECT/INSERT/UPDATE policies, NO DELETE, NO RPC); the manifest + `TENANT_TABLES` enrolment in the SAME PR (active union 25→26, `updateDenialKind → "rls-invisible"` mirroring `quote_acceptances`); the three envelope commands `planQuoteFollowUp` / `completeQuoteFollowUp` / `annotateQuoteFollowUp` (direct RLS-client writes, audit `{ targetId }` only, 23505 → clear VALIDATION_FAILED "En öppen uppföljning finns redan för offerten."); the pure Europe/Stockholm `classifyFollowUp` + `selectNextOpenFollowUp` / `followUpChipState`; the plan dialog + completion sheet + decide-here jumps + next-follow-up chip + list `Har uppföljning`/`Försenad uppföljning` filters + overdue badge; the auto-complete-on-lost action seam.
- **Custom-message seam:** `CommandError` gained an OPTIONAL `userMessage` (no new `CommandErrorCode`; the stable family stays closed) so the DB-enforced one-open (23505) and already-completed rejections surface their specific clear messages via `envelope-core`. Backward-compatible (absent ⇒ the generic per-code message; all 765 INT tests green).
- **10.3-DOCS-01 (P18 legacy delta):** the Lovable oracle is a behavioral reference only and was not reachable this run; terminology resolved from the owner-confirmed workflow + the UX strawman. The legacy app had quote follow-up handling (workflow, due lists, completion); this story ships the equivalent MANUAL workflow (plan/see/complete + due/overdue + the decide-here jumps) with NO data migration (demo-data-only MVP posture). No legacy code copied.
- **Parallel-open-follow-ups STOP (UXB-A6, R-1034) — documented residual, NOT a blocker:** the design assumes ONE open follow-up per quote (complete-then-plan-next), DB-enforced by the partial unique index. If the oracle/owner later REQUIRES parallel open follow-ups per quote, that reverses UXB-A6 and is a STOP — take to oracle/owner; do NOT silently drop the index.
- **Accepted non-atomicity residual (SETTLED DESIGN DECISION 5):** auto-complete-on-lost is a TWO-command action-layer orchestration (lost flip FIRST, then complete), NOT a widened RPC (the frozen `mark_quote_version_lost` RPC is untouched). A rare transient failure of the completion after a successful lost flip leaves an orphaned OPEN follow-up on a now-lost quote — non-corrupting and recoverable (the sheet stays available; the user can `Klarmarkera` it manually; both writes are independently audited). Happy path proven in 10.3-INT-03.
- **PII (Task 6.7):** all follow-up-note fixtures (factory `adminInsertQuoteFollowUp` seed + the three E2E seeds) are anonymized shape-only. NO golden fixture was authored, so no extension of the standing `anonymization-scan.ts` control was required (R-1015 trigger not hit).

### File List

**New files**
- `supabase/migrations/20260719130000_quote_follow_ups.sql`
- `src/server/commands/quotes/follow-ups.ts`
- `src/features/quotes/follow-up-dates.ts`
- `src/features/quotes/follow-up-view.ts`
- `src/features/quotes/follow-up-action-state.ts`
- `src/components/quotes/PlanFollowUpButton.tsx`
- `src/components/quotes/FollowUpSheet.tsx`
- `src/components/quotes/FollowUpPanel.tsx`
- `src/components/quotes/FollowUpChip.tsx`

**Modified files**
- `src/scope/manifest.ts` (enrol `quote_follow_ups` in the `quotes` module)
- `src/server/commands/quotes/{validation.ts,quote-db.ts,index.ts}`
- `src/server/commands/{command-errors.ts,envelope-core.ts}` (optional `CommandError.userMessage`)
- `src/features/quotes/{read.ts,actions.ts}`
- `src/components/quotes/{QuoteDetailView.tsx,QuoteList.tsx,MarkLostButton.tsx}`
- `tests/integration/rls/{tenant-table-inventory.ts,migration-reset.int.test.ts,cross-tenant-isolation.rls.test.ts}`
- `tests/unit/scope/{manifest-shape.test.ts,manifest-derivations.test.ts}`
- `tests/factories/tenants.ts` (`adminInsertQuoteFollowUp` + `adminSelectFollowUps` + extend `adminSelectQuoteLabel`)
- `tests/e2e/global-setup.ts` (three follow-up quote seeds)
- Unskipped/green scaffolds: `tests/unit/features/quotes/{follow-up-dates,follow-up-view}.test.ts`, `tests/unit/server/commands/follow-up-validation.test.ts`, `tests/integration/rls/{quote-follow-ups-migration-reset.int,quote-follow-ups.rls}.test.ts`, `tests/integration/commands/quote-follow-ups.int.test.ts`, `tests/e2e/quotes/quote-follow-up.e2e.spec.ts`

**Review-fix files (2026-07-19, Med finding — error-banner occlusion)**
- `src/components/quotes/PlanFollowUpButton.tsx` (relocate error/retry banners into the Dialog form)
- `src/components/quotes/FollowUpSheet.tsx` (relocate error/retry banners into the Dialog form + `autoOpen` render seam)
- `tests/integration/components/follow-up-error-visibility.test.ts` (NEW — render-tree regression: banners render inside the open Dialog, not the section body)
...
| 2026-07-19 | 1.0 | Implemented (status → review). Migration + manifest/H4 enrolment (25→26, rls-invisible profile) + 3 envelope commands (23505→clear VALIDATION_FAILED via new optional `CommandError.userMessage`) + pure Europe/Stockholm due/overdue + chip selection + plan dialog/completion sheet/decide-here jumps/next-follow-up chip/list filters + overdue badge + auto-complete-on-lost action seam. All ATDD scaffolds unskipped-and-green (3 UNIT + 3 INT/RLS + 1 E2E); migration-reset policy enumeration + cross-tenant/anon/H4 suites extended. Gates green (typecheck/lint/build/unit/int/e2e). |
| 2026-07-19 | 1.1 | Review-fix (Med): relocated the plan/complete error + retry banners INSIDE the Dialog form in `PlanFollowUpButton.tsx` + `FollowUpSheet.tsx` (they were occluded behind the fixed z-50 overlay on error). Added `FollowUpSheet.autoOpen` render seam + a new `react-dom/server` render-tree regression (`tests/integration/components/follow-up-error-visibility.test.ts`, 2/2 green) asserting the banners render above the overlay. Gates: typecheck/lint clean. |

### Review Findings

Triage of the code review (primary Tier-A auditor lens + auto-bmad-local security lens) on 2026-07-19. Verdict: Changes Requested. Counts — Critical 0 / High 0 / Med 1 / Low 2. Security lens: no findings. Blind/edge lenses were deliberately not run in epic-mode Tier A (not failed layers).

- [x] [Review][Defer][Low] (auto-resolved as defer per triage recommendation, epic mode) Hand-rolled follow-up chip + overdue badge bypass the "no new badge component" reuse constraint — New `FollowUpChip.tsx` renders bespoke inline tone classes and does not import `StatusBadge`/`status.ts`; `QuoteList.tsx`'s inline overdue badge duplicates `QUOTE_STATUS_COLORS.lost` (`bg-rose-100 text-rose-900 border-rose-300`). Text-first a11y contract (AC2) is honored and rows/chips are functionally correct — a DRY/reuse deviation, not an AC failure. Note: `StatusBadge` is keyed to `QuoteVersionStatus`, so it cannot directly model the follow-up tri-state (overdue/due-today/upcoming) and `ConnectionChip` has no component in the repo, so literal reuse is not feasible without extending `status.ts`. Recommended: defer: fold the follow-up chip + overdue-badge tones into a shared `status.ts` primitive (extend it with follow-up tone tokens) during Story 10.4's explicit "render consistently" pass (SETTLED DESIGN DECISION 6 / 10.4 AC2) — no new deferred-work entry needed (already 10.4-scoped).
- [x] [Review][Decision][Low] RESOLVED won't-fix (auto-resolved as dismiss per triage recommendation, epic mode — spec-compliant forward-provisioning; flagged for owner visibility in the epic report): `annotateQuoteFollowUp` command + `annotateQuoteFollowUpAction` shipped with no UI caller — grep confirms no `.tsx` invokes the annotate action; only the command (`follow-ups.ts`), action (`actions.ts`), validator (`validation.ts`) and export (`index.ts`) exist. Tasks 3.2 + 5.4 require exactly the command + action; no task mandates an annotate UI, and SETTLED DESIGN DECISION 1 makes the table UPDATE-able for annotation. Recommended: dismiss: spec-compliant forward-provisioning — the command + action are the mandated deliverables and a future note-edit affordance consumes them; confirm no annotate UI was expected in 10.3.
- [x] [Review][Patch][Med] Plan/complete error + retry feedback render OUTSIDE the modal Dialog, occluded on error [src/components/quotes/PlanFollowUpButton.tsx:73] — on a failed submit `dialogOpen = open && state.status !== "success"` stays true, so the full-screen `bg-black/40` Dialog overlay (`fixed inset-0 z-50`, body-scroll-locked) stays on top while the `plan-follow-up-error`/`complete-follow-up-error` + "Försök igen." banners live in the section body BEHIND it — the AC1/AC3-required clear message exists in the DOM but is invisible while the dialog is open. Reachable via a transient `SERVER_ERROR` on submit or the "Uppföljningen är redan avslutad." race on complete. Fix: relocate the error/retry banners INSIDE the Dialog form so they render above the overlay in both `PlanFollowUpButton.tsx` and `FollowUpSheet.tsx` (src/components/quotes/FollowUpSheet.tsx:64). **RESOLVED 2026-07-19:** both banners moved into the Dialog `<form>` (inside the `z-10` panel) in `PlanFollowUpButton.tsx` + `FollowUpSheet.tsx` — the section-body copies are gone, so on error the clear message paints ABOVE the overlay. Added regression `tests/integration/components/follow-up-error-visibility.test.ts` (a pure `react-dom/server` render-tree assertion — `FollowUpSheet` is props-driven and imports no server-only module, so it renders with NO DOM/DB) proving the `complete-follow-up-error` + "Försök igen." banners render AFTER the `role="dialog"` panel opens and are ABSENT from the section body. A tiny `autoOpen` render seam was added to `FollowUpSheet` (mirrors `PlanFollowUpButton.autoOpen`; the live panel never sets it) so the dialog-open error state is deterministically renderable. `PlanFollowUpButton` got the identical relocation but transitively imports the server action (`next/headers`), so it is not statically renderable — its fix is verified by typecheck/lint + the identical structural change. Gates: `tsc --noEmit` clean; `eslint` clean; the new Vitest suite 2/2 green.
