...
- [x] **Task 1 — Author the manifest schema + coherence validator (AC2, AC4)**
  ...
  - [x] 1.2 Write the coherence validator as a pure function over the manifest (input = the manifest constant, output = a list of coherence violations, empty = coherent). Encode the four 10.1 rules from AC4. **Do NOT encode the matrix-row rule (11.1, EB-A5).**
  - [x] 1.3 Add a compile-time `satisfies ScopeManifest` guard on the manifest constant AND a uniqueness invariant: no duplicate table / nav route / file-owner-type / widget id across modules (learn from the deferred `READINESS_CODES` gap — its exhaustiveness guard checked membership but NOT uniqueness; a dup compiles fine and silently inflates a union — see Constraints).
- [x] **Task 2 — Author `src/scope/manifest.ts` with the Phase A active set + Phase B pending stubs (AC2)**
  ...
  - [x] 2.3 Verify the manifest is coherent: run the Task 1 validator against it and confirm zero violations; confirm `tsc` passes.
- [x] **Task 3 — Ship the coherence validator unit suite (AC4)**
  - [x] 3.1 Create `tests/unit/scope/` suite. Positive: the real manifest is coherent (validator returns zero violations) and `satisfies`-guarded (typecheck is the compile-time half).
  - [x] 3.2 Negative: one focused case per AC4 rule — inject an incoherent manifest fixture (active module missing epic ref; an orphan nav item / tenant table / widget / notification category / file owner type / public surface not traceable to an active module; a pending module carrying live surface; a public-surface union of four) and assert the validator flags each. Prove the validator BITES (anti-vacuous).
- [x] **Task 4 — Derive `FORBIDDEN_DEFERRED_CATEGORIES` from the manifest (AC3)**
  - [x] 4.1 Refactor `src/features/files/deferred-categories.ts` so `FORBIDDEN_DEFERRED_CATEGORIES` is **derived from `pending` modules** (their file-index deferred tokens), not authored.
  - [x] 4.2 **Prove equality against a PINNED Phase-A baseline (non-circular):** the derived set must equal exactly the current 7 authored tokens `["fortnox","supplier","asset","rental","hr","dou","upphandling"]` (order-insensitive). Assert against a **pinned literal** of these 7 (not against the derivation itself — see the circular-derivation trap in Dev Notes). If the derivation does NOT reproduce exactly these 7, that is the Stop Condition signal — reconcile (do not paper over); escalate only if it reveals genuine unresolvable pre-existing drift (see Constraints / Stop Conditions).
...
Claude Opus 4.8 (dev-story delegate, auto-bmad).

### Debug Log References

- All 26 `tests/unit/scope/**` ATDD tests green (SHAPE 8, COH 7, DERIVE 6, DOCS 5).
- `pnpm typecheck` clean; `pnpm lint` clean (0 errors; 1 pre-existing unrelated warning in `tests/unit/lib/money/vat.test.ts`).
- Full unit gate: 1412 tests, 1411 pass. The single failure (`9.2-REPEAT-01` golden-loader byte-serialization on `tests/fixtures/golden/lovable/acceptance.json`) is a **pre-existing local Windows `core.autocrlf=true` checkout artifact** (committed blob is pure LF / 0 CRLF; working tree has 33 CRLF; the golden JSON fixtures are not pinned `eol=lf` in `.gitattributes`). It is unrelated to Story 10.1 (no golden fixture touched) and passes on CI (Linux/LF). The unit gate grew (added `tests/unit/scope/**`), never shrank (AC-PH-4).
- DB-backed RLS/H4 suites (local Supabase stack reachable): 218/218 pass across `rls-inventory-gate.int`, `cross-tenant-isolation.rls`, `anon-path-isolation.rls`, `audit-anon-mutation-enrollment.rls`, `settings-rls.int` — the H4 gate confirms the manifest-derived `TENANT_TABLES` equals the live 24-table DB schema (non-circular grounding, Task 6.1).
- `verify:service-role-containment`, `verify:bundle-containment`, and `pnpm build` all pass (the new client-reachable `@/scope` import path introduces no bundle/service-role leak).

### Completion Notes List

- **AC1 (governance re-baseline):** `AGENTS.md` phase statement → Phase B / Legacy Parity Release; deferral list → the Phase C ledger (PRD §14); scope enforcement points at `src/scope/manifest.ts`. Re-baselined the enforcement/scope statements in `.claude/agents/phase-scope-reviewer.md`, `.codex/agents/phase-scope-reviewer.toml`, `CLAUDE.md` (phase note + reviewer-table row), `docs/process/agent-workflow.md`, `docs/process/branching-and-pr-policy.md`, `docs/process/codex-config-verification.md`, `docs/process/claude-code-coexistence.md`, `docs/process/local-setup.md`. Historical/record docs (`docs/migration/*`, planning artifacts, demo-environment operational prose) left intact per Task 7.4. The `.claude/settings.json` deny set + `guard.ps1` hook were NOT touched (Stop Condition guard test green).
- **AC2 (typed manifest):** `src/scope/manifest.ts` = `SCOPE_MANIFEST` (`satisfies ScopeManifest`, explicit type annotation for a stable public type). 8 Phase-A `active` modules partition exactly 24 tenant tables / 7 nav routes / 7 file owner types under wave `A`; 19 Phase-B `pending` modules (waves B1a/B1b/B2/B3) carry no live surface. Each Phase-A active module carries its real delivering-epic ref (E1–E8) + a baseline `activatedAt` (Open Question 1 default). Schema (`manifest-schema.ts`) uses literal-union `wave`/`PublicSurface`/`EpicRef` for `tsc` typo-checks; EB-A10 platform scope is expressible (`provisioning` marked `scope: 'platform'`).
- **AC3 (four derivations, zero drift):** deny-list (`FORBIDDEN_DEFERRED_CATEGORIES` ← `pending` modules' `deferredFileToken`), nav registry (`EXPECTED_NAV_ROUTES` ← active nav routes), H4 `TENANT_TABLES` (← active modules' tenant tables), and the deferred-token scan (transitive via the live deny-list import) all derive from the manifest. Each proven byte-equal to an INDEPENDENT ground truth (pinned literals + the authored `nav-items.ts` hrefs + the live DB schema) — never derived==derived. Fail-loud retained (DERIVE-06 + the H4 gate + the guardrail scans).
- **AC4 (coherence validator):** `validateManifestCoherence` implements the four 10.1 rules (active-missing-epic; orphan-surface; pending-module-live-surface; public-surface-exceeds-closed-set) PLUS the Task-1.3 cross-module uniqueness invariant (the `READINESS_CODES` gap improvement). The permission-matrix-row rule is deliberately NOT implemented (EB-A5 → Story 11.1); COH-06 asserts its absence.
- **Deny-token → module mapping (highest-risk item):** all 7 authored tokens map to genuine Phase-B `pending` modules with valid waves — `fortnox`(E33), `supplier`(E25/suppliers), `asset`(E22/assets), `rental`(E21/rentals), `hr`(E31), `dou`(E27), `upphandling`(E29/tenders). DoU and tender/FKU MANUAL cores are Phase B (B3); only their AI automation is Phase C — so no `wave: 'C'` was needed and the AC2 5-value wave union stands.
- **`TenantTableName` decision:** kept as an explicit 24-literal union (the exhaustive per-table metadata switches' compile-time contract); only the runtime `TENANT_TABLES` re-sources (derives from the manifest). Deriving the literal union from the manifest via `as const` was rejected because it breaks the ATDD tests' `as { modules: AnyModule[] }` casts (readonly-tuple vs mutable-array comparability). The introspection + every metadata helper are untouched (Task 6.2).
- **Module `id` typed as bare `string` (deviation from Task 1.1's literal-union requirement — accepted):** `ScopeModule.id` is declared `readonly id: string` in `src/scope/manifest-schema.ts`, NOT a literal union, unlike `wave` (`Wave`) and `publicSurfaces` (`PublicSurface[]`) which are literal-union typed. Rationale (mirrors the `TenantTableName` call): `id` is a free-form stable identifier — not a derivation key and not the safety-critical `publicSurface` closed set (ADR-B004's three-value union) — so a strict `tsc` typo-check on it is not warranted. Crucially, a literal-union `id` would break the synthetic-id coherence/invariant fixtures that deliberately build `ScopeManifest` values with ids outside the real set (e.g. `manifest-invariants.test.ts` INV-05 uses `id: "operators"`, and the coherence negative fixtures inject orphan/synthetic modules) — those tests must be able to construct arbitrary ids to prove the runtime validator BITES. It would also add per-module type-maintenance on every future activation for no runtime safety gain (the cross-module uniqueness invariant, Task 1.3, already catches duplicate ids at validation time). Decision: keep `id: string`; do NOT force the union.

### File List

**New (product code):**
- `src/scope/manifest-schema.ts` — `ScopeManifest`/`ScopeModule` types, literal-union `Wave`/`PublicSurface`/`EpicRef`, pure derivation selectors, `validateManifestCoherence`.
- `src/scope/manifest.ts` — the `SCOPE_MANIFEST` constant (8 active + 19 pending modules).
- `src/scope/nav-registry.ts` — `EXPECTED_NAV_ROUTES` (manifest-derived).

**Modified (product/test code):**
- `src/features/files/deferred-categories.ts` — `FORBIDDEN_DEFERRED_CATEGORIES` now derives from the manifest.
- `tests/integration/rls/tenant-table-inventory.ts` — `TENANT_TABLES` derives from the manifest; `TenantTableName` re-authored as an explicit 24-literal union (introspection + metadata helpers untouched).

**Modified (governance docs / agents):**
- `AGENTS.md`, `CLAUDE.md`
- `.claude/agents/phase-scope-reviewer.md`, `.codex/agents/phase-scope-reviewer.toml`
- `docs/process/agent-workflow.md`, `docs/process/branching-and-pr-policy.md`, `docs/process/codex-config-verification.md`, `docs/process/claude-code-coexistence.md`, `docs/process/local-setup.md`
...
- [x] [Review][Decision][Med] Module `id` typed as bare `string`, not the literal-union Task 1.1 requires — `ScopeModule` declares `readonly id: string` (`src/scope/manifest-schema.ts`), so a module-id typo is not compile-checked and there is no compile-time enum of module ids, whereas `wave` (`Wave`) and `publicSurfaces` (`PublicSurface[]`) did get literal unions. The deviation is undocumented (the completion notes record the analogous `TenantTableName` "keep explicit union" call but not this `id: string` choice). Real tension: a strict `id` union would break the synthetic-id coherence/invariant fixtures (e.g. `manifest-invariants.test.ts` INV-05 builds a `ScopeManifest` with `id: "operators"`) and add per-module maintenance on every activation. Recommended: fix: document the `id: string` deviation + rationale in the story completion notes (mirroring the `TenantTableName` decision entry) — `id` is a free-form stable identifier, not a derivation key or the safety-critical `publicSurface` closed set, so a literal union is not warranted; do NOT force the union.
- [x] [Review][Patch][Low] Residual "Phase A" scope label left in a re-baselined governance doc [docs/process/local-setup.md:97] — line 97 still reads "Supabase is the Phase A backend (Auth + Postgres + Storage, architecture §6)" while the same file's top-of-file phase line WAS re-baselined to Phase B; Task 7.3 required grepping and re-baselining residual enforcement/scope "Phase A" statements. Fix: change "the Phase A backend" to "the backend" (or "the Phase B backend"), leaving the architecture §6 cross-reference intact.
- [x] [Review][Defer][Low] `tenantTablesFromManifest` unions ALL active modules regardless of `scope`, so a future platform-scoped module's tables would leak into the H4 tenant-table inventory [src/scope/manifest-schema.ts:518] — deferred, latent forward-seam (not caused-actionable now). No active platform module carries tables in 10.1 (`provisioning`/E12 is pending, empty); INV-05 deliberately proves a platform-scoped active module with `tenantTables: ["platform_operators"]` validates coherent. When E12 activates a `platform_operators` table it would appear in the derived `TENANT_TABLES` and be asserted against the tenant-owned schema by the H4 gate. The schema provides no `platformTables`/scope-filter seam to keep platform tables out of the tenant union — an explicit "later derivation" per architecture §5.3; add the scope filter (or a `platformTables` field) when the first platform table lands at E12 activation.
