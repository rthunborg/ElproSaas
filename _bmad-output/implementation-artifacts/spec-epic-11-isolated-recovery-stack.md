...
- `.github/workflows/ci.yml:287` — `Start pinned isolated Storage loader and runtime`: discovers the private gateway mapping, requires its loopback Storage status route before the proof, and prints bounded, credential-redacted diagnostics if the mapping or route is unavailable.
- `.github/workflows/ci.yml:322` — `Prove physical Storage loader preserves protected rows`: snapshots complete seeded logical rows before bytes exist, invokes the exact pinned loader, then runs the real API/trigger proof.
