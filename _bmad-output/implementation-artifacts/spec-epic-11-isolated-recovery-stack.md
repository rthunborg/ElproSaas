...
- `.github/workflows/ci.yml:287` — `Start pinned isolated Storage loader and runtime`: on a synthetic-only failure, prints bounded, credential-redacted Auth/REST/Storage/gateway diagnostics before stopping the job.
- `.github/workflows/ci.yml:301` — `Prove physical Storage loader preserves protected rows`: snapshots complete seeded logical rows before bytes exist, invokes the exact pinned loader, then runs the real API/trigger proof.
