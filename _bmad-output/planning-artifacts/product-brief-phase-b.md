...
| N-2 | ✅ Closed 2026-07-26; detailed 2026-09-17 and supplemented by Decisions 1–8A on 2026-09-19: internal operator provisioning only; no self-serve/AI flow; strict-v1 Swedish legal-entity identity; migration-owned insert-only baseline catalogue with TypeScript mirror; stateless approval; dual idempotency; sole RPC under normal operator JWT plus server HMAC; least-privilege owner; signed dispatch reservation/exact outcome; fresh hash-only token generation; bounded attempts; exact readiness; non-granting platform capability | E12 implements the approved contract and coordinated key/server/migration rollout; pricing remains tenant data and secrets are never committed |
...
**Current Story 12.1 owner contract (Decisions 1–8A, 2026-09-19):** keep one public authenticated `SECURITY DEFINER provision_tenant` RPC. It runs under the operator's normal Supabase JWT, derives current allow-listed `auth.uid()`, and requires a ≤2-minute domain-separated server HMAC for every mutation. A dedicated `NOLOGIN NOINHERIT` owner has only required table and filtered Vault access; there is no legacy delegate, service-role provisioning DML/call, custom JWT, or second callable DEFINER writer. The separate 256-bit key uses the length-prefixed Node/Postgres pattern, key IDs, bounded current/previous overlap, and never appears in storage, logs, audit, browser, or source. The migration-owned insert-only database catalogue is execution authority; TypeScript is a coherence-tested preview/build mirror. Initial provision and every Decision 7C resend use DB commit → signed `reserve_dispatch` installing only the fresh SHA-256/current generation and returning durable provider identity facts → at most one Auth call → signed sanitized outcome for that exact reservation/generation. Replay/reconciliation remain provider-free; caller retry identity, stale outcomes, raw-token RPC input, and trusted `fresh_approval` booleans are forbidden. Three dispatches fit one approved snapshot; a fourth requires fresh preview/approval with signed monotonic approval generation. Existing strict identity/canonicalization, token binding/expiry, exact readiness, no four-eyes rule, and non-granting `Platform.Operator.Access` remain binding. Rollout provisions app/Vault key first, then signer/server, then enforcement migration/removal of legacy authority; no secret is committed.
