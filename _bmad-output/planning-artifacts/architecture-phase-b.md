...
| `provisionTenant` | strict-v1 identity/request facts + tenant + immutable-catalogue baseline ID/version/hash/settings + first-Admin invited membership + initial handoff state atomically; Auth Admin handoff begins only after commit, and later provider outcomes/reconciliation use narrow action variants of the same RPC | dual idempotency: same request UUID/hash reconciles first and returns existing tenant/current state; same UUID/different hash → `IDEMPOTENCY_CONFLICT`; different UUID/same canonical identity → `ALREADY_PROVISIONED` |
...
1. **`provision_tenant` is SECURITY DEFINER**, gated on `is_platform_operator()` — the operator has, by definition, no membership in the tenant being created (the chicken-egg); this is the sanctioned, separately-approved DEFINER case foreseen by ADR-A009. The transaction commits the database baseline and `pending_first_admin_invite` state first. Supabase Auth Admin invitation then runs server-side in the command, never in the RPC and never client-reachable. The same sole RPC has narrowly validated action variants and may only perform initial atomic provisioning; record invite `requested`, `unknown`, or `failed`; record reconciliation results; and audit each transition. It is not a general service-role mutation surface. The existing Epic 11 invitation-acceptance path may mark `ready` when it activates the first-Admin membership and the full readiness predicate holds. Epic 12 adds no second DEFINER surface.
...
### 15.4A Provisioning flow and subscription data (N-2 plus Story 12.1 owner contract, amended 2026-09-17 and 2026-09-19)
...
4. the server re-normalizes and re-hashes, reloads the exact immutable baseline-catalogue entry, and recalculates its content hash; mismatch rejects and missing/changed catalogue content returns `PREVIEW_STALE`;
5. `provision_tenant` commits the tenant, selected server-owned baseline ID/version/hash and values, invited membership, identity/idempotency facts, initial handoff state, and audit atomically;
6. after commit, the server reconciles through Epic 11; a provider dispatch happens only for the explicit `retry_first_admin_invite` action and at most once in that command execution;
7. every database/Auth handoff transition is audited, including the authenticated operator-derived approver, approval time, request ID, preview hash, baseline ID/version/content hash, attempt number, sanitized outcome, reconciliation action, and any approved token rotation.
...
- **Immutable server-owned baseline catalogue:** `TENANT_PROVISIONING_BASELINES` is version-controlled; every published entry has a baseline ID, monotonic version, and content hash. Changes create a new version and published versions are never edited. Requests select ID/version and cannot provide arbitrary baseline values. Preview shows ID/version/hash and exact values; execution reloads the exact entry and recalculates its hash, returning `PREVIEW_STALE` if missing or changed. Entry content covers `sv-SE`, `Europe/Stockholm`, `SEK`, approved existing VAT/tax-profile references (not raw rates), terms placeholders preserving sign-off warnings, approved number-series defaults, and conservative display defaults.
...
**Post-commit Auth authority and truthfulness.** Persist `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, and `ready`. Provider acceptance means `requested`, not delivered. Timeout/lost response means `unknown`, not failed. Definitive failure retains only a sanitized code and an operator retry. The sole `provision_tenant` RPC exposes only validated action variants for initial provisioning, recording invite requested/unknown/failed, recording reconciliation results, and auditing each transition. General service-role database mutation is forbidden. The existing Epic 11 acceptance path may set `ready` when it activates the membership and the readiness predicate below holds; add no second DEFINER surface.

**Invitation-token lifecycle (Decision 7C; supersedes same-token reuse across dispatch attempts).** Reuse Epic 11 validation and membership activation. The initial authorised dispatch creates a fresh 32-byte CSPRNG application token server-side; the raw value exists only in memory for that call, and only its SHA-256 hash is persisted. Every authorised explicit `retry_first_admin_invite` resend is a new dispatch generation: after reconciliation, generate a new raw token in memory and use the sole narrow `provision_tenant` RPC to atomically revoke the prior token hash, reserve the new attempt/generation, and record only the new SHA-256 hash. The provider call follows that reservation and occurs at most once; its sanitized outcome is then recorded through an approved action variant. Each current token is bound to invitation ID, tenant, membership, normalized email, `tenant_admin`, and expiry; atomic rotation invalidates older links. Acceptance uses Epic 11 validation against the current non-revoked binding plus verified Auth identity. Raw tokens never enter database storage, operator/browser output, logs, or audits. V1 adds no recoverable escrow, deterministic derivation, or provider/callback redesign.

**Replay and provider attempts.** Same-request replay and reconciliation return the existing tenant/current state after reconciling membership, invitation, and Auth identity by normalized email; they never rotate a token and never call the provider. Every resend requires explicit authorised `retry_first_admin_invite` after reconciliation, makes at most one provider call per command execution, and persists the generation/attempt plus sanitized outcome. `unknown` never triggers automatic resend. One approved provisioning snapshot permits at most three total dispatch attempts—the initial dispatch and at most two explicit resends—and every attempt gets a fresh token generation. A fourth dispatch requires a fresh hash-bound preview and explicit approval and begins a new invitation generation. Return tenant ID, state, attempt number, and reconciliation action on every replay/retry result.
...
| Cross-tenant/privilege leakage via provisioning/operator paths | Operator console exposes identity/status only; sole narrow DEFINER `provision_tenant` with operator check + action negatives; `Platform.Operator.Access` is non-granting and tenant-excluded; NFR54 end-to-end second-tenant proof. |
...
| AB-A19 | Provisioning is a deterministic, bounded operator service. Phase B ships no AI provisioning surface; free-text agent instructions are rejected. Any off-product assistance remains outside the authority boundary and receives no database access, arbitrary SQL capability, secrets, or credentials. The sole RPC, hash-only token, explicit-attempt, immutable-catalogue, exact-readiness, and platform-capability constraints are binding (§3.3/§15.4A). | amended 2026-09-17 and 2026-09-19; supersedes the 2026-07-26 "agent as orchestrator" product-flow wording |
...
| **N-2 provisioning** | No self-serve ever; deterministic strict-v1 operator service, exact email/VAT canonicalization, immutable baseline catalogue, stateless hash-bound approval, DB-first/Auth-second reconciliation, dual idempotency, sole narrow RPC, hash-only Epic 11 tokens, explicit bounded attempts, exact readiness, and non-granting platform capability; subscription data never hardcoded. | §3.3, §15.4A, AB-A19 |
