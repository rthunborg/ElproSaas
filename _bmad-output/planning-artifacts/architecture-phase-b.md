...
| E12 | *(columns / constraints)* | Additive provisioning facts include country code + normalized organisation number, request UUID + canonical request hash, immutable baseline ID/monotonic version/content hash, normalized first-Admin email, exact handoff status, provider attempt counter, and sanitized provider outcome. Invitation records reuse Epic 11 and store only a cryptographically random token hash bound to invitation/tenant/membership/email/`tenant_admin`/expiry. A durable unique constraint reserves `(country_code, normalized_organization_number)` across active, inactive, and archived tenants; request identity is independently unique. The onboarding checklist is **derived** from server data, not stored; per-admin dismissal is a small column, not a table. No preview table is added. |
...
**Invitation-token lifecycle.** Reuse Epic 11. Generate cryptographically random tokens server-side, persist only their hashes, never return raw tokens to the operator browser, and never write raw tokens to logs or audits. Bind each token to invitation ID, tenant, membership, normalized email, `tenant_admin`, and expiry. Reconciliation reuses an active token. Only explicit resend revokes the old token and creates a new one; acceptance uses Epic 11 validation and membership activation.

**Replay and provider attempts.** Same-request replay is reconciliation-first: return the existing tenant/current state after reconciling membership, invitation, and Auth identity by normalized email. It performs no provider work implicitly. Provider dispatch requires explicit `retry_first_admin_invite`, at most one provider call per command execution, and persistence of the attempt counter plus sanitized outcome. Allow three dispatch attempts per invitation. `unknown` never triggers automatic resend. After attempt three, require a fresh preview/approval that rotates the invitation token. Return tenant ID, state, attempt number, and reconciliation action on every replay/retry result.
