...
| E12 | *(columns / constraints)* | Additive `tenants` provisioning facts include country code + normalized organisation number, request UUID + canonical request hash, baseline profile/version, and first-Admin handoff status. A durable unique constraint reserves `(country_code, normalized_organization_number)` across active, inactive, and archived tenants; request identity is independently unique. The onboarding checklist is **derived** from server data, not stored; per-admin dismissal is a small column, not a table. No preview table is added. |
...
| `provisionTenant` | strict-v1 identity/request facts + tenant + versioned baseline settings + first-Admin invited membership + initial handoff state atomically; Auth Admin handoff begins only after commit | dual idempotency: same request UUID/hash returns original result; same UUID/different hash → `IDEMPOTENCY_CONFLICT`; different UUID/same canonical identity → `ALREADY_PROVISIONED` |
...
1. **`provision_tenant` is SECURITY DEFINER**, gated on `is_platform_operator()` — the operator has, by definition, no membership in the tenant being created (the chicken-egg); this is the sanctioned, separately-approved DEFINER case foreseen by ADR-A009. The transaction commits the database baseline and `pending_first_admin_invite` state first. Supabase Auth Admin invitation then runs server-side in the command, never in the RPC and never client-reachable. Later handoff transitions and reconciliation reuse existing authorised server/invitation paths; Epic 12 adds no second DEFINER surface.
...
### 15.4A Provisioning flow and subscription data (N-2 plus Story 12.1 owner contract, amended 2026-09-17)
...
1. the authenticated, allow-listed operator submits `schema_version = 1` plus the structured request and `request_id`;
...
4. the server re-normalizes and re-hashes; mismatch rejects and a baseline-version change returns `PREVIEW_STALE`;
5. `provision_tenant` commits the tenant, selected server-owned baseline, invited membership, identity/idempotency facts, initial handoff state, and audit atomically;
6. after commit, the server reconciles and requests the first-Admin Auth invitation through the existing Epic 11 machinery;
7. every database/Auth handoff transition is audited, including the authenticated operator-derived approver, approval time, request ID, preview hash, and baseline version.

**Tenant eligibility and canonical identity.** ElPro tenants are companies/legal entities, not individuals. V1 provisions Swedish non-personal legal entities only: `country_code = 'SE'` plus a normalized organisation number. Normalization removes spaces and hyphens, requires ten digits, validates the Swedish checksum, and rejects personnummer-shaped identities. Swedish sole proprietorships (`enskild firma`) are therefore unsupported because their organisation identity is the proprietor's personnummer. This restriction does not apply to a tenant's CRM end customers: they may be companies or private individuals, with no personnummer capture added here. A supplied VAT registration number must be consistent with the canonical organisation identity but never substitutes for it.

Canonical identity is `(country_code, normalized_organization_number)`; formatting and legal-name variations are equivalent. Inactive/archived tenants keep the reservation and must be reactivated instead of duplicated. The database enforces uniqueness across every tenant status. Request idempotency is separate: same request UUID/hash returns the original result; same UUID/different content returns `IDEMPOTENCY_CONFLICT`; different UUID/same canonical organisation returns `ALREADY_PROVISIONED` with existing identity/status. No replay path silently updates the tenant.

**Stateless preview and approval.** Preview writes nothing — including no audit row, Auth call, or preview record. It returns `schema_version`, `request_id`, normalized identity, validations/warnings, action (`CREATE | ALREADY_PROVISIONED | CONFLICT`), exact baseline profile/version, all proposed tenant/company/commercial/module values, first-Admin name and normalized email, proposed audit events, unsupported/deferred fields, and the server-calculated hash. The approver is only the authenticated allow-listed operator's `auth.uid()`; caller-supplied identity is invalid. One operator may preview and approve; there is no Phase B four-eyes requirement.
...
- **Server-owned versioned baseline:** `sv-SE`, `Europe/Stockholm`, `SEK`, approved existing VAT/tax-profile references (not raw rates), terms placeholders preserving sign-off warnings, approved number-series defaults, and conservative display defaults. Preview shows the exact selected profile/version and values.
...
**Post-commit Auth truthfulness.** Persist `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, and `ready`. Provider acceptance means `requested`, not delivered. Timeout/lost response means `unknown`, not failed. Definitive failure retains only a sanitized code and an operator retry. Before resend, reconcile membership, durable invitation, and Auth identity by normalized email; reuse any usable Epic 11 invitation and send anew only when none exists. `ready` requires server confirmation of a usable invitation/Auth identity bound to the intended membership, not proof of email delivery or recipient acceptance. Audit every transition through existing authorised server/invitation paths; add no second DEFINER surface.
...
| **N-2 provisioning** | No self-serve ever; deterministic strict-v1 operator service, stateless hash-bound approval, DB-first/Auth-second reconciliation, dual idempotency; subscription data never hardcoded. | §15.4A, AB-A19 |
