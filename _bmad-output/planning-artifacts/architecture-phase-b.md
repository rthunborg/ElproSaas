...
11. **ADR-B011 (decided 2026-09-23; owner amendments 2026-09-24):** Epic 13 implements Story 13.4 with sandbox proof while real-recipient sending stays disabled until a separate owner go-live. Customer quote email carries the valid PDF as a private, durable delivery attachment created by the authorized send request; the worker reads only that artifact and revalidates currentness before provider submission. Recipient and source are frozen when the authorized sender selects the linked customer/contact email. No public online-acceptance link is introduced; only active-module producers may send. The [decision record](../../docs/decisions/ADR-B011-epic-13-email-release-and-quote-delivery.md) governs this correction to the historical N-6 example.
...
Platform.Operator.Access  [scope=platform; tenantGrantable=false; tenantRoles=[]]
```

This is a seed, not a closed universe: each module activation adds its own keys in the same PR (§5.5). The names above are binding for the areas they cover — do not re-coin `Jobs.Read` alongside `Jobs.ViewAll`. `Platform.Operator.Access` is classification metadata for the separately authorized platform route registry, not a tenant permission.
...
- **Matrix rows land in the same PR as the module's manifest activation** (FR68, ADR-B003): activating a module without its matrix rows fails the manifest coherence validator (§5.4). The E12 platform exception is the explicit non-granting `Platform.Operator.Access` row with `scope: 'platform'`, `tenantGrantable: false`, and `tenantRoles: []`. It exists so the platform surface remains manifest/matrix traceable; it grants nothing and `is_platform_operator()` remains the exclusive authority.
- For `scope: 'tenant'`, the matrix drives, from one derivation: command-layer capability checks (§3.5), nav visibility (§15.1), per-tab/widget visibility, per-role landing routes (§15.3), the admin "effective permissions" viewer (server-rendered from this constant), and the per-role negative-test generator (§16.1). Tenant role assignment, effective-entitlement selection, tenant navigation, and tenant module settings must filter out every platform-scoped row. The operator route registry may consume `Platform.Operator.Access` separately, and coherence validation requires the row without treating the provisioning platform module as a tenant module or tenant-selectable entitlement.
...
### 9.1 Wave B1a (full depth) — 9 tenant-owned + 3 platform-scoped + 1 ops table
...
| E12 | `tenant_provisioning_baselines` **(platform-scoped)** | Authoritative execution catalogue, created and populated only by migrations. Each immutable, insert-only row carries baseline ID, monotonic version, canonical content, and content hash. Published versions are never updated; no runtime role has DML. The dedicated `provision_tenant` owner has only the required read grant, while the TypeScript `TENANT_PROVISIONING_BASELINES` constant is a preview/build-time mirror checked for exact DB/TS coherence. |
| E12 | *(columns / constraints)* | Additive provisioning facts include country code + normalized organisation number, request UUID + canonical request hash, immutable baseline ID/monotonic version/content hash, normalized first-Admin email, exact handoff status, provider dispatch generation/attempt counter, and sanitized provider outcome. Invitation records reuse Epic 11, but each authorised provider dispatch has a fresh 32-byte CSPRNG application token whose raw value exists only in memory; the database stores only SHA-256 bound to invitation/tenant/membership/email/`tenant_admin`/expiry, with prior hashes atomically revoked on explicit resend. A durable unique constraint reserves `(country_code, normalized_organization_number)` across active, inactive, and archived tenants; request identity is independently unique. The onboarding checklist is **derived** from server data, not stored; per-admin dismissal is a small column, not a table. No preview table is added. |
...
**Summary:** B1a adds 9 tenant-owned tables (+3 platform-scoped, +1 ops); B1b adds 8 ungated + ~9–11 gated job-depth candidates; B2 outlines ~26; B3 outlines ~19–20 (+3 Fortnox post-ADR). Every activation extends `TENANT_TABLES`, the exact-policy enumeration, and the per-role negative suite in the same PR.
...
| `provisionTenant` | strict-v1 identity/request facts + tenant + migration-owned catalogue baseline ID/version/hash/settings + first-Admin invited membership + initial handoff state atomically; every mutation carries the operator JWT plus a server-minted HMAC attestation; Auth Admin handoff begins only after a signed dispatch reservation commits, and exact-generation provider outcomes use narrow variants of the same RPC | dual idempotency: same request UUID/hash reconciles first and returns existing tenant/current state without provider work; same UUID/different hash → `IDEMPOTENCY_CONFLICT`; different UUID/same canonical identity → `ALREADY_PROVISIONED` |
...
1. **`provision_tenant` is the sole public authenticated SECURITY DEFINER provisioning writer.** The server invokes it under the operator's normal Supabase Auth JWT; the function derives and rechecks the current allow-listed `auth.uid()` through `is_platform_operator()`. `authenticated` receives EXECUTE on this exact RPC only; `PUBLIC`, `anon`, `authenticator`, and `service_role` have no provisioning-RPC execution or provisioning-table DML, and no caller receives membership in the owner role. Every mutating action additionally requires a valid server-only provisioning HMAC attestation (§15.4A); either proof missing or invalid fails generically with zero writes. The RPC is owned by a dedicated least-privilege `NOLOGIN NOINHERIT` role with only required table operations and filtered Vault-secret access. It may only perform initial atomic provisioning; reserve a dispatch by installing the fresh token hash/current generation; record exact-reservation provider `requested`, `unknown`, or `failed` outcomes; record provider-free reconciliation results; and audit each durable transition. Supabase Auth Admin runs only after a signed reservation commits, never in the RPC and never client-reachable. The existing Epic 11 acceptance path may mark `ready` when the full readiness predicate holds. Epic 12 adds no second callable DEFINER writer, custom provisioning JWT, or service-role database call.
...
Single-row mutations (role changes, follow-up scheduling/completion, notification read flips, preference toggles, conflict acceptance without booking edit) remain envelope commands on RLS-protected queries. Error codes extend the stable family: `PERMISSION_DENIED`, `BOOKING_CONFLICT_UNACKNOWLEDGED`, `SERIES_EDIT_SCOPE_REQUIRED`, `BILLING_BASIS_LOCKED`, `ALREADY_PROVISIONED`, `IDEMPOTENCY_CONFLICT`, `UNSUPPORTED_FIELD`, `UNSUPPORTED_SCHEMA_VERSION`, `PREVIEW_STALE`, `TOKEN_REVOKED`, `PROVIDER_ATTEMPT_LIMIT_REACHED`, `JOB_TYPE_DOWNGRADE_FORBIDDEN`, `SYNC_OPERATION_CONFLICT` (public surfaces map to generic responses, §6.2).
...
- **AuthZ:** Supabase-authenticated user AND `is_platform_operator()` (the `platform_operators` allow-list, §9.1). The separately consumed `Platform.Operator.Access` matrix row is non-granting route-classification metadata (`scope: platform`, `tenantGrantable: false`, `tenantRoles: []`), never an alternate authorization path. Tenant users get the generic denial; tenant role/entitlement/nav/module-setting consumers exclude platform rows, and the operator area is invisible to tenant nav.
- **Data exposure:** console read-models expose **tenant identity/status/first-Admin state only** — never tenant business data (UX §4.3; NFR54). Identity is country code + normalized organisation number; legal-name differences are display data, not distinct identity. Provisioning runs via `provisionTenant` (§14), fully audited (audit rows carry the new tenant's tenant_id).
...
### 15.4A Provisioning flow and subscription data (N-2 plus Story 12.1 owner contract, amended 2026-09-17 and Decisions 1–8A on 2026-09-19)
...
**Deterministic bounded flow.** Phase B ships no AI provisioning flow. A founder/developer or other allow-listed platform operator supplies the strict, versioned request to the deterministic provisioning service. Free-text agent instructions are rejected, and no agent or client receives general database access, arbitrary production SQL, secrets, credentials, or a custom provisioning JWT. `provision_tenant` (§14) remains the only provisioning write path; there is no legacy delegate/fallback or broad service-role provisioning-table DML.

1. the authenticated, allow-listed operator submits `schema_version = 1` plus the structured request and `request_id`; provider dispatch/retry control is a separate command action, not a schema-v1 tenant field;
2. the server normalizes and validates the request and returns a stateless, write-nothing preview plus `preview_hash`;
3. the operator explicitly approves by resubmitting the original request, the same request ID, and the hash;
4. the server re-normalizes and re-hashes, reloads the exact migration-owned database catalogue row, and recalculates its content hash; mismatch rejects and missing/changed catalogue content returns `PREVIEW_STALE`;
5. server provisioning code mints a short-lived, domain-separated HMAC attestation over the complete execution projection and calls `provision_tenant` under the same operator's normal JWT; the RPC independently checks both `auth.uid()` allow-list authority and the attestation, then commits the tenant, selected baseline values, invited membership, identity/idempotency facts, initial handoff state, and audit atomically;
6. after that commit, initial dispatch and every explicit resend create a fresh application token in server memory, then use signed `reserve_dispatch` to atomically install only its SHA-256 hash/current generation and receive the durable email/membership/role/expiry/reservation facts used for the provider call;
...
**Stateless preview and approval.** Preview writes nothing — including no audit row, Auth call, or preview record. It returns `schema_version`, `request_id`, normalized identity, validations/warnings, action (`CREATE | ALREADY_PROVISIONED | CONFLICT`), exact baseline ID/monotonic version/content hash, all proposed tenant/company/commercial/module values, first-Admin name and normalized email, proposed audit events, unsupported/deferred fields, and the server-calculated hash. The approver is only the authenticated allow-listed operator's `auth.uid()`; caller-supplied identity is invalid. One operator may preview and approve; there is no Phase B four-eyes requirement.

**Canonical first-Admin email.** Trim and Unicode-NFC-normalize the input; IDNA-normalize and lowercase the domain; lowercase the local part as an explicit product policy; preserve plus-addressing; and do not strip dots or tags. Reject display-name syntax, comments, malformed addresses, and multiple addresses. This canonical email drives reconciliation and the readiness comparison to the Auth identity.

**Strict v1 request.** Unknown fields produce field-level `UNSUPPORTED_FIELD`; unsupported versions produce `UNSUPPORTED_SCHEMA_VERSION`. Nothing is silently ignored, downgraded, or stored for later.

- **Required:** schema version; request UUID; legal company name; country code and organisation number; first-Admin name/email; baseline profile ID/version; subscription plan identifier/status; included users; additional-user price in integer öre; contract start date.
- **Optional:** VAT registration number; address; primary email/phone; contract/trial end; billing reference; structured commercial overrides; Reply-To; known manifest-active module IDs; approved feature flags; already-supported tenant/company profile fields.
- **Immutable migration-owned baseline catalogue:** the authoritative execution data is an insert-only database catalogue populated by version-controlled migrations. Every published row has baseline ID, monotonic version, canonical content, and content hash; changes insert a new version and never edit a published row. No runtime role has catalogue DML. Requests select ID/version and cannot supply arbitrary values. Preview shows ID/version/hash and exact values; execution reloads the exact database row and recalculates its hash, returning `PREVIEW_STALE` if missing or changed. TypeScript `TENANT_PROVISIONING_BASELINES` remains only the preview/build-time mirror, with mandatory DB/TypeScript coherence tests. Catalogue content covers `sv-SE`, `Europe/Stockholm`, `SEK`, approved existing VAT/tax-profile references (not raw rates), terms placeholders preserving sign-off warnings, approved number-series defaults, and conservative display defaults. This supersedes the earlier TypeScript-only execution-authority wording while preserving version-controlled immutability.
- **Rejected from v1:** logo/file uploads; Fortnox credentials/settings; base schedules/calendar configuration; initial users beyond first Admin; initial imports; pending/inactive modules; arbitrary feature flags; raw VAT/ROT/grön-teknik rates; final legal terms; free-text agent instructions; secrets/credentials. A recognized future field may be reported as deferred in preview but blocks execution until removed; support requires a new schema version and approved story/module activation, never automatic later application.

**Provisioning mutation authority (Decision 8A).** Every mutating action passes through the one public authenticated `SECURITY DEFINER provision_tenant` RPC under the operator's normal Supabase Auth JWT and requires both the current allow-listed `auth.uid()` and a short-lived domain-separated HMAC-SHA-256 attestation minted only by server provisioning code. The dedicated 256-bit provisioning key is separate from JWT, service-role, and quote-PDF keys, held in the app secret store and as a filtered Supabase Vault secret. Signing and verification use the repository's established length-prefixed Node/Postgres HMAC pattern, explicit key IDs, at most two minutes of validity, and a bounded current/previous-key overlap no longer than that TTL. The attestation is internal and is never persisted, logged, audited, or returned to the browser. Unsigned, forged, expired, wrong-domain, wrong-key, or tampered requests fail generically with zero writes.

The attestation binds the action and schema version; authenticated actor `auth.uid()`; request ID plus canonical request projection/hash; canonical organisation identity; preview hash and explicit approval evidence; baseline ID/version/content hash; invitation-token SHA-256 hash; exact invitation, membership, and reservation IDs; absolute dispatch and approval generations plus expected current generation; sanitized outcome; attestation/key IDs; and issued/expiry instants. Trusted caller booleans such as `fresh_approval`, caller-provided email/membership/operation/role/expiry facts, and stale/out-of-order/conflicting outcomes are rejected. Fresh approval is represented by signed monotonic approval-generation evidence.

**Post-commit Auth authority and truthfulness.** Persist `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, and `ready`. Provider acceptance means `requested`, not delivered. Timeout/lost response means `unknown`, not failed. Definitive failure retains only a sanitized code and an operator retry. The sole RPC exposes only validated signed mutation variants for initial provisioning, dispatch reservation, exact-reservation invite requested/unknown/failed outcomes, reconciliation recording, and transition audit. Read-only preview remains stateless/no-write; reconciliation remains provider-free and returns only sanitized facts. The existing Epic 11 acceptance path may set `ready` when it activates the membership and the readiness predicate below holds; add no second callable DEFINER writer.

**Invitation-token lifecycle (Decision 7C, authority tightened by Decision 8A; supersedes same-token reuse across dispatch attempts).** Reuse Epic 11 validation and membership activation. The initial authorised dispatch creates a fresh 32-byte CSPRNG application token server-side; the raw value exists only in memory for that call, and only its SHA-256 hash is signed and persisted. Every authorised explicit `retry_first_admin_invite` resend is a new dispatch generation: after provider-free reconciliation, generate a new raw token in memory and call signed `reserve_dispatch` to atomically revoke the prior hash, install the new hash/current generation, reserve the attempt, and return durable normalized email/membership/`tenant_admin`/expiry/reservation facts. Retry identity comes only from that RPC result, never caller input. The provider call follows that committed reservation and occurs at most once; its sanitized outcome is separately attested for that exact reservation/generation. Stale, out-of-order, or conflicting outcomes fail closed. Each current token remains bound to invitation, tenant, membership, normalized email, `tenant_admin`, and expiry; rotation invalidates older links. Raw tokens never enter the RPC, database storage, operator/browser output, logs, or audits. V1 adds no recoverable escrow, deterministic derivation, or provider/callback redesign.

**Replay and provider attempts.** Same-request replay and reconciliation return the existing tenant/current state after reconciling membership, invitation, and Auth identity by normalized email; they never rotate a token and never call the provider. Every resend requires explicit authorised `retry_first_admin_invite` after reconciliation, makes at most one provider call per command execution, and persists the absolute generation/attempt plus sanitized outcome. `unknown` never triggers automatic resend. One approved provisioning snapshot permits at most three total dispatch attempts—the initial dispatch and at most two explicit resends—and every attempt gets a fresh token generation. A fourth dispatch requires a fresh stateless preview and explicit approval whose signed monotonic approval generation authorizes the next dispatch generation. Return tenant ID, state, attempt number, and reconciliation action on every replay/retry result.

**Coordinated rollout.** Provision the dedicated application secret and matching Vault key first; deploy a compatible signer/server path next; then apply the enforcement migration that installs the database catalogue/owner/grants and removes the legacy delegate and service-role provisioning DML. Current/previous key overlap exists only for the bounded two-minute maximum TTL. No secret value is committed.

**Exact readiness.** `ready` requires all of: atomic database provisioning completed; the approved baseline ID/version/content hash recorded; the first-Admin membership active; a non-null Auth user ID on that membership; the bound Auth identity email equal to the normalized first-Admin email; and no unresolved definitive provisioning failure. Provider acceptance alone is insufficient.
...
| Cross-tenant/privilege leakage via provisioning/operator paths | Operator console exposes identity/status only; sole narrow DEFINER `provision_tenant` requires live allow-listed JWT identity plus a ≤2-minute domain-separated HMAC, is owned by a least-privilege NOLOGIN/NOINHERIT role, and has no legacy/service-role fallback; migration-owned catalogue and signed reservation/outcome ordering are negative-tested; `Platform.Operator.Access` is non-granting and tenant-excluded; NFR54 end-to-end second-tenant proof. |
...
| Provisioning/onboarding (FR73–76, NFR54) | §3.3 platform capability, §9.1, §14 (`provisionTenant`), §15.4/§15.4A |
...
| AB-A14 | `job_runs`, `platform_operators`, and the migration-owned insert-only `tenant_provisioning_baselines` catalogue are the enumerated non-tenant-owned exceptions (ops log / platform identity / provisioning authority), documented in §9.1 with their own RLS/grant posture; everything else stays direct-`tenant_id`. | amended 2026-09-19 by Decision 8A |
...
| AB-A19 | Provisioning is a deterministic, bounded operator service. Phase B ships no AI provisioning surface; free-text agent instructions are rejected. Any off-product assistance remains outside the authority boundary and receives no database access, arbitrary SQL capability, secrets, or credentials. The sole normal-JWT RPC plus server-HMAC dual proof, least-privilege owner, migration-owned catalogue, signed dispatch/outcome protocol, hash-only token, explicit-attempt, exact-readiness, and platform-capability constraints are binding (§3.3/§15.4A). | amended 2026-09-17 and Decisions 1–8A on 2026-09-19; supersedes the 2026-07-26 "agent as orchestrator" and TypeScript-only execution-authority wording |
...
| **N-2 provisioning** | No self-serve ever; deterministic strict-v1 operator service, exact email/VAT canonicalization, migration-owned insert-only baseline catalogue with TypeScript mirror, stateless hash-bound approval, dual normal-JWT/server-HMAC mutation proof, DB-first signed dispatch reservation/Auth-second outcome, dual idempotency, sole narrow RPC/least-privilege owner, hash-only Epic 11 tokens, explicit bounded attempts, exact readiness, and non-granting platform capability; subscription data never hardcoded. | §3.3, §15.4A, AB-A19 |
