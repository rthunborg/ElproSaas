...
- **N-2 / Story 12.1 (E12 provisioning, amended 2026-09-17 and supplemented by Decisions 1–7C on 2026-09-19)** — no self-serve and no Phase B AI provisioning flow. An allow-listed operator provisions Swedish non-personal legal entities only through strict schema v1, exact email/VAT canonicalization, canonical organisation identity plus request-hash idempotency, and a stateless write-nothing preview bound to authenticated-operator approval and an immutable baseline ID/version/content hash. The sole `provision_tenant` RPC has only narrow initial/outcome/reconciliation/attempt-generation/audit variants. The initial authorised dispatch and every explicit resend use a fresh in-memory 32-byte CSPRNG token and persist only SHA-256 bound to the Epic 11 invitation facts; after reconciliation the RPC atomically revokes the prior hash and reserves the new generation before one provider call, invalidating old links. Replay/reconciliation never rotate or dispatch; `unknown` never auto-resends. One approved snapshot permits three total dispatches, while a fourth requires fresh preview/approval and a new invitation generation. No raw-token escrow, deterministic derivation, or callback/provider redesign is allowed; this Decision 7C contract supersedes same-token reuse across dispatch attempts. `ready` has the exact DB/baseline/active-membership/Auth-ID+email/no-failure predicate. `Platform.Operator.Access` is non-granting platform metadata and `is_platform_operator()` remains the exclusive authority. Unsupported/deferred fields block execution and are never stored for later; subscription data remains data, never hardcoded prices.
