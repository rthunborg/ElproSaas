...
- **N-2 / Story 12.1 (E12 provisioning, amended 2026-09-17 and supplemented 2026-09-19)** — no self-serve and no Phase B AI provisioning flow. An allow-listed operator provisions Swedish non-personal legal entities only through strict schema v1, exact email/VAT canonicalization, canonical organisation identity plus request-hash idempotency, and a stateless write-nothing preview bound to authenticated-operator approval and an immutable baseline ID/version/content hash. The sole `provision_tenant` RPC has only narrow initial/outcome/reconciliation/audit variants; Epic 11 invitation tokens are cryptographically random, hash-only, bound, reused on reconciliation, and rotated only by explicit resend. Replay reconciles before action; provider work requires explicit `retry_first_admin_invite`, permits one call per execution and three attempts per invitation, and then requires fresh preview/approval and rotation. `ready` has the exact DB/baseline/active-membership/Auth-ID+email/no-failure predicate. `Platform.Operator.Access` is non-granting platform metadata and `is_platform_operator()` remains the exclusive authority. Unsupported/deferred fields block execution and are never stored for later; subscription data remains data, never hardcoded prices.
