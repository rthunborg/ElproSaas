...
>
> **Current field-scope override — 2026-09-03:** do not execute the historical PWA/offline instruction below. ADR-B009 supersedes ADR-B007 for Phase B: field workflows remain responsive at 360×640 but require connectivity; PWA installation/manifest, service-worker caching, durable offline storage, offline reads/writes, local queues, synchronization/replay/conflict states, and background/reconnect-driven offline synchronization are Phase C. Story 10.7 is a documentation/governance alignment story after verification, not an implementation claim or an E14–E18 technical prerequisite.
...
**Phase B / Legacy Parity Release.** Phase A (Epics 1–9, internal pilot MVP) is complete. Phase B was ratified in a party-mode session and has a full planning package: `_bmad-output/planning-artifacts/` holds `prd-phase-b.md`, `architecture-phase-b.md` (ADR-B001…B009, with ADR-B007 historical and ADR-B009 current for field posture), `ux-design-specification-phase-b.md`, `epics-phase-b.md` (Epics 10–34, wave-tagged B1a/B1b/B2/B3), plus the frozen Phase A originals (`prd.md`, `architecture.md`, `epics.md` — do NOT edit those; they are the pilot record).
...
3. **Historical mobile posture — superseded 2026-09-03.** The paragraph formerly directed an installable PWA with genuine offline capture. Preserve that owner answer and ADR-B007 as history, but do not implement it in Phase B. ADR-B009 requires connected responsive field workflows at 360×640, suitable transient unsent-input protection, explicit connection/failure/retry states, and server-confirmed success. The complete PWA/offline package is Phase C; no native app.
...
- **N-2 / Story 12.1 (E12 provisioning, amended 2026-09-17 and supplemented through Decision 8A on 2026-09-19; current status `in-progress`)** — no self-serve or Phase B AI flow. An allow-listed operator provisions Swedish non-personal legal entities through strict v1, exact canonicalization, dual idempotency, and a stateless write-nothing preview. The authoritative baseline catalogue is migration-owned, insert-only DB data; TypeScript is only its mandatory-coherence preview/build mirror. The sole public authenticated `SECURITY DEFINER provision_tenant` RPC is called under the operator's normal JWT, rechecks current `auth.uid()`, and requires a ≤2-minute domain-separated server HMAC for every mutation. Its separate 256-bit app/Vault key uses the length-prefixed Node/Postgres pattern and bounded current/previous key IDs; neither attestation nor secret is stored, logged, audited, browser-returned, or committed. A dedicated `NOLOGIN NOINHERIT` owner has least privilege; remove legacy fallback and service-role provisioning DML, and add no custom JWT/second writer. Initial provision and every Decision 7C resend use DB commit → signed `reserve_dispatch` installing only a fresh SHA-256/current generation and returning durable provider identity facts → at most one Auth call → signed sanitized exact-reservation outcome. Replay/reconciliation never rotate or dispatch; caller retry identity, stale outcomes, and trusted `fresh_approval` booleans are forbidden. Three dispatches fit one snapshot; dispatch four requires fresh preview/approval with signed monotonic approval generation. Existing token binding/expiry, exact readiness, no four-eyes rule, and `Platform.Operator.Access` constraints remain. Roll out key first, compatible signer/server second, enforcement/removal migration third; do not claim the build complete until implemented and verified.
