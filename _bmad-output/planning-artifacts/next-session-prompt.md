...
- **N-2 / Story 12.1 (E12 provisioning, amended 2026-09-17)** — no self-serve and no Phase B AI provisioning flow. An allow-listed operator provisions Swedish non-personal legal entities only through strict schema v1, canonical organisation identity plus request-hash idempotency, a stateless write-nothing preview bound to explicit authenticated-operator approval, and a DB-first/Auth-second reconciled invitation handoff. Unsupported/deferred fields block execution and are never stored for later. Subscription data (`SubscriptionPlan`, `IncludedUsers`, `AdditionalUserPrice`, `EnabledModules`, `CommercialOverrides`, …) remains data, never hardcoded prices.
