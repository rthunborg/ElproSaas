...
**Journey B5: An operator provisions a second company.** An allow-listed platform operator enters a strict request for a Swedish non-personal legal entity, reviews a write-nothing server-hashed preview of the migration-owned database baseline ID/version/content hash, and explicitly approves it. Server code calls the sole RPC under that operator's normal JWT with a short-lived internal HMAC attestation; the database commit precedes a signed dispatch reservation, and the Auth call uses only durable facts returned by that reservation. Replay/reconciliation shows the same tenant, state, attempt, and next action without provider work. Initial dispatch and each explicit retry use a distinct fresh 32-byte token, persist only SHA-256, bind the signed outcome to the exact reservation/generation, and make at most one provider call; dispatch four needs fresh preview/approval with a new signed approval generation. The first Admin reaches exact readiness only after membership activation and matching Auth identity, then walks onboarding to a working state. No engineer touched anything; nothing the new tenant does can read or affect any other tenant. Self-serve public signup is permanently out per N-2.
...
- FR75: Provisioning is audited and fully tenant-isolated; canonical `(country_code, normalized_organization_number)` is unique across every tenant status, and archived/inactive identity must be reactivated rather than duplicated. Same request UUID/hash is provider-free reconciliation-first and returns tenant/current state/attempt/action without rotation; conflicts return `IDEMPOTENCY_CONFLICT` or `ALREADY_PROVISIONED`; no replay silently updates. The sole public authenticated `SECURITY DEFINER provision_tenant` RPC is called under the operator's normal JWT, rechecks current allow-listed `auth.uid()`, and requires a server-only domain-separated HMAC-SHA-256 attestation for every mutation. A dedicated least-privilege `NOLOGIN NOINHERIT` owner has only required table and filtered Vault access; callers receive exact RPC execution only, never owner membership or provisioning-table DML. `PUBLIC`, `anon`, `authenticator`, and `service_role` have no provisioning execution/DML. No legacy delegate/fallback, custom provisioning JWT, service-role database call, or second callable DEFINER writer exists. The dedicated 256-bit key is separate from JWT/service-role/quote-PDF keys, follows the length-prefixed Node/Postgres pattern, uses key IDs/current+previous overlap bounded to a maximum two-minute TTL, fails closed, and is never persisted/logged/audited/browser-returned. The attestation binds the complete architecture §15.4A action, identity, request, preview/approval, baseline, token-hash, exact reservation, generation, outcome, and time projection. Initial provisioning and every Decision 7C resend use DB commit → signed `reserve_dispatch` that installs only the fresh hash/current generation and returns durable provider identity facts → at most one Auth call → separately signed sanitized outcome for the exact reservation/generation. Caller-supplied retry identity and stale/out-of-order/conflicting outcomes fail closed. `unknown` never auto-resends. One approved snapshot permits three dispatches; a fourth requires fresh preview/approval expressed by signed monotonic approval generation, never a trusted boolean. Raw tokens remain server-memory-only and never enter the RPC. Epic 11 acceptance validates the current binding and may set `ready` only for the exact predicate; provider acceptance is requested, not delivered or ready.
- FR76: Self-serve public tenant signup is **permanently out of scope for Phase B** — owner answer N-2 (2026-07-26): there will be no self-registration for customer companies; each is registered and provisioned internally after a contract is signed. No public registration surface ships. Operator-driven provisioning is the only path. Schema v1 uses a strict field allow-list and an immutable, migration-owned, insert-only database catalogue: each row has ID/monotonic version/canonical content/content hash; published rows are never edited, changed content inserts a new version, no runtime role has DML, and requests select ID/version rather than arbitrary values. TypeScript `TENANT_PROVISIONING_BASELINES` is only the preview/build-time mirror with mandatory DB/TypeScript coherence tests; this supersedes TypeScript-only execution authority. Unknown fields return `UNSUPPORTED_FIELD`, unsupported versions return `UNSUPPORTED_SCHEMA_VERSION`, rejected/deferred capabilities are never ignored/stored, preview writes nothing, execution reloads/re-hashes the database authority and returns `PREVIEW_STALE` on mismatch, approver identity comes only from the authenticated operator, and no second person is required. Activation also supplies non-granting `Platform.Operator.Access`; authorization remains exclusively `is_platform_operator()`. Rollout provisions matching app/Vault key material first, deploys compatible signer/server code, then applies enforcement/removes legacy authority; no secret is committed (`architecture-phase-b.md` §3.3/§15.4A).
...
- FR81: Email sending implementation is **specified** by N-6 (2026-07-26): a central verified sending subdomain, display name `[Företagsnamn] via [Systemnamn]`, tenant Reply-To, and the flow priority invitations/security → customer quote PDF delivery → internal accept/reject notification → job assignment → quote reminders → digests. Quote reminders stop automatically on accept, reject, withdrawal, a new version, or expiry. Invoices are sent from Fortnox, not from us. Story 13.3 runs queued/non-sending; Story 13.4 implements and sandbox-tests the provider, with real-recipient delivery default-off until a separate owner go-live. Customer quote mail attaches the valid PDF without a public acceptance link; producers for pending modules wait for activation (`architecture-phase-b.md` §4.6; ADR-B011).
...
- NFR54: Tenant provisioning must be proven leak-free, secret-safe, and duplicate-safe: automated negatives cover the sole normal-JWT + server-HMAC RPC boundary; exact owner/grants and absent legacy/service-role mutation paths; unsigned/forged/expired/wrong-domain/tampered zero-write failures; non-granting platform capability; identity uniqueness/reconciliation replay; migration-owned catalogue immutability plus DB/TypeScript coherence; write-nothing preview; signed dispatch reservation and durable-fact provider inputs; fresh SHA-256-only token generation/revocation; stale/out-of-order outcome rejection; three-dispatch budget and signed monotonic fourth-dispatch approval; exact readiness; post-commit Auth reconciliation; secret absence from DB/audit/log/browser; identity/status-only operator projections; and an end-to-end second-tenant proof (AC-PH-3).
...
| N-6 | Email activation (domain, from-address, first flows) | ✅ **Answered.** Central verified subdomain, `[Företag] via [System]`, tenant Reply-To, six-step flow priority, reminder stop conditions, delivery-log fields (arch §4.6). ADR-B011 keeps real delivery separately owner-gated and removes the historical quote online-acceptance link from Phase B. |
