...
- FR75: Provisioning is audited and fully tenant-isolated; it can neither read nor affect any other tenant, canonical organisation identity stays unique across every status, replay reconciles without silent update/provider work, the sole RPC remains narrowly bounded and requires live operator identity plus server HMAC for mutation, Epic 11 tokens remain hash-only, provider attempts are signed/explicit/bounded, and DB/Auth partial state and exact readiness remain truthfully reconcilable.
- FR76: Self-serve public tenant signup is **permanently out** (N-2 — no self-registration for customer companies, ever); operator-driven provisioning is the only path and must use strict schema v1, a migration-owned insert-only baseline catalogue with TypeScript coherence mirror, dual idempotency, a stateless write-nothing preview, authenticated-operator approval with signed monotonic generation evidence, and the non-granting platform capability contract.
...
- NFR54: Tenant provisioning proven leak-/secret-/duplicate-safe — automated negatives cover the narrow RPC, platform capability exclusion, catalogue binding, replay/attempt/token/readiness paths, and identity/status-only console, plus the end-to-end second-tenant proof (AC-PH-3).
...
- AR-B13: Transaction-sensitive commands use SECURITY INVOKER RPCs per architecture §14 (`createBooking`/`updateBooking`, `createBookingSeries`/`updateBookingSeries`, `resolveConflict`, `markQuoteVersionLost`, `markJobComplete`, `lockBillingBasis`, `provisionTenant`, email-queue claim/process). The only callable DEFINER provisioning exception is `provision_tenant`: normal operator-JWT invocation, internal `auth.uid()` allow-list check, server-HMAC proof on mutation, dedicated least-privilege `NOLOGIN NOINHERIT` owner, and negative tests. Background service context remains separately contained by §4.3.
- AR-B14: Tenant provisioning: `platform_operators` allow-list + migration-owned insert-only `tenant_provisioning_baselines` catalogue (platform-scoped exception tables) + `is_platform_operator()` hardened helper; non-granting `Platform.Operator.Access` classification; narrow signed `provisionTenant` actions with reconciliation-first idempotency (`ALREADY_PROVISIONED`) and exact dispatch reservation/outcome binding; operator console in the same app at `src/app/operator/**` outside tenant shell/context, exposing tenant identity/status only (NFR54). No custom provisioning JWT, legacy delegate, broad service-role DML, or TypeScript-only execution authority.
...
- AR-B27 (amended 2026-09-19 through Decision 8A): **§15.4A provisioning** — a deterministic strict-v1 platform-operator service; Phase B ships no AI provisioning flow and rejects free-text agent instructions. It enforces Swedish non-personal legal-entity identity, exact email/VAT canonicalization, dual idempotency, a migration-owned insert-only baseline catalogue with a TypeScript preview mirror, stateless hash-bound approval, and a DB-first/Auth-second audited state machine. The sole `provision_tenant` RPC runs under the operator's normal JWT, rechecks `auth.uid()`, and requires a short-lived server HMAC for every mutation; its dedicated owner and grants are least privilege. Signed dispatch reservation returns durable provider identity facts and signed outcomes bind the exact reservation/generation. Hashed invitation tokens, reconciliation-only replay, explicit bounded attempts, exact readiness, and non-granting `Platform.Operator.Access` metadata are binding. Legacy fallback, service-role provisioning DML, caller retry identity, and trusted approval booleans are forbidden. Subscription terms are tenant data, never hardcoded.
...
- UX-BDR6: Operator console: three data-entry steps (Företagsuppgifter → migration-owned Baslinje ID/version/hash → Bjud in första Admin) followed by the complete server-hashed preview and explicit approval; re-entrant from durable server state; replay/reconciliation shows tenant/state/attempt/action without rotating or dispatching. Provider resend is explicit, never automatic on `unknown`, with signed reservation before at most one call and an exact-generation outcome afterward; each snapshot permits three fresh-token dispatch generations, while a fourth requires fresh preview/approval with signed monotonic approval evidence. HMAC/reservation internals and raw tokens are never UI fields. Handoff statuses and readiness are truthful and never claim delivery; dry run writes nothing. First-Admin `Kom igång` checklist remains 5-item, server-derived, dismissable, pinned until done, and deep-linked; **N-2 rules out public signup permanently**.
...
An allow-listed operator provisions a Swedish non-personal legal-entity tenant end-to-end (strict request → immutable-catalogue write-nothing preview → hash-bound approval → atomic DB baseline → reconciliation-first bounded first-Admin Auth handoff → exact readiness → onboarding checklist) with zero engineering steps. **N-2 (2026-07-26): self-serve signup is permanently out** — not a seam being built toward. The 2026-09-17 and 2026-09-19 owner contract fixes canonical organisation identity, email/VAT canonicalization, request-plus-identity idempotency, sole-RPC authority, hashed Epic 11 tokens, provider attempts, exact states/readiness, non-granting platform permission metadata, and the strict v1 baseline/rejection list. Phase B ships no AI provisioning flow. Subscription terms (`SubscriptionPlan`, `IncludedUsers`, `AdditionalUserPrice`, `EnabledModules`, `CommercialOverrides`, …) are stored as **data** — no price is ever hardcoded (architecture §15.4A).
...
**Activation:** The manifest `provisioning` platform module becomes active with its platform surfaces and the non-granting `Platform.Operator.Access` row in the same change; tenant-module entitlement/nav/table derivations exclude it.
...
**Scope:** `platform_operators` + `is_platform_operator()`; migration-owned insert-only `tenant_provisioning_baselines` + TypeScript coherence mirror; non-granting `Platform.Operator.Access`; strict-v1 sole `provisionTenant` DEFINER RPC under normal operator JWT plus server HMAC for mutation; dedicated least-privilege owner/grants; Swedish non-personal legal-entity identity and exact email/VAT canonicalization; dual idempotency; stateless hash-bound preview/approval with signed monotonic approval generation; DB-first signed dispatch reservation/Auth-second exact-generation outcome; hash-only token lifecycle and bounded explicit dispatch attempts; exact readiness; operator console at `/operator` (identity/status only); three-step provisioning wizard plus approval confirmation; `Kom igång` onboarding checklist; NFR54 negative suite.
...
**Risks:** DEFINER RPC/action-variant or owner/grant misuse; attestation forgery/replay/domain confusion/key leakage; platform permission metadata accidentally becoming tenant-grantable; operator console leaking tenant business data or raw invite tokens; request/identity replay creating duplicate/silently updated tenants or provider calls; DB/TypeScript catalogue drift; stale reservation/outcome or caller-supplied retry identity; attempt-limit/token-rotation defects; post-commit Auth uncertainty misreported as failure/delivery/readiness; onboarding checklist claiming completion it cannot verify.
...
**And** activation adds `Platform.Operator.Access` as `scope: platform`, `tenantGrantable: false`, `tenantRoles: []`; the operator registry may consume it, but authorization remains exclusively `is_platform_operator()` and tenant roles, entitlements, nav, and module settings exclude it.

**Given** an operator invokes `provisionTenant`
**When** the command executes
**Then** the server calls the `provision_tenant` SECURITY DEFINER RPC under the operator's normal Supabase Auth JWT; the hardened RPC derives and rechecks current allow-listed `auth.uid()`, and every mutating action also requires a valid domain-separated server HMAC attestation before it creates the tenant, applies the exact migration-owned catalogue baseline, creates the first-Admin invited membership, stores identity/idempotency facts, and sets `pending_first_admin_invite` atomically
**And** `authenticated` has EXECUTE on this exact RPC only; `PUBLIC`, `anon`, `authenticator`, and `service_role` have neither provisioning-RPC execution nor provisioning-table DML, no caller inherits the dedicated least-privilege `NOLOGIN NOINHERIT` function-owner role, and that owner has only required table operations plus filtered Vault access
**And** the initial transaction and a separately signed `reserve_dispatch` commit before the Supabase Auth Admin invitation runs server-side in the command, never in the RPC and never client-reachable
**And** the sole RPC's narrow action variants may only perform initial atomic provisioning, signed dispatch reservation, exact-reservation requested/unknown/failed outcome recording, provider-free reconciliation recording, and transition audit; no legacy delegate/fallback, general service-role DB mutation, custom provisioning JWT, or second callable DEFINER writer is added
**And** every provisioning action is audited with the new tenant's tenant_id, while the existing Epic 11 acceptance path may mark `ready` when it activates the first-Admin membership.

**Given** a provisioning mutation
**When** server provisioning code authorizes it
**Then** it mints an internal HMAC-SHA-256 attestation using a dedicated 256-bit key separate from JWT, service-role, and quote-PDF keys, with explicit key ID, the established length-prefixed Node/Postgres encoding, fail-closed verification, maximum two-minute TTL, and current/previous-key overlap bounded to that TTL
**And** the attestation binds action/schema, actor, request/canonical projection+hash, organisation identity, preview hash/approval, baseline ID/version/hash, invitation-token hash, exact invitation/membership/reservation IDs, absolute dispatch and approval generations, expected current generation, sanitized outcome, attestation/key IDs, and issued/expiry
**And** it is held only in the app/Vault authority boundary and is never persisted, logged, audited, or returned to the browser; unsigned, forged, expired, wrong-domain, wrong-key, or tampered requests fail generically with zero writes.

**Given** a v1 tenant identity
**When** the request is validated
**Then** only `country_code = 'SE'` plus a normalized, checksum-valid ten-digit organisation number for a non-personal legal entity is accepted; spaces/hyphens are removed, personnummer-shaped identities and sole proprietorship tenants are rejected, and optional VAT is trimmed/uppercased/space-stripped and accepted only as `SE` + the same organisation number + `01`, without becoming an alternate identity
...
**Given** the first-Admin email
**When** it is normalized
**Then** the server trims and applies Unicode NFC, IDNA-normalizes and lowercases the domain, lowercases the local part by product policy, preserves plus-addressing, strips neither dots nor tags, and rejects display names, comments, malformed syntax, or multiple addresses.

**Given** request and organisation identity replay
**When** the command re-runs or races
**Then** the same request UUID/hash reconciles membership/invitation/Auth identity first and returns tenant ID/current state/attempt/reconciliation action without implicit provider work; the same UUID with different content returns `IDEMPOTENCY_CONFLICT`; a different UUID for the same canonical organisation returns `ALREADY_PROVISIONED` with identity/status; exactly one tenant exists and no path silently updates it.

**Given** an allow-listed operator requests a dry run
**When** the server previews schema v1
**Then** it writes no DB, audit, Auth, or preview row and returns schema/request identity, normalized identity, validations/warnings, `CREATE | ALREADY_PROVISIONED | CONFLICT`, exact baseline ID/monotonic version/content hash, every proposed tenant/company/commercial/module value, first-Admin name/normalized email, proposed audit events, unsupported/deferred fields, and a server-calculated `preview_hash`.
...
**Then** the server re-normalizes/re-hashes, derives the approver only from the authenticated allow-listed `auth.uid()`, reloads the exact insert-only database catalogue row and recalculates its content hash, rejects mismatch, returns `PREVIEW_STALE` if the row is missing or changed, and audits approver/time/request/hash/baseline ID/version/hash plus signed monotonic approval generation; the same operator may preview and approve and no four-eyes rule applies
**And** published catalogue rows are migration-owned and immutable, changes insert a new monotonic version, no runtime role has DML, requests select ID/version rather than supply arbitrary values, and mandatory tests prove the TypeScript preview/build-time mirror exactly matches the database authority.
...
**And** the initial authorised dispatch and every explicit resend each generate a fresh 32-byte CSPRNG application token in server memory, sign/store only SHA-256, bind the current generation to invitation/tenant/membership/normalized email/`tenant_admin`/expiry, and never pass a raw token into the RPC, storage, operator/browser output, logs, or audits
**And** same-request replay and reconciliation never rotate and never dispatch; each authorised explicit resend uses signed `reserve_dispatch` to atomically revoke the prior hash, install the fresh hash/current generation, reserve the attempt, and return the durable normalized email/membership/role/expiry/reservation facts from which provider input is derived, invalidating all older links
**And** at most one provider call follows the committed reservation; its sanitized outcome is separately attested for that exact reservation/generation, while stale, out-of-order, or conflicting outcomes and caller-supplied retry identity fail closed
**And** provider resend requires explicit `retry_first_admin_invite` after reconciliation, never auto-resends from `unknown`, and permits three total fresh-token dispatches per approved snapshot; a fourth requires fresh preview/approval expressed as signed monotonic approval-generation evidence, never a trusted `fresh_approval` boolean
**And** v1 adds no recoverable raw-token escrow, deterministic derivation, or provider/callback redesign; this Decision 7C contract supersedes the earlier same-token-reuse-across-dispatches wording
**And** every result returns tenant ID, state, attempt number, and reconciliation action.

**Given** the first-Admin membership is accepted
**When** the system evaluates `ready`
**Then** it requires completed atomic DB provisioning, recorded approved baseline ID/version/content hash, active first-Admin membership, non-null Auth user ID, matching normalized Auth email, and no unresolved definitive provisioning failure; provider acceptance alone is insufficient.

**Given** NFR54 negatives
**When** the suite runs
**Then** non-operators are denied generically; provisioning for tenant X can neither read nor affect tenant Y (cross-tenant negatives on the provisioning path); forged/absent operator claims and unsigned/forged/expired/wrong-domain/tampered attestations are rejected with zero writes.

**Technical Notes:** Architecture §3.3/§14/§15.4/§9.1. Provisioning identity, request hash, baseline ID/version/content hash, normalized first-Admin email, handoff status, absolute approval/dispatch generation, reservation ID, attempt counter, and sanitized outcome live as durable additive facts/constraints; the attestation and raw token do not. No preview table is introduced. Coordinated rollout provisions the app/Vault key first, deploys the compatible signer/server path, then applies the enforcement migration/removes legacy authority; no secret is committed and current/previous overlap is bounded by the two-minute TTL. The onboarding checklist is derived server-side, not stored (per architecture — per-admin dismissal is a small column). The strict v1 fields and rejection list are binding from architecture §15.4A.

**Test Requirements:** RPC/action integration tests (atomicity, idempotency/concurrency, dual JWT+attestation proof, operator gate, exact grants/owner authority, durable identity uniqueness); unsigned/forged/expired/wrong-domain/wrong-key/tampered attestation zero-write negatives; strict schema/identity/VAT/email tests; migration-owned catalogue immutability and DB/TypeScript coherence/staleness tests; CSPRNG/SHA-256-only token binding, signed reservation, durable-fact provider input, per-dispatch generation, prior-hash revocation, old-link rejection, exact-generation/out-of-order outcome rejection; DB/Auth state, readiness, replay/reconciliation no-dispatch proof, explicit one-call retry, three-dispatch cap, signed fourth-attempt approval generation, and response-loss fault injection; platform capability tests; legacy-function/service-role DML absence; audit/log/browser secret-absence; migration reset green.

**Security/RLS Impact:** Very high. The sole callable DEFINER provisioning surface of B1a; normal JWT plus server HMAC, dedicated least-privilege owner, exact grants, filtered Vault access, and full negative treatment are mandatory.

**Money/Tax/Quote Impact:** None.

**Migration/Coexistence Impact:** Additive platform/operator, migration-owned baseline catalogue, owner/grant, and tenant identity/idempotency/handoff schema. The enforcement migration removes the legacy delegate and broad service-role provisioning DML only after the app/Vault key and compatible signer/server deploy. Existing tenants must participate in the same canonical identity reservation; missing/colliding identity is a migration stop, not an exemption. Legacy `register_company` P2 remains the thinned behavioral reference only.

**Dependencies:** Epic 11 (membership/invite machinery).

**Stop Conditions Requiring Human Approval:** Stop if any additional callable SECURITY DEFINER writer beyond `provision_tenant`, service-role database call, custom provisioning JWT, recoverable token escrow, or committed secret appears necessary; provisioning would write into another tenant's rows; a tenant outside the approved v1 identity class must be supported; or implementation needs to widen/change the strict schema, attestation binding, catalogue, approval generation, idempotency, or Auth-handoff contract.
...
**Then** three data-entry steps execute — 1) Företagsuppgifter (legal name, SE organisation identity, optional canonical matching VAT/contact data), 2) Baslinje (exact migration-owned database catalogue ID/version/content hash and all proposed values displayed from the TypeScript preview mirror, not re-typed), 3) Bjud in första Admin (name + normalized email) — followed by the hash-bound preview and explicit approval confirmation
**And** the wizard is re-entrant from durable server state: a dry run writes nothing, execution resubmits the original request/hash, an interrupted handoff displays tenant/state/attempt/reconciliation action, and replay reconciles before rendering `ALREADY_PROVISIONED` instead of duplicate-create success
**And** provider dispatch is an explicit `retry_first_admin_invite` action after reconciliation: the server reserves the fresh hash/generation, uses only durable RPC-returned provider facts, makes at most one call, and records the exact-generation signed outcome; replay/reconciliation never rotate or dispatch and `unknown` never blind-resends; attempts 1–3 are distinct fresh-token generations whose sanitized outcomes are visible; before attempt 4 the UI requires a fresh preview/approval backed by a newer signed monotonic approval generation and explains that every older link becomes invalid
...
**Test Requirements:** AuthZ tests (operator vs tenant user vs anonymous); platform-capability tenant-exclusion tests; strict field-level validation and unsupported-field presentation; write-nothing preview/hash/approval/stale-catalogue tests; wizard resume/reconciliation-first replay and explicit handoff-attempt/token-rotation tests; console raw-token/business-data exposure negatives; shell-isolation check (no tenant-context imports in `operator/**`).
...
**When** the app loads
**Then** the tenant already satisfies Story 12.1's exact `ready` predicate, and the `Kom igång` checklist renders (pinned on the dashboard until done, dismissable and resumable) with five items: Företagsinställningar, Moms & visning, Offertvillkor (carrying the Phase A sign-off warning posture), Arbetsroller & priser, Bjud in användare
