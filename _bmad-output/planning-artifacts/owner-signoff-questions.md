...
**Provisioning amendment (2026-09-17; supplemented by Decisions 1–7C on 2026-09-19):** the owner closed the Story 12.1 intent-gap rounds. Provisioning v1 is internal-only and limited to Swedish non-personal legal entities; it uses strict schema allow-lists, canonical organisation identity plus request-hash idempotency, a stateless hash-bound preview/approval, and DB-first/Auth-second handoff. The supplement fixes the sole RPC's narrow post-provider authority, the fresh-token/SHA-256-only Epic 11 dispatch lifecycle, observation-only reconciliation/replay, three total dispatches per approved snapshot with fresh approval before a fourth, immutable baseline ID/version/content-hash catalogue, exact email/VAT canonicalization and readiness, and non-granting `Platform.Operator.Access` metadata. Phase B ships no AI provisioning flow.

The supplement's Decisions 1–7C are binding: the sole `provision_tenant` RPC may only perform initial provisioning, record requested/unknown/failed, record reconciliation/attempt-generation results, and audit transitions; Epic 11 acceptance may mark `ready`. The initial authorised dispatch and every explicit resend each generate a fresh 32-byte CSPRNG token in memory and persist only SHA-256 bound to invitation/tenant/membership/normalized email/`tenant_admin`/expiry. Replay and reconciliation never rotate or dispatch. After reconciliation, explicit `retry_first_admin_invite` atomically revokes the prior hash and reserves the new hash/attempt through the sole RPC before one provider call, invalidating older links; raw tokens never reach storage, operator/browser output, logs, or audits. `unknown` never auto-resends. One approved snapshot permits three total dispatches; a fourth requires fresh preview/approval and starts a new invitation generation. No escrow, deterministic derivation, or provider/callback redesign is added. Decision 7C explicitly supersedes earlier same-token reuse across provider attempts. `TENANT_PROVISIONING_BASELINES` entries are immutable ID/monotonic-version/content-hash records. Email uses trim/NFC, IDNA+lowercase domain, lowercase local, preserved plus/dots/tags, and strict syntax rejection; VAT is `SE` + matching ten-digit organisation number + `01`. `ready` requires DB completion, recorded baseline ID/version/hash, active first-Admin membership, non-null matching Auth identity, and no unresolved definitive failure. `Platform.Operator.Access` is `scope: platform`, `tenantGrantable: false`, `tenantRoles: []`; `is_platform_operator()` remains the exclusive authority and tenant consumers exclude the row.
...
| ~~N-2~~ | ~~Business model / per-company pricing / provisioning flow (Roadmap 7)~~ — **resolved 2026-07-26; detailed 2026-09-17; supplemented 2026-09-19:** no self-serve/AI flow; allow-listed operator only; Swedish non-personal legal entities; strict v1; exact email/VAT canonicalization; immutable baseline catalogue; write-nothing hash-bound preview/approval; dual idempotency; sole narrow RPC authority; hash-only Epic 11 token lifecycle; reconciliation-first bounded dispatch; exact readiness; non-granting platform capability; commercial terms remain tenant data | resolved; Story 12.1 returns to draft with no live owner question |
