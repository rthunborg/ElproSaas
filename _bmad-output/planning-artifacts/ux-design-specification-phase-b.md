...
- After database commit, the first-Admin panel surfaces `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, or `ready` with a truthful Swedish explanation. `requested` says the provider accepted the request, never that email was delivered; timeout uses `unknown`, not failure, and does not trigger automatic resend. The panel always offers reconciliation before provider work; `Försök skicka inbjudan igen` is an explicit `retry_first_admin_invite` action, makes at most one provider call, and displays the persisted attempt (1–3) plus sanitized outcome. After attempt 3 it requires a fresh preview/approval and explains that the prior invitation is revoked and replaced; no raw token is displayed.
