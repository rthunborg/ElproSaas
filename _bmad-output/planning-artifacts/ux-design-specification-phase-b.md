...
- Three data-entry steps: **1) Företagsuppgifter** (legal name, `SE` organisation identity, optional VAT normalized to `SE` + matching ten-digit organisation number + `01`, contact/profile data; explain that individuals and `enskild firma` are unsupported tenants), **2) Baslinje** (exact immutable server catalogue ID/monotonic version/content hash with `sv-SE`, `Europe/Stockholm`, `SEK`, tax-profile references, terms warnings, number-series and conservative display defaults shown, not re-typed), **3) Bjud in första Admin** (name + normalized e-mail). E-mail help/error behavior reflects trim/NFC, IDNA/lowercase domain and lowercase-local policy, preserves plus-addressing/dots/tags, and rejects display names, comments, malformed input, and multiple addresses. This tenant rule does not affect CRM end customers, which may be companies or private individuals without personnummer capture.
...
- `Godkänn och provisionera` is enabled only for a clean preview. It resubmits the original request and server hash; a changed request requires a new preview, and missing/changed catalogue content returns `PREVIEW_STALE` with `Baslinjen har ändrats – granska igen`. The approval summary shows authenticated approver, approval time, request ID, preview hash, and baseline ID/version/content hash.
...
- After database commit, the first-Admin panel surfaces `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, or `ready` with a truthful Swedish explanation. `requested` says the provider accepted the request, never that email was delivered; timeout uses `unknown`, not failure, and does not trigger automatic resend. The panel always offers observation-only reconciliation before provider work; replay/reconciliation never rotates a token or dispatches. `Försök skicka inbjudan igen` is an explicit `retry_first_admin_invite` action, makes at most one provider call, and displays the persisted attempt (1–3) plus sanitized outcome. Each displayed attempt is a distinct fresh-token dispatch generation whose prior link is invalidated; before attempt 4 the panel requires a fresh preview/approval and explains that a new invitation generation invalidates all older links. No raw token is displayed.
...
**No self-serve signup (N-2, answered 2026-07-26; contract amended 2026-09-17 and 2026-09-19):** every customer company is registered and provisioned internally after a contract. No public registration surface, route, or copy ships. Phase B ships no AI provisioning interaction and rejects free-text agent instructions. The deterministic operator flow uses the strict request/canonicalization, immutable baseline catalogue, write-nothing preview, server hash, explicit approval, dual idempotency, narrow sole-RPC authority, hashed Epic 11 tokens, explicit bounded attempts, and exact DB/Auth readiness in architecture §15.4A.
...
**Flow 5 — An operator provisions the second tenant (Journey B5).** 1) Operator console → `Provisionera ny tenant`: eligible SE legal-entity details → exact immutable baseline ID/version/hash → normalized first-Admin identity. 2) `Förhandsgranska` returns `CREATE`, all values and the hash while `Inget har sparats` remains true. 3) The same authenticated operator approves; database state commits. Replay/reconciliation reports tenant/state/attempt/action without rotation or a provider call. Each explicit retry makes one dispatch with a fresh 32-byte in-memory token and SHA-256-only persistence; `unknown` never blind-resends. The approved snapshot allows three total generations, and a fourth needs fresh preview/approval and a new invitation generation that invalidates older links. 4) Epic 11 acceptance activates the membership; `ready` appears only when the exact DB/baseline/membership/Auth-email/no-failure predicate holds, never merely because the provider accepted a request. 5) The new Admin signs in; `Kom igång` is pinned and they complete settings → moms → villkor → arbetsroller → invite five users with roles. 6) **Climax:** all items green — a working tenant, zero engineering steps (AC-B1a-1); replay returns the existing tenant, and nothing can read or touch another tenant (NFR54).
...
| N-2 + Story 12.1 owner contract | Provisioning/onboarding (§4.3) | ✅ **Closed 2026-07-26; amended 2026-09-17 and 2026-09-19.** No public signup or Phase B AI provisioning surface. The operator wizard is strict-v1, Swedish-non-personal-legal-entity only, canonicalizes email/VAT exactly, previews immutable baseline ID/version/hash without writes, binds approval to the server hash, reconciles replay before explicit bounded provider attempts, keeps Epic 11 tokens raw-secret/hash-only, and renders exact readiness truthfully. |
