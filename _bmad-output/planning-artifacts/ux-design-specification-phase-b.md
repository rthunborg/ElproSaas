...
- Three data-entry steps: **1) Företagsuppgifter** (legal name, `SE` organisation identity, optional consistent VAT/contact/profile data; explain that individuals and `enskild firma` are unsupported tenants), **2) Baslinje** (exact server-owned profile/version with `sv-SE`, `Europe/Stockholm`, `SEK`, tax-profile references, terms warnings, number-series and conservative display defaults shown, not re-typed), **3) Bjud in första Admin** (name + normalized e-mail). This tenant rule does not affect CRM end customers, which may be companies or private individuals without personnummer capture.
- `Förhandsgranska` is a dry run and visibly states `Inget har sparats`. The review shows schema/request ID, normalized identity, validations/warnings, action (`CREATE`, `ALREADY_PROVISIONED`, or `CONFLICT`), exact baseline profile/version, every tenant/company/commercial/module value, first-Admin name/e-mail, proposed audit events, and unsupported/deferred fields. Unknown/rejected fields get field-level errors; deferred future fields may be shown but disable approval until removed.
- `Godkänn och provisionera` is enabled only for a clean preview. It resubmits the original request and server hash; a changed request requires a new preview, and `PREVIEW_STALE` shows `Baslinjen har ändrats – granska igen`. The approval summary shows authenticated approver, approval time, request ID, preview hash, and baseline version.
- Re-entry derives from durable server state, not completed browser steps. Same request replay returns the original result; same request ID with changed content surfaces `IDEMPOTENCY_CONFLICT`; another request for the same canonical organisation surfaces `ALREADY_PROVISIONED`. None offers an implicit update.
- After database commit, the first-Admin panel surfaces `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, or `ready` with a truthful Swedish explanation and operator retry where applicable. `requested` says the provider accepted the request, never that email was delivered; timeout uses `unknown`, not failure. Retry first reconciles existing membership/invitation/Auth identity by normalized email and does not offer a blind resend.
...
**No self-serve signup (N-2, answered 2026-07-26; contract amended 2026-09-17):** every customer company is registered and provisioned internally after a contract. No public registration surface, route, or copy ships. Phase B ships no AI provisioning interaction and rejects free-text agent instructions. The deterministic operator flow uses the strict request, write-nothing preview, server hash, explicit approval, dual idempotency, and DB-first/Auth-second states in architecture §15.4A.
...
**Flow 5 — An operator provisions the second tenant (Journey B5).** 1) Operator console → `Provisionera ny tenant`: eligible SE legal-entity details → exact baseline → first-Admin identity. 2) `Förhandsgranska` returns `CREATE`, all values and the hash while `Inget har sparats` remains true. 3) The same authenticated operator approves; database state commits, then invitation handoff reports `requested`, `unknown`, `failed`, or `ready` without claiming delivery. 4) The new Admin signs in; `Kom igång` checklist is pinned and they complete settings → moms → villkor → arbetsroller → invite five users with roles. 5) **Climax:** all items green — a working tenant, zero engineering steps (AC-B1a-1); replay returns the existing tenant, and nothing can read or touch another tenant (NFR54).
...
| N-2 + Story 12.1 owner contract | Provisioning/onboarding (§4.3) | ✅ **Closed 2026-07-26; amended 2026-09-17.** No public signup or Phase B AI provisioning surface. The operator wizard is strict-v1, Swedish-non-personal-legal-entity only, write-nothing preview + server-hash approval, dual-idempotent, and truthful across DB-first/Auth-second handoff states. |
