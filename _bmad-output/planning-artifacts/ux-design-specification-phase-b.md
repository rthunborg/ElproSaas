...
- B1a scope is **seed roles, no custom role builder** (UXB-A7): the five tenant roles listed with description and member count; per-role view shows only tenant-grantable permission-matrix rows grouped by module — the matrix visibly **grows as tenant modules activate** (PB-D2): a tenant module's rows appear only when it is manifest-active, with the activating wave noted. Platform-scoped rows such as non-granting `Platform.Operator.Access` never appear in tenant role assignment, effective entitlements, tenant nav, or tenant module settings; the separate operator route registry may consume them.
...
**Operator console (FR73, FR75):** a separate operator-scoped area (not in tenant nav; hosting/auth model is an architecture item). Only the authenticated allow-listed platform operator may preview or approve; approver identity is never an editable field. The same operator may do both — no four-eyes interaction is required.

- Tenant list (legal name, `SE` + normalized organisation number, tenant status, created, exact first-Admin handoff state) → `Provisionera ny tenant`. Identity formatting or legal-name variation does not create a second row; inactive/archived matches show `Redan provisionerad` and direct the operator to reactivation rather than edit/duplicate. No cross-tenant data is visible beyond tenant identity/status (NFR54 posture).
- Three data-entry steps: **1) Företagsuppgifter** (legal name, `SE` organisation identity, optional VAT normalized to `SE` + matching ten-digit organisation number + `01`, contact/profile data; explain that individuals and `enskild firma` are unsupported tenants), **2) Baslinje** (exact migration-owned database catalogue ID/monotonic version/content hash with `sv-SE`, `Europe/Stockholm`, `SEK`, tax-profile references, terms warnings, number-series and conservative display defaults shown through the coherence-tested TypeScript preview mirror, not re-typed), **3) Bjud in första Admin** (name + normalized e-mail). E-mail help/error behavior reflects trim/NFC, IDNA/lowercase domain and lowercase-local policy, preserves plus-addressing/dots/tags, and rejects display names, comments, malformed input, and multiple addresses. This tenant rule does not affect CRM end customers, which may be companies or private individuals without personnummer capture.
- `Förhandsgranska` is a dry run and visibly states `Inget har sparats`. The review shows schema/request ID, normalized identity, validations/warnings, action (`CREATE`, `ALREADY_PROVISIONED`, or `CONFLICT`), exact baseline ID/version/content hash, every tenant/company/commercial/module value, first-Admin name/e-mail, proposed audit events, and unsupported/deferred fields. Unknown/rejected fields get field-level errors; deferred future fields may be shown but disable approval until removed.
- `Godkänn och provisionera` is enabled only for a clean preview. It resubmits the original request and server hash; a changed request requires a new preview, and missing/changed database catalogue content returns `PREVIEW_STALE` with `Baslinjen har ändrats – granska igen`. The approval summary shows authenticated approver, approval time, request ID, preview hash, and baseline ID/version/content hash. The server's HMAC attestation and key are deliberately invisible: they are never fields, browser output, logs, or audit content.
- Re-entry derives from durable server state, not completed browser steps. Same-request replay reconciles first and shows tenant ID, current state, attempt number, and reconciliation action without provider work; same request ID with changed content surfaces `IDEMPOTENCY_CONFLICT`; another request for the same canonical organisation surfaces `ALREADY_PROVISIONED`. None offers an implicit update.
- After database commit, the first-Admin panel surfaces `pending_first_admin_invite`, `first_admin_invite_unknown`, `first_admin_invite_requested`, `first_admin_invite_failed`, or `ready` with a truthful Swedish explanation. `requested` says the provider accepted the request, never that email was delivered; timeout uses `unknown`, not failure, and does not trigger automatic resend. The panel always offers provider-free reconciliation before provider work; replay/reconciliation never rotates or dispatches. `Försök skicka inbjudan igen` is an explicit `retry_first_admin_invite` action whose server path first reserves the exact new generation, derives email/membership/role/expiry only from durable RPC-returned facts, then makes at most one provider call and records a signed sanitized outcome for that reservation. The UI displays attempt 1–3 and outcome, never caller-editable retry identity, reservation internals, HMAC proof, or raw token. Before attempt 4 it requires a fresh preview/approval backed by a newer signed monotonic approval generation and explains that a new invitation generation invalidates every older link.
- `ready` is shown only after completed DB provisioning, recorded approved baseline ID/version/hash, active first-Admin membership with non-null Auth user ID, matching normalized Auth email, and no unresolved definitive failure. Provider request acceptance alone never renders success.
...
**No self-serve signup (N-2, answered 2026-07-26; contract amended 2026-09-17 and through Decision 8A on 2026-09-19):** every customer company is registered and provisioned internally after a contract. No public registration surface, route, or copy ships. Phase B ships no AI provisioning interaction and rejects free-text agent instructions. The deterministic operator flow uses strict request/canonicalization, migration-owned catalogue with a preview mirror, write-nothing preview, explicit hash-bound approval, dual idempotency, the sole normal-JWT/server-HMAC RPC boundary, signed reservation/outcome sequencing, hashed Epic 11 tokens, bounded attempts, and exact DB/Auth readiness in architecture §15.4A. These authority mechanics remain server-internal and add no second-person UI.
...
**Preferences (per user):** `Notisinställningar` under the profile menu — matrix rows = notification categories (grouped by module), columns = `I appen` / `E-post`. The e-post column renders but is **inactive with an explainer** ("e-postutskick aktiveras senare") until real sending is enabled for that deployment under ADR-B011; in-app toggles are live from B1a. Essential/security notices are non-disableable and marked so.

**Email touchpoints (N-6 answered 2026-07-26; ADR-B011 release boundary):** the sender renders as **`[Företagsnamn] via [Systemnamn]`** from the central verified subdomain, with **Reply-To set to the tenant's own address** so customer replies reach the company, not us. Flow priority: invitations/security → customer quote PDF attachment → internal accept/reject notification → job assignment → quote reminders → digests; later-module flows wait for their manifest activation. **Quote reminders stop automatically** on accept, reject, withdrawal, a new version, or expiry — the reminder settings UI must state those five conditions so nobody expects to have to cancel manually. Invoice emails come from **Fortnox**, not from us, so the customer never receives two. Story 13.4 proves the provider path in a sandbox while real-recipient delivery stays off until separate owner go-live. Every outbound email carries tenant identity and a clear subject convention; authenticated internal mail may deep-link to the record, while Phase B customer quote email attaches the valid PDF and has no public quote-view/accept/reject link. Non-essential mail carries a tokenized ADR-B004 unsubscribe link. Unsubscribe landing page: minimal, confirms scope ("du får inte längre påminnelser via e-post"), offers re-subscribe, no login required, no tenant enumeration.
...
| Notifications | Unread/read, category-muted, email-column inactive until ADR-B011 go-live, producer-freshness stamp, empty ("inga notiser"). |
...
| Background producers | Last-scan recency, scan-failure surfaced to Admin (NFR47 visibility), "queued/not yet sending" email posture before ADR-B011 go-live. |
...
**Flow 5 — An operator provisions the second tenant (Journey B5).** 1) Operator console → `Provisionera ny tenant`: eligible SE legal-entity details → exact migration-owned baseline ID/version/hash → normalized first-Admin identity. 2) `Förhandsgranska` returns `CREATE`, all values and the hash while `Inget har sparats` remains true. 3) The same authenticated operator approves; server-internal JWT+HMAC authority commits database state without exposing the attestation. Replay/reconciliation reports tenant/state/attempt/action without rotation or provider work. Each explicit retry reserves one fresh-token generation, uses only durable RPC-returned provider identity facts, makes at most one call, and records the signed sanitized outcome; `unknown` never blind-resends. The snapshot allows three total generations, and a fourth needs fresh preview/approval with monotonic approval evidence. 4) Epic 11 acceptance activates the membership; `ready` appears only for the exact predicate. 5) The new Admin signs in; `Kom igång` is pinned and they complete settings → moms → villkor → arbetsroller → invite five users with roles. 6) **Climax:** all items green — a working tenant, zero engineering steps (AC-B1a-1); replay returns the existing tenant, and nothing can read or touch another tenant (NFR54).
...
| N-2 + Story 12.1 owner contract | Provisioning/onboarding (§4.3) | ✅ **Closed 2026-07-26; amended 2026-09-17 and through Decision 8A on 2026-09-19.** No public signup or Phase B AI provisioning surface. The operator wizard is strict-v1, Swedish-non-personal-legal-entity only, canonicalizes email/VAT, previews the migration-owned catalogue without writes, binds approval to the server hash, and reconciles before explicit bounded attempts. Server-internal normal-JWT/HMAC authority, signed reservation/outcome ordering, raw-secret/hash-only tokens, monotonic reapproval, and exact readiness preserve truthful UX without exposing security material. |
...
| N-6 | Notification preferences e-mail column, email touchpoints (§4.4) | ✅ **Closed 2026-07-26.** Sender shows as `[Företagsnamn] via [Systemnamn]` with the tenant's own Reply-To; flow priority and the five automatic reminder-stop conditions defined; invoices come from Fortnox, not from us (arch §4.6). ADR-B011 keeps real delivery separately owner-gated and quote email free of online acceptance. |
