...
- [Phase 8 â€” nfr] R-817 MIME check is client-declared File.type, not content-sniffed â€” honest residual cleared by security review under demo-data-only; re-open on any move to real-customer files.
...
- [Phase 5 â€” dev-story] readFileIndex had to be injectable (not a raw-query mirror) for the archived-drop RLS test to be meaningful, and FORBIDDEN_DEFERRED_CATEGORIES had to live outside guardrail-scanned index sources (its own 'fortnox' token trips the scanner) â€” two non-obvious traps for future file-index work.
- [Phase 5 â€” dev-story] Sent-quote locked-PDF surface required a NEW read-only CommitmentFilesPanel: quote_version is not an active upload owner type and EntityFilePanel's prop is compile-time-pinned to ActiveOwnerType â€” the pin quietly forces a second read-only panel variant for commitment files.
- [Phase 6 â€” automate] Archive-error classifier was left inlined in the use-server action while the twin upload classifier was correctly extracted â€” a consistency miss dev should have caught given the story cites the coverage-shape lesson; now aligned (classifyArchiveError pure module).
