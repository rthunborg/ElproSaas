...
| 13.4-AC7 Current private PDF only, revalidated before submit, no public link, and all five terminal reminder conditions stop enqueue/send | P0 | PARTIAL | Current/no-public-link `13.4-INT-004` — `tests/integration/email/quote-delivery-attachment.atdd.int.test.ts:9`; stale/missing/checksum/currentness rejection `:20,29`; processor-seam five-state loop `:41`; real database/producer no-enqueue cases for accepted, rejected, lost/withdrawn, superseded, and expired — `tests/integration/notifications/notifications.atdd.int.test.ts:55`. Missing: the claimed delivery/reminder is not rechecked against terminal quote state at the final pre-provider submission boundary. |
