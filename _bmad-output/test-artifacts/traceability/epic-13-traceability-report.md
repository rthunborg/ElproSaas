...
sourceSha: 'b5004cf00792e740af299af768035eee43af61c3'
tempCoverageMatrixPath: 'C:\tmp\tea-trace-coverage-matrix-2026-09-24T19-44-15-957Z.json'
---
...
Static discovery found Epic 13 evidence at unit/static, integration/API/RLS, and production-server browser levels. The containing evidence set spans 31 files and 155 declared cases: 19 unit/static files with 104 declarations, eight integration/API/RLS files with 29 declarations, and four E2E files with 22 declarations. Several shared manifest and containment files include older-epic cases; only assertions that exercise an Epic 13 requirement are credited in the matrix.
...
| Integration/API/RLS | `job-runs.int.test.ts`, notification emission/dedupe/preferences/RLS, outbox concurrency/retry/suppression/RLS, activated delivery, quote artifact/currentness/recipient snapshot, and public unsubscribe capability | Story 13.2 records 551/551 aggregate required tests and its six focused cases. Story 13.3 records 578/578 required integration/RLS/jobs tests. Story 13.4 records a clean reset and 1,181/1 skipped required tests; the one skip is the separately configured CI-only recovery-storage proof and is outside Story 13.4 coverage. |
...
```json
{
  "liveManifestHeader": {
    ...
    "producer": "",
    "read_error": "",
    "current_source_sha": "b5004cf00792e740af299af768035eee43af61c3"
  },
  "liveRecords": []
}
```
...
- **Error paths:** Deterministic deadline/chunk resume, producer failure sanitization, notification optimistic-read recovery, concurrent dedupe, stale-claim recovery, retry exhaustion, suppression, closed release posture, missing/changed/stale quote artifacts, terminal reminder states, unknown/revoked/rate-limited tokens, and forbidden import/provider paths are represented.
...
| 13.2-AC2 Concurrent/retried due scans deduplicate and terminal work emits nothing | P0 | FULL | `13.2-INT-002` — `notifications.atdd.int.test.ts:44` runs six concurrent scans and a terminal accepted case against the database. |
| 13.2-AC3 Every tenant role gets its own capped bell; foreign user/tenant/anonymous/raw-write paths deny | P0 | FULL | `13.2-RLS-001` — `notifications.atdd.int.test.ts:59`; raw/cross-tenant/anon denial `:75`; all-role capped bell — `notifications.atdd.e2e.spec.ts:78` (runtime matrix over five roles); count helper — `tests/unit/components/notifications/notification-presentation.test.ts:25`. |
...
| 13.4-AC7 Current private PDF only, revalidated before submit, no public link, and all five terminal reminder conditions stop enqueue/send | P0 | PARTIAL | Current/no-public-link `13.4-INT-004` — `tests/integration/email/quote-delivery-attachment.atdd.int.test.ts:9`; stale/missing/checksum/currentness rejection `:20,29`; five statuses are looped at the processor seam `:41`. Missing: database/producer evidence that each named terminal state independently prevents reminder enqueue and is rechecked at the claimed send boundary. |
| 13.4-AC8 Prepared artifact plus quote finalization/outbox enqueue is atomic; failures leave auditable recoverable truth and never false sent | P0 | PARTIAL | Success through the real command is observed in `quote-delivery-recipient-snapshot.int.test.ts:26`; artifact claim/consume and missing-artifact recovery are covered by `email-delivery-activation.atdd.int.test.ts:29,87`. Missing: injected artifact-preparation and finalization/enqueue transaction-failure tests proving rollback/recovery audit behavior and absence of false finalization/outbox/sent state. |
...
- The three partial criteria are not raised merely for lacking E2E. Each omits a named P0 branch or failure boundary from the formal requirement.

## Step 4 — Coverage Gap Analysis

The automatic execution mode resolved to agent-team. Independent workers classified gaps, checked heuristic/live-evidence blind spots, and recomputed coverage statistics; the results were merged into the Phase 1 coverage matrix at `C:\tmp\tea-trace-coverage-matrix-2026-09-24T19-44-15-957Z.json`.
...
1. **13.4-AC6 — recipient cancellation and reauthorization.** Selection validation and snapshot freezing are covered. No reachable executing test changes a pending delivery's recipient, cancels it, and requires a newly authorized delivery.
2. **13.4-AC7 — reminder stop boundaries.** Current/private artifact checks and a five-status processor loop are covered. There is no database/producer proof that each named terminal state independently blocks enqueue and is rechecked at the claimed-send boundary.
3. **13.4-AC8 — atomic failure and recovery.** The real success path, artifact claim/consume, and missing-artifact recovery are covered. There are no injected artifact-preparation or finalization/enqueue transaction-failure tests proving rollback, recovery audit evidence, and absence of false finalization, outbox, or sent state.
...
- Missing dedicated browser journeys: **1**, the quote-send journey spanning recipient selection through durable queued state (`13.4-AC6`, `13.4-AC8`). This UI gap is secondary to the named execution/data-integrity gaps above.
- Missing UI states: **0**.
...
1. Add an executing recipient-change cancellation/reauthorization test and the matching reachable command contract for `13.4-AC6`.
2. Add database-backed producer tests for all five reminder terminal states at enqueue and claimed-send recheck boundaries for `13.4-AC7`.
3. Add fault injection around artifact preparation and the finalization/enqueue transaction, asserting rollback, recovery audit state, and no false success for `13.4-AC8`.
4. Add one production-server browser journey from linked recipient selection through truthful queued state after the P0 command/database gaps close.
...
**Rationale:** P0 coverage is 88% (required: 100%). 0 critical requirements are wholly uncovered, but three P0 requirements remain only partially covered and therefore do not count as FULL under the deterministic threshold.

### Decision criteria

| Criterion | Threshold | Actual | Status |
| --- | ---: | ---: | --- |
| P0 coverage | 100% | 88% | NOT_MET |
...
| Requirement | Missing acceptance evidence | Required remediation |
| --- | --- | --- |
| `13.4-AC6` | Pending-recipient change does not have a reachable cancellation/new-authorization implementation and executing test. | Implement and test change → cancel → fresh authorized delivery, including retry use of the frozen original snapshot. |
| `13.4-AC7` | The five terminal reminder states are checked only in one processor-seam loop; database enqueue prevention and claimed-send recheck are not proven. | Add one visible database/producer case per accept, reject, withdrawal, supersession, and expiry at both boundaries. |
| `13.4-AC8` | Preparation and finalization/enqueue failure boundaries lack injected rollback/recovery tests. | Inject failures before preparation completion and inside the finalization/enqueue transaction; assert recovery audit state and no false finalized, queued, or sent result. |

### Evidence summary

- Formal oracle: final acceptance criteria in Stories 13.1–13.4, high confidence.
- Requirements: 26 total, 23 FULL, three PARTIAL, zero NONE.
- Recorded execution includes the final clean reset, 1,181 required integration tests with one unrelated CI-only recovery-stack skip, 1,894 unit tests with one unrelated skip, and 173 Playwright tests passed with four explicit unrelated skips and zero failures.
- Static trace inventory: 29 uniquely mapped active cases across 17 files; no mapped skip, fixme, or pending case.
...
1. Close `13.4-AC6`, `13.4-AC7`, and `13.4-AC8` with the executing evidence above.
2. Add the quote-send browser journey after the underlying command/database branches are covered.
3. Re-run `/bmad-testarch-trace` and require 100% P0 FULL coverage before the epic gate can pass.
