...
The currently released application remains **FAIL / HIGH** for reliability until the callback repair's hosted authenticated behavior is verified: its previous server-only callback cannot consume Supabase Auth's implicit session fragment. PR [#62](https://github.com/rthunborg/ElproSaas/pull/62) at `2fb018a` passed current-head CI (1,756 units / zero skips; 1,028 required database/RLS / zero skips; 138 browser passed / four explicit skips / zero failed) and merged as `a264b54d83471432bf867f403cb4ef1f3d23a44a` at 2026-09-14T15:14:29Z. Production deployment `dpl_B5hVF5UajFn9ynE5hCLVk7t3MRpU` became READY for `a264b54` at 2026-09-14T15:15:09.968Z on the canonical alias. Bounded unauthenticated probes at 15:16Z returned `/login` 200, `/auth/invite/confirm` 307 with canonical `Location` `/auth/invite/complete`, and the completion page 200. This proves deployed routing/origin only, not a session-bearing emailed-link acceptance, hosted login, or external delivery. Dependent PR #63's pre-retarget source run at `cb5a1d3` also passed; after retargeting and cleanly merging current `main`, its new head needs a live check result. No required check is bypassed. An authenticated dashboard inspection saved the canonical hosted Site URL and single callback redirect URL, and confirmed stock `{{ .ConfirmationURL }}` invite/recovery templates with custom SMTP off. That configuration evidence is not hosted application or external-delivery evidence. Performance has a real local observation but no owner-approved target. PR #61 closed all nine recorded test-maintenance findings; maintainability retains only the separate owner decision on whether quantitative coverage/duplication targets are required.
...
| Reliability | **CONCERNS** | The corrected local transport smoke preserved multiple callback markers for invite and recovery. PR #62 passed current-head CI, merged, and its Production deployment is READY; bounded probes verify the deployed canonical callback route and completion page, but hosted session-bearing callback/login and external SMTP remain unproved. The previously tracked reset-retry command/action seam landed separately in PR #60. The hosted Site URL, single redirect URL, stock `{{ .ConfirmationURL }}` templates, and default-SMTP posture are inspected and recorded; availability/recovery targets remain open. |
| Maintainability | **CONCERNS** | PR #61 was merged and deployed at `3dd292a`; CI run 34846239517 passed 1,753 units / 0 skipped, 1,028 required integration/RLS tests / 0 skipped, and 136 browser tests with four explicit skips. Independent focused review scored 100/100 and closed all nine recorded clock, support-file, hydration/readiness, naming, and grouping findings. PR #62 current-head CI passed 1,756 units / 0 skipped, 1,028 required database/RLS / 0 skipped, and 138 browser passed / four explicit skips / zero failed. The pre-retarget PR #63 source CI also passed; its retargeted head still needs a live result. The owner decision on quantitative coverage/duplication targets remains open. |
...
Accordingly, the controlled local R-1109 transport-and-callback boundary is **PASS for the reviewed PR #62 code candidate**. The current-head full local browser result is corroborated by passing required CI, and PR #62 has merged and deployed. Bounded production probes establish canonical callback routing and completion-page availability only; they do not establish a session-bearing emailed-link journey, hosted login, or external delivery. Vercel Production has the canonical `NEXT_PUBLIC_APP_URL`; the authenticated demo-dashboard inspection saved the matching Site URL, exactly one callback redirect URL, and verified stock `{{ .ConfirmationURL }}` invite/recovery templates with custom SMTP off. That removes hosted-configuration uncertainty, but not hosted authenticated callback/login or external-delivery uncertainty. External SMTP deliverability and recipient-mailbox acceptance still require an approved non-demo Supabase project plus a controlled mailbox; no hosted delivery evidence is claimed here.

### Availability, recovery, and operational limits

The demo login probe, bounded callback-route probes, and release-window error-log inspection are useful deployability observations, but no uptime, error-rate, MTTR, retention, or alert threshold is approved. The official Vercel project error aggregate since #62 deployment readiness returned no runtime errors in the selected interval; the earlier aggregate query timed out, so this is limited release-window evidence only. These observations remain **CONCERNS**, not availability PASS evidence. The current demo backup inventory is empty and point-in-time recovery is disabled. The local logical restore rehearsal now has a scoped PASS: the 2026-09-14 final run restored intact selected `auth`, `storage`, `public`, `supabase_migrations`, and local-only `test_support` schema/data into a UUID scratch database under the preflighted existing local role, matched migration and domain-row checks, and removed the scratch database. `test_support` is included only for a local fixture trigger dependency; it is not a hosted/production schema claim. This local result supplies neither platform-role bootstrap nor Storage object-byte recovery, a hosted backup policy, or an RPO/RTO claim. Production recovery remains open until backup policy, hosted recovery method, RPO, and RTO are approved and verified.
...
nfr_assessment:
  date: '2026-09-14'
  epic: '11'
  assessed_revision: 'PR #62 deployed a264b54d83471432bf867f403cb4ef1f3d23a44a; dependent PR #63 cutoff 8c6f4e37984e41531d12a5bbd07f6de5313c2b33 plus uncommitted evidence-tool/docs work'
  mode: advisory
  overall_status: 'CONCERNS'
  overall_risk: 'MEDIUM'
  released_application_status: 'FAIL/HIGH until merged PR #62 deployed callback/login verification completes'
  domains:
    security: 'PASS'
    performance: 'CONCERNS'
    reliability: 'CONCERNS'
  ...
  advisory_open_issue: 'Hosted authenticated callback/login verification, retargeted PR #63 CI, plus performance, recovery, operational, and coverage/duplication owner decisions remain'
  measured_evidence:
    ...
    current_pr63_ci: 'https://github.com/rthunborg/ElproSaas/actions/runs/34851178821 — pre-retarget head cb5a1d3 passed: 1756 unit / 0 skipped; 101 DB files / 1028 tests / 0 skipped; 138 browser passed / 4 explicit skips / 0 failed. Retargeted head requires a live result'
    hosted_auth_configuration: '2026-09-14 authenticated dashboard inspection: Site URL https://elpro-saas.vercel.app; exactly one redirect URL https://elpro-saas.vercel.app/auth/invite/confirm; invite/recovery previews use {{ .ConfirmationURL }}; custom SMTP off; confirm email on; anonymous sign-ins and manual linking off'
    release_window_runtime_errors: 'Official Vercel project aggregate since #62 deployment readiness returned no runtime errors in the selected interval; limited release-window observation, not uptime/error-rate/retention/alert evidence'
  ...
  open_evidence:
    - 'Hosted authenticated callback/login verification for merged PR #62; bounded route probes do not establish it'
    - 'Live CI result for the retargeted PR #63 head, then merge/deployment'
    ...
    - 'Approved hosted backup/recovery policy, RPO, and RTO; the scoped local rehearsal does not establish production recovery'
    - 'Owner decision on quantitative coverage and duplication requirements'
  next_action: 'Verify merged PR #62 hosted authenticated callback/login, retain retargeted PR #63 CI, then merge/deploy it before reassessing CONCERNS.'
```
