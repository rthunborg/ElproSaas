...
The currently released application is **CONCERNS / MEDIUM** for reliability while recovery and operational evidence complete. PR [#62](https://github.com/rthunborg/ElproSaas/pull/62) passed its current-head CI, merged as `a264b54d83471432bf867f403cb4ef1f3d23a44a`, and deployed. PR [#63](https://github.com/rthunborg/ElproSaas/pull/63) then merged as `dc83665fad9e4cf269d6edad791e97383142a38b`; its required current-head CI and post-merge main CI [34863085288](https://github.com/rthunborg/ElproSaas/actions/runs/34863085288) passed. The current Production deployment is READY for `dc83665`; its existing deployment identifier is unchanged. The authorized controlled hosted invitation proof passed: a real Google SMTP-delivered message was received, the unmodified mailed link completed hosted confirmation and the application completion page, and the invited user reached the dashboard with only the isolated test-tenant membership. Private verification confirmed active membership, assigned role, and succeeded delivery record. This proved the deployed invitation acceptance leg, not user-facing invitation initiation: setup deliberately used the authenticated backend invitation-operation path and Auth Admin API rather than the Next.js UI/Server Action. The recovery leg remains in progress. Automated CI remains local-only. PR #61 closed all nine recorded test-maintenance findings. The owner has chosen risk-based regression and traceability gates for the pilot, without a blanket coverage or duplication percentage gate.
...
| Performance | **CONCERNS** | The current R-1108 probe uses the approved authenticated profile: Tenant A 120 active memberships, Tenant B 24, five roles with 24 primary assignments each, 40 secondary-role members, and 160 assignments. Across 25 samples after five warmups, Admin read p95 was **45.98 ms**, synthetic bulk effective-permissions p95 **2.00 ms**, and the projection used three authenticated RLS requests per iteration. These are local regression observations, not a production SLO or capacity limit. |
...
| Monitoring | 99.5% monthly availability; server-error rate <1%; checks every five minutes; alert after three consecutive failures; retain 30 days of operational history. | Approved policy; no 30-day observation or alert-delivery proof yet. |
...
nfr_assessment:
  ...
  overall_risk: 'MEDIUM'
  released_application_status: 'CONCERNS/MEDIUM: hosted authenticated email journey and operational evidence remain pending'
  domains:
    security: 'PASS'
    performance: 'CONCERNS'
    reliability: 'CONCERNS'
  ...
  measured_evidence:
    latest_released_ci: 'main post-merge run 34863085288 passed: 1756 unit / 0 skipped; 1028 required integration/RLS / 0 skipped; 138 browser passed / 4 explicit skips / 0 failed'
  ...
  next_action: 'Complete the authorized controlled hosted mail proof, then configure and verify the approved monitoring and backup/recovery controls before reassessing CONCERNS.'
```
