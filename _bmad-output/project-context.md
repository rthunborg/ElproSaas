...
- **Epic 3 (this epic, in code):** the FIRST tenant-owned BUSINESS tables — CRM (`customers`/`facilities`/`contacts`), company settings (`company_settings`/`quote_terms`), and pricing (`work_roles`/`articles`) — plus the first money (integer öre / basis points), the first structural sign-off gate (quote-terms `approved_at`), and the small **snapshot-source contract** (`src/lib/snapshots`) that Epic 5/6 will freeze. The tenant-owned table set is now **ten** tables (all enrolled in the H4 gate). A real browser E2E runner (Playwright) is now configured and CI-gated. **There is still NO money/VAT/ROT calculation engine — Epic 4 owns it;** Epic 3 is storage + validation + snapshot capture only.
...
- **SEK money is stored as INTEGER ÖRE (`bigint`); VAT rate is stored as BASIS POINTS** (2500 = 25.00%; default 2500, configurable, NEVER a code literal). This discipline is now LIVE in code (work-role cost/sell hourly rates, article unit prices, `company_settings.vat_rate_bp`). Two enforcement layers, both mandatory for any new money field:
  - **Command-layer validator `isOreAmount(v)`** (`src/server/commands/pricing/validation.ts`): accepts ONLY a non-negative SAFE integer number (0 .. `Number.MAX_SAFE_INTEGER`); REJECTS floats, negatives, NaN/±Infinity, overflow, locale-comma strings (`"850,00"`), decimal/numeric strings (`"850.00"`), and any string/null/object. The UI converts kronor->öre at the boundary; the command RE-VALIDATES the integer öre as the authority (the UI parse is a UX nicety only). Reuse `isOreAmount` for every new öre field.
  - **DB CHECK** (`..._ore >= 0`) belt-and-braces the lower bound; a `23514` maps to `VALIDATION_FAILED`.
- **NO money/VAT/ROT CALCULATION ENGINE exists yet — Epic 4 owns it.** Epic 3 STORES and CAPTURES money; it never computes a total, VAT amount, net/gross, or ROT/grön-teknik deduction. Do not build the engine, and do not treat a stored `vat_rate_bp` + `default_vat_display` as a computation surface. The `default_vat_display` enum (`company_togglable`/`company_excl`) is captured/stored AS-IS; the "private -> always incl-VAT" presentation rule is Epic 4's, not encoded here.
...
- **Coverage-shape lesson (load-bearing):** pull critical CONTRACT logic OUT of `"use client"` components into pure, unit-testable functions consumed at the real call site, so the fast unit gate protects it. A regression in logic that lives inline in a client component (e.g. the quote-terms approve-vs-save state precedence) escapes the fast unit gate and is only caught by slow e2e — extract it (e.g. `resolveTermsApproved()`) and unit-pin it.
...
Last Updated: 2026-07-01 (Epic 3 close — CRM, company settings, pricing foundation)
