...
profile shape no longer carries (an old persona string, say) — are normally IGNORED and
reported informationally. The one exception is the obsolete v0.24 rendered-agent profile
schema: ``--apply`` replaces those known shipped names/mappings with the current schema,
while preserving unrelated config, current-profile retunes, and valid custom profiles/routes.
That migration is idempotent; later stale/custom surface still requires ``--reset`` to prune.
...
# Exact shipped identifiers from the pre-v0.30 rendered-agent profile schema.
# They are not user namespaces: v0.24 required rendered `ab-*` agents and these
# names/mappings were module-owned. The migration removes only this closed set.
LEGACY_PROFILE_NAMES = {
    "ab-verification", "ab-deep", "ab-standard", "ab-alt-deep",
    "ab-alt-standard", "ab-security",
}
LEGACY_PHASE_PROFILE_KEYS = {
    "create_story", "dev_story", "code_review_review",
    "code_review_review_secondary", "code_review_review_tertiary",
    "code_review_security", "code_review_verification", "code_review_fix",
    "project_context", "uat",
}

# v0.24 shipped values let migration distinguish a real project retune from an
# obsolete default. Only deviations are translated; carrying the old gpt-5.5 /
# rendered-agent defaults forward would silently defeat the current routing
# policy. Several old roles collapsed into one semantic profile, so sources are
# ordered by closest meaning and non-conflicting leaves from later sources merge.
LEGACY_PROFILE_DEFAULTS = {
    "ab-deep": {"claude:model": "opus", "claude:effort": "xhigh",
                "codex:model": "gpt-5.5", "codex:reasoning_effort": "xhigh",
                "opencode:model": "", "opencode:variant": ""},
    "ab-standard": {"claude:model": "opus", "claude:effort": "high",
                    "codex:model": "gpt-5.5", "codex:reasoning_effort": "high",
                    "opencode:model": "", "opencode:variant": ""},
    "ab-alt-deep": {"claude:model": "sonnet", "claude:effort": "xhigh",
                    "codex:model": "gpt-5.4", "codex:reasoning_effort": "xhigh",
                    "opencode:model": "", "opencode:variant": ""},
    "ab-alt-standard": {"claude:model": "sonnet", "claude:effort": "high",
                        "codex:model": "gpt-5.4", "codex:reasoning_effort": "high",
                        "opencode:model": "", "opencode:variant": ""},
    "ab-security": {"claude:model": "opus", "claude:effort": "xhigh",
                    "codex:model": "gpt-5.5", "codex:reasoning_effort": "xhigh",
                    "opencode:model": "", "opencode:variant": ""},
}
LEGACY_RETUNE_SOURCES = {
    "standard": ("ab-deep", "ab-standard"),
    "critical": ("ab-security",),
    "diverse_review": ("ab-alt-deep",),
    "light": ("ab-alt-standard",),
    "default": ("ab-alt-standard",),
}
...
def detect_legacy_profile_schema(cfg_lines: Sequence[str]) -> bool:
    """True only for the closed pre-v0.30 module-owned profile surface."""
    profiles = parse_profiles_blocks(cfg_lines, find_block(cfg_lines, "profiles"))
    # A phase name by itself is insufficient: old-looking custom/stale routes
    # may live beside the current schema and ordinary additive healing must not
    # prune them. The v0.24 shipped profile names are the authoritative stamp.
    return bool(set(profiles) & LEGACY_PROFILE_NAMES)
...
def analyze(config_text: str, asset_text: str, config_version: str | None,
    ...
    legacy_profile_schema = detect_legacy_profile_schema(cfg_lines)
    ...
    needs_reseed = bool(missing_pp or missing_profiles or legacy_profile_schema)
    return {
        ...
        "legacy_profile_schema": legacy_profile_schema,
...
def _replace_top_block(text: str, name: str, replacement: list[str]) -> str:
    """Replace one top-level block, or append it when absent."""
    lines = text.splitlines(keepends=True)
    span = find_block(lines, name)
    block = list(replacement)
...
def migrate_legacy_profile_schema(config_text: str, asset_text: str) -> tuple[str, dict]:
    """Replace the closed v0.24 profile schema without losing valid overrides.

    Current shipped profile names already present in the old config retain their
    raw model/effort blocks. Other non-legacy profile names are retained as custom
    profiles. Current phase keys retain only mappings to a retained profile (or a
    deliberate blank); obsolete phase keys are removed, while unrelated valid
    custom phase keys survive.
    """
    cfg_lines = config_text.splitlines(keepends=True)
    asset_lines = asset_text.splitlines(keepends=True)
    cfg_prof = parse_profiles_blocks(cfg_lines, find_block(cfg_lines, "profiles"))
    ...
    kept_custom = [n for n in cfg_prof if n not in asset_prof and n not in LEGACY_PROFILE_NAMES]
    kept_names = set(asset_prof) | set(kept_custom)
    legacy_retunes: dict[str, dict[str, str]] = {}
    for name, defaults in LEGACY_PROFILE_DEFAULTS.items():
        if name not in cfg_prof:
            continue
        values = _profile_leaf_values(cfg_lines, cfg_prof[name]["start"], cfg_prof[name]["end"])
        changed = {key: value for key, value in values.items()
                   if key in defaults and value != defaults[key]}
        if changed:
            legacy_retunes[name] = changed

    translated: dict[str, dict[str, str]] = {}
    mapped_legacy_retunes: list[dict] = []
    legacy_retune_conflicts: list[dict] = []
    for target, sources in LEGACY_RETUNE_SOURCES.items():
        if target in cfg_prof:  # an explicit current-schema override always wins
            continue
        merged: dict[str, str] = {}
        owners: dict[str, str] = {}
        for source in sources:
            for key, value in legacy_retunes.get(source, {}).items():
                # Critical Codex work is an invariant, not a retunable legacy
                # override: never migrate it below Sol/xhigh.
                if target == "critical" and key in (
                        "codex:model", "codex:reasoning_effort"):
                    continue
                if key in merged and merged[key] != value:
                    legacy_retune_conflicts.append({
                        "target": target, "key": key,
                        "kept_source": owners[key], "kept_value": merged[key],
                        "ignored_source": source, "ignored_value": value,
                    })
                    continue
                merged[key] = value
                owners[key] = source
        if merged:
            translated[target] = merged
            mapped_legacy_retunes.extend(
                {"source": source, "target": target,
                 "keys": sorted(k for k, owner in owners.items() if owner == source)}
                for source in sources if any(owner == source for owner in owners.values()))

    def rendered_profile(name: str) -> list[str]:
        if name in cfg_prof:
            return list(cfg_lines[cfg_prof[name]["start"]:cfg_prof[name]["end"]])
        if name not in translated:
            return list(asset_lines[asset_prof[name]["start"]:asset_prof[name]["end"]])
        base = _profile_leaf_values(
            asset_lines, asset_prof[name]["start"], asset_prof[name]["end"])
        base.update(translated[name])
        lines = [f"  {name}:\n"]
        for tool, keys in (
                ("claude", ("model", "effort")),
            ...
            lines.append(f"    {tool}:\n")
            for key in keys:
                lines.append(f"      {key}: {json.dumps(base[f'{tool}:{key}'])}\n")
        return lines

    profile_block = ["profiles:\n"]
    for name in list(asset_prof) + kept_custom:
        if len(profile_block) > 1 and profile_block[-1].strip():
            profile_block.append("\n")
        if name in asset_prof:
            profile_block.extend(rendered_profile(name))
        else:
            profile_block.extend(cfg_lines[cfg_prof[name]["start"]:cfg_prof[name]["end"]])

    chosen_pp = dict(asset_pp)
    preserved_phase_overrides: list[str] = []
    for key in asset_pp:
        value = cfg_pp.get(key)
        if value is not None and (value == "" or value in kept_names):
            chosen_pp[key] = value
            if value != asset_pp[key]:
                preserved_phase_overrides.append(key)
    custom_pp = {
        key: value for key, value in cfg_pp.items()
        if key not in asset_pp and key not in LEGACY_PHASE_PROFILE_KEYS
        and (value == "" or value in kept_names)
    }
    phase_block = ["phase_profiles:\n"]
    phase_span = find_block(asset_lines, "phase_profiles")
    if phase_span is not None:
        for line in asset_lines[phase_span[0] + 1:phase_span[1]]:
            if _indent(line) == 2 and ":" in _strip_comment(line.strip()):
                key = _strip_comment(line.strip()).partition(":")[0].strip()
                if key in chosen_pp:
                    comment = ""
                    m = re.search(r"\s+#", line.rstrip("\r\n"))
                    if m:
                        comment = "  " + line.rstrip("\r\n")[m.start():].strip()
                    line = f"  {key}: {chosen_pp[key]}{comment}\n"
            phase_block.append(line)
    if custom_pp:
        if phase_block and phase_block[-1].strip():
            phase_block.append("\n")
        phase_block.append("  # Preserved valid project-local phase routes.\n")
        phase_block.extend(f"  {key}: {value}\n" for key, value in custom_pp.items())

    migrated = _replace_top_block(config_text, "profiles", profile_block)
    migrated = _replace_top_block(migrated, "phase_profiles", phase_block)
    return migrated, {
        "from": "v0.24-rendered-agents",
        "to": "current-model-profiles",
        "removed_profiles": [n for n in cfg_prof if n in LEGACY_PROFILE_NAMES],
        "removed_phase_profiles": [k for k in cfg_pp if k in LEGACY_PHASE_PROFILE_KEYS],
        "preserved_custom_profiles": kept_custom,
        "mapped_legacy_retunes": mapped_legacy_retunes,
        "legacy_retune_conflicts": legacy_retune_conflicts,
        "preserved_phase_overrides": preserved_phase_overrides,
        "preserved_custom_phase_profiles": list(custom_pp),
    }
...
def apply(config_text: str, asset_text: str, config_version: str | None,
    ...
    if info["legacy_profile_schema"]:
        migrated_text, migration = migrate_legacy_profile_schema(config_text, asset_text)
        # Re-enter the ordinary additive healer after the closed legacy names
        # have gone; it supplies new setup keys and performs the version stamp.
        healed = apply(migrated_text, asset_text, config_version, module_version, setup_text)
        healed["legacy_profile_migration"] = migration
        return healed
...
def _run_self_test() -> int:
    ...
    # ----------------------------------------------------------------------- #
    # The asset must define EXACTLY the eleven phase_profiles keys ...
    a_pp = parse_phase_profiles(a_lines, find_block(a_lines, "phase_profiles"))
    PHASE_KEYS = ("build", "followup_review", "final_convergence", "security_layer", "cross_model_layer", "tea_triage",
    ...
    # ... and EXACTLY the four shipped profiles, each model/effort ONLY (no persona strings — D12).
    a_prof = parse_profiles_blocks(a_lines, find_block(a_lines, "profiles"))
    PROFILE_NAMES = ("default", "light", "standard", "critical", "diverse_review")
    ...
    expected_codex = {
        ...
        "diverse_review": ("gpt-5.6-luna", "xhigh"),
    }
    for name, pair in expected_codex.items():
        vals = _profile_leaf_values(a_lines, a_prof[name]["start"], a_prof[name]["end"])
        assert (vals["codex:model"], vals["codex:reasoning_effort"]) == pair, (name, vals)
    expected_phase_profiles = {
        "build": "standard", "followup_review": "standard", "final_convergence": "critical",
        "security_layer": "critical", "cross_model_layer": "diverse_review",
        "tea_triage": "light", "tea_per_story": "standard",
        "tea_epic": "critical", "tea_epic_audit": "standard",
        "retrospective": "default", "deferred_reconcile": "standard",
    }
    assert a_pp == expected_phase_profiles, a_pp
    ...
    assert pub["missing_phase_profiles"]["tea_triage"] == "light", pub
    assert pub["missing_phase_profiles"]["followup_review"] == "standard", pub
    assert "build" not in pub["missing_phase_profiles"], pub["missing_phase_profiles"]
    # `default` + `critical` + `diverse_review` absent => whole missing profiles.
    assert set(pub["missing_profiles"]) == {"default", "critical", "diverse_review"}, pub["missing_profiles"]
    ...
    # customised-vs-default (PROFILE axis): standard's claude.model retune (haiku, default opus) is surfaced.
    assert any(c == {"profile": "standard", "key": "claude:model", "value": "haiku", "default": "opus"}
    ...
    assert not any(c["profile"] == "critical" for c in pub["customized_profiles"]), pub["customized_profiles"]
    ...
    assert {"profile": "standard", "key": "meta:role_blurb"} in pub["stale_profile_keys"], pub["stale_profile_keys"]
    assert {(s["profile"], s["key"]) for s in pub["stale_profile_keys"]} == {
        ("standard", "meta:description"), ("standard", "meta:role_blurb"), ("standard", "meta:status_example"),
        ("light", "meta:description")}, pub["stale_profile_keys"]
    ...
    assert h_pp.get("dev_story") == "standard", "heal must never strip a stale phase mapping"
    assert 'role_blurb: "deep work"' in healed, "heal must never strip a stale persona key"
    # ... the user retune is preserved: standard.claude.model stays haiku, NOT reset to the asset's opus.
    ...
    assert "high-stakes" in healed, "critical block not copied verbatim"
    ...
    cfg_custom = cfg_custom.replace("  build: standard", "  build: ab-ultradeep", 1)
    cfg_custom = cfg_custom.replace("  tea_triage: light", "  tea_triage: team-fast", 1)
    ...
    assert {"key": "build", "value": "ab-ultradeep", "default": "standard"} in info_cust["customized_phase_profiles"], info_cust["customized_phase_profiles"]
    assert {"key": "tea_triage", "value": "team-fast", "default": "light"} in info_cust["customized_phase_profiles"], info_cust["customized_phase_profiles"]
    ...
    # Drop ONLY standard's claude.effort from an otherwise-complete config.
    cfg_subkey = _drop_leaf(fresh_from_asset, "standard", "claude", "effort")
    assert cfg_subkey != fresh_from_asset, "fixture: standard claude.effort line not dropped"
    ...
    assert any(m["profile"] == "standard" and m["missing_key"] == "claude:effort" for m in info3["manual_review"]), info3["manual_review"]
    ...
    # standard retuned (model+effort) and one phase mapping retuned, on top of the asset.
    retuned_body = _set_leaf(_set_leaf(asset_text, "standard", "claude", "model", "haiku"), "standard", "claude", "effort", "low")
    assert _leaf(retuned_body, "standard")["claude:model"] == "haiku", "fixture: standard claude block not retuned"
    retuned_body = retuned_body.replace("  build: standard", "  build: light", 1)
    assert "build: light" in retuned_body, "fixture: build mapping not retuned"
    ...
    assert ("standard", None, "claude:model") in changed, changed
    assert ("standard", None, "claude:effort") in changed, changed
    assert (None, "phase_profiles", "build") in changed, changed
    ht = full["new_text"]
    h_vals = _leaf(ht, "standard")
    assert h_vals["claude:model"] == "opus" and h_vals["claude:effort"] == "high", h_vals
    ...
    two = _set_leaf(_set_leaf(retuned_body, "light", "claude", "model", "sonnet"), "light", "claude", "effort", "low")
    one = reset(_mk_cfg("0.8.0", two), asset_text, "0.8.0", "0.9.0", "standard")
    assert one["version_restamp"] is None, "scoped reset must NOT restamp"
    assert {c.get("profile") for c in one["would_change"]} == {"standard"}, one["would_change"]
    assert one["layers_sync_needed"] is True, one
    ot = one["new_text"]
    o_x, o_h = _leaf(ot, "standard"), _leaf(ot, "light")
    assert o_x["claude:model"] == "opus", "standard not restored"
    assert o_h["claude:model"] == "sonnet" and o_h["claude:effort"] == "low", "light retune clobbered by scoped reset"
    assert 'profiles_source_version: "0.8.0"' in ot, "scoped reset changed the stamp"
    assert "build: light" in ot, "scoped profile reset must not touch phase_profiles"
    ...
    pres = reset(_mk_cfg("0.8.0", asset_text.replace("  tea_triage: light", "  tea_triage: standard", 1)),
    ...
    for layer_key, other in (("security_layer", "standard"), ("cross_model_layer", "critical")):
    ...
    assert {"block": "phase_profiles", "key": "dev_story", "current": "standard", "default": None} in st["would_change"], st["would_change"]
    ...
    cfg_drop = _mk_cfg("0.9.0", _drop_leaf(asset_text, "standard", "claude", "effort"))
    assert any(m["profile"] == "standard" and m["missing_key"] == "claude:effort"
               for m in analyze(cfg_drop, asset_text, "0.9.0", "0.9.0")["manual_review"]), "fixture: missing sub-key not detected"
    fixed = reset(cfg_drop, asset_text, "0.9.0", "0.9.0", "standard")["new_text"]
    ...
    assert _leaf(rt, "standard")["claude:model"] == "opus", "standard not reset"
    assert parse_phase_profiles(r_lines, find_block(r_lines, "phase_profiles")) == {"build": "standard"}, "phase_profiles touched by 'profiles' scope"
    # A single <profile-name> reset leaves the user-added profile intact (no prune on a scoped reset).
    rone = reset(mini, asset_text, "0.9.0", "0.9.0", "standard")
    ...
    assert {"profile": "light", "key": "meta:role_blurb"} in up_info["stale_profile_keys"], up_info["stale_profile_keys"]
    ...
    assert {"profile": "light", "key": "meta:role_blurb", "current": "deep work", "default": None} in up["would_change"], up["would_change"]
    ...
    renamed = asset_text
    for cur, old in (("  light:", "  zz-old-a:"), ("  standard:", "  zz-old-b:"),
                     ("  critical:", "  zz-old-c:"), ("  diverse_review:", "  zz-old-d:")):
    ...
    # Automatic v0.24 schema adoption is narrower than reset: remove only the
    # closed rendered-agent names/routes, retain unrelated config, a current
    # profile retune, and a valid project-local profile/phase route.
    legacy24 = (
        ...
        '  build: standard\n  project_scan: project-local\n'
    )
    leg_info = analyze(legacy24, asset_text, "0.24.0", "0.31.0")
    assert leg_info["legacy_profile_schema"] is True and leg_info["needs_reseed"] is True, leg_info
    leg_res = apply(legacy24, asset_text, "0.24.0", "0.31.0")
    leg_text = leg_res["new_text"]
    migration = leg_res["legacy_profile_migration"]
    assert migration and set(migration["removed_profiles"]) == {"ab-deep", "ab-standard"}, migration
    assert set(migration["removed_phase_profiles"]) == {"dev_story", "code_review_fix"}, migration
    assert migration["preserved_custom_profiles"] == ["project-local"], migration
    assert "owner_note: keep-me" in leg_text and "model: sonnet" in leg_text, leg_text
    leg_lines = leg_text.splitlines(keepends=True)
    leg_prof = parse_profiles_blocks(leg_lines, find_block(leg_lines, "profiles"))
    leg_pp = parse_phase_profiles(leg_lines, find_block(leg_lines, "phase_profiles"))
    assert set(leg_prof) == set(PROFILE_NAMES) | {"project-local"}, sorted(leg_prof)
    assert not (set(leg_prof) & LEGACY_PROFILE_NAMES), sorted(leg_prof)
    assert leg_pp["build"] == "standard" and leg_pp["project_scan"] == "project-local", leg_pp
    assert not (set(leg_pp) & LEGACY_PHASE_PROFILE_KEYS), leg_pp
    assert _leaf(leg_text, "standard")["claude:model"] == "sonnet", "current retune was lost"
    assert 'profiles_source_version: "0.31.0"' in leg_text, "migration was not restamped"
    leg_again = apply(leg_text, asset_text, "0.31.0", "0.31.0")
    assert leg_again["legacy_profile_migration"] is None and leg_again["new_text"] == leg_text, \
    ...
    xcfg = (
        ...
        + asset_text.replace("  retrospective: default", "  # (retrospective dropped)", 1)
    )
    assert "retrospective: default" not in xcfg, "fixture: pp key not actually dropped"
    ...
    assert xpp.get("retrospective") == "default", "phase_profiles key not re-seeded (cross-axis)"
    assert xh.index("retrospective: default") < xh.index("\nbuild:"), "collision mis-ordered: pp key escaped its block"
...
def apply_to_file(config_path: Path, asset_path: Path, module_version: str | None,
    ...
    changed = bool(res["reseeded_phase_profiles"] or res["reseeded_profiles"]
                   ...
                   or res["legacy_profile_migration"])
    if changed:
        _write_text(config_path, res["new_text"])
    return {
        ...
        "legacy_profile_migration": res["legacy_profile_migration"],
        "config_path": str(config_path),
    }
