...
* **nesting** (P6): the chain orchestrator (depth 0) → generic delegate subagent
  ...
  - ``--cli-phases`` containing ``build``, ``followup_review`` and ``final_convergence`` ⇒ ``ok``
  ...
  - codex: ``$CODEX_HOME/config.toml`` (default ``~/.codex/config.toml``) then
    ...
    plus ``codex debug models`` when a configured or
    ``--codex-nesting-models`` model must be classified. Current Codex selects
    multi-agent V2 per model: every in-tool build/follow-up delegate reporting
    ``multi_agent_version: v2`` ⇒ ``ok`` and ``agents.max_depth`` is irrelevant.
    A V1/legacy delegate instead requires ``agents.max_depth >= 2``. Luna is a
    leaf-only V1 model and is never accepted as a nested-fan-out owner, even when
    depth is available. The obsolete ``features.multi_agent_v2`` flag is ignored:
    capability comes from the actual owner model catalog, never a global switch. Otherwise ⇒
    ``hard_stop``; an unknown model, failed catalog probe, or unparseable (or
    non-UTF-8) config file ⇒ ``warn``.
...
Usage::
    ...
    preflight.py --project-root DIR --host claude-code|codex|opencode|other --tier subagents|inline
                 ...
                 [--cli-phases CSV] [--codex-nesting-models CSV]
                 [--story-source sprint|stories] [--spec-folder DIR]
...
_CLI_PHASES_EXEMPT = {"build", "followup_review", "final_convergence"}
_CODEX_LEAF_ONLY_PREFIXES = ("gpt-5.6-luna",)
...
_MSG_CLI_EXEMPT = (
    "every nested-fan-out phase runs via the CLI route"
...
_FIX_CODEX = (
    "use V2-capable models for the in-tool build/follow-up delegates"
    " (`codex debug models` must report `multi_agent_version: v2`),"
    " or, for any V1/legacy delegate, add\n[agents]\nmax_depth = 2\nto the highest-priority"
    " Codex config (<project>/.codex/config.toml overrides ~/.codex/config.toml);"
    " then restart Codex"
)
_FIX_OPENCODE = (
...
def real_runner(argv: Sequence[str], cwd: str | None = None) -> tuple[int, str, str]:
    ...
    try:
        p = subprocess.run(
            ...
            cwd=cwd, timeout=_PROBE_TIMEOUT,
        )
...
def _probe_codex_model_versions(run: Runner) -> tuple[dict[str, str | None], str | None]:
    """Return ``slug -> multi_agent_version`` from the installed Codex catalog.

    ``debug models --bundled`` is deliberately treated as a capability probe, not a
    version check: current Codex can expose V1 and V2 models in the same binary.
    Malformed/failed probes are data for the nesting verdict and never traceback.
    """
    argv = ["codex", "debug", "models"]
    rc, stdout, stderr = run(argv)
    if rc != 0:
        summary = (stderr or stdout).strip().splitlines()
        suffix = f": {summary[0][:240]}" if summary else ""
        return {}, f"`{' '.join(argv)}` failed (exit {rc}){suffix}"
    try:
        payload = json.loads(stdout)
    except (TypeError, ValueError) as exc:
        return {}, f"`{' '.join(argv)}` returned invalid JSON: {exc}"
    entries = payload.get("models") if isinstance(payload, dict) else None
    if not isinstance(entries, list):
        return {}, f"`{' '.join(argv)}` JSON has no models list"
    ...
    for entry in entries:
        ...
        versions[slug.strip()] = version.strip().lower() if isinstance(version, str) else None
    if not versions:
        return {}, f"`{' '.join(argv)}` returned no named models"
    return versions, None


def _classify_codex_nesting(
    host: str,
    tier: str,
    project_root: Path,
    env: Mapping[str, str],
    home: Path,
    codex_models: Sequence[str],
    run: Runner,
) -> dict:
    ...
    # The models that own the nested fan-out are the in-tool build and follow-up
    # delegates, not necessarily the root orchestrator. The caller supplies those
    # resolved profile models. Falling back to the effective configured model keeps
    # older callers accurate when one model is used throughout.
    requested_models = list(codex_models)
    configured_model = merged.get("model")
    if not requested_models and isinstance(configured_model, str) and configured_model.strip():
        requested_models = [configured_model.strip()]
    models: list[str] = []
    ...
    if models:
        # This is a product routing constraint, not a catalog-probe inference.
        # Luna is the shipped leaf review/triage model and must never become a
        # build-auto fan-out owner, even when catalog inspection fails and a
        # generic V1 depth-2 fallback would otherwise be available.
        leaf_only = [model for model in models if model.startswith(_CODEX_LEAF_ONLY_PREFIXES)]
        if leaf_only:
            named = ", ".join(leaf_only)
            return _nesting(
                host, tier, "hard_stop",
                f"Codex delegate model(s) are leaf-only V1 reviewers ({named})"
                " — Luna cannot own build-auto's nested review fan-out",
                _FIX_CODEX, sources,
            )
        command_source = "command:codex debug models"
        sources.append(command_source)
        versions, probe_error = _probe_codex_model_versions(run)
        if probe_error is None:
            disabled = [model for model in models if versions.get(model) == "disabled"]
            missing = [model for model in models if model not in versions]
            needs_v1_depth = [
                model for model in models
                if model in versions and versions[model] in (None, "v1")
            ]
            unknown = [
                model for model in models
                if model in versions and versions[model] not in (None, "disabled", "v1", "v2")
            ]
            if disabled:
                named = ", ".join(disabled)
                return _nesting(
                    host, tier, "hard_stop",
                    f"Codex delegate model(s) disable multi-agent tools ({named})"
                    " — the delegate cannot spawn build-auto's subagents",
                    _FIX_CODEX, sources,
                )
            if needs_v1_depth and not depth_ok:
                named = ", ".join(needs_v1_depth)
                where = "absent" if max_depth is None else repr(max_depth)
                return _nesting(
                    host, tier, "hard_stop",
                    f"Codex delegate model(s) use the V1/legacy multi-agent runtime ({named})"
                    f" and agents.max_depth is {where} (V1 default 1)"
                    " — the delegate cannot spawn build-auto's subagents",
                    _FIX_CODEX, sources,
                )
            if depth_ok and (needs_v1_depth or missing or unknown):
                named = ", ".join(needs_v1_depth + missing + unknown)
                return _nesting(
                    host, tier, "ok",
                    f"Codex delegate model(s) using or requiring the V1 depth fallback: {named};"
                    f" agents.max_depth = {max_depth} allows the depth-2 chain",
                    None, sources,
                )
            if missing or unknown:
                named = ", ".join(missing + unknown)
                return _nesting(
                    host, tier, "warn",
                    f"Codex model-driven nesting could not be verified for: {named}"
                    " (model missing from the effective catalog or has unknown multi-agent metadata)",
                    _FIX_CODEX, sources,
                )
            if not needs_v1_depth:
                named = ", ".join(models)
                return _nesting(
                    host, tier, "ok",
                    f"Codex delegate model capability is V2 ({named}); agents.max_depth is ignored",
                    None, sources,
                )
        if depth_ok:
            return _nesting(
                host, tier, "ok",
                f"Codex model capability probe failed ({probe_error}), but agents.max_depth = {max_depth} allows the depth-2 chain",
                None, sources,
            )
        return _nesting(
            host, tier, "warn",
            f"Codex model-driven nesting could not be verified ({probe_error})",
            _FIX_CODEX, sources,
        )
    ...
    return _nesting(
        host, tier, "hard_stop",
        f"no Codex nesting model was supplied or configured, agents.max_depth is {where}"
        " (V1 default 1); the obsolete features.multi_agent_v2 flag is not a capability source"
        " — the delegate cannot spawn build-auto's subagents",
        _FIX_CODEX, sources,
    )
...
def classify_nesting(
    ...
    """The ``nesting`` block per spec §1.4 (pure given env/home/run/filesystem)."""
    ...
    if host == "codex":
        return _classify_codex_nesting(host, tier, project_root, env, home, codex_models, run)
    if host == "opencode":
...
def preflight(
    ...
    *,
    ...
    cli_phases: Sequence[str] = (),
    codex_nesting_models: Sequence[str] = (),
    ...
    nesting = classify_nesting(
        ...
        env,
        home,
        codex_models=codex_nesting_models,
        run=run,
    )
...
def _run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert len(read) == 4 and Path(read[-1]).parts[-2:] == ("custom", "config.user.toml"), read
        ...
        n = classify_nesting("codex", "subagents", root, ["build", "followup_review", "final_convergence"], {}, home)
        assert n["status"] == "ok" and n["detail"] == _MSG_CLI_EXEMPT and n["fix"] is None, n
        n = classify_nesting("codex", "subagents", root, ["followup_review", "build", "final_convergence", "tea_epic"], {}, home)
        ...
        # --- nesting: codex ---
        model_probe = _fake_runner({
            "codex debug models": (0, json.dumps({"models": [
                {"slug": "gpt-5.6-sol", "multi_agent_version": "v2"},
                {"slug": "gpt-5.6-terra", "multi_agent_version": "v2"},
                {"slug": "gpt-5.6-luna", "multi_agent_version": "v1"},
                {"slug": "gpt-5.5", "multi_agent_version": None},
                {"slug": "gpt-disabled", "multi_agent_version": "disabled"},
            ]}), ""),
        })
        # V2 is selected by the nested-spawner model, even when the old feature flag is
        # absent/false and V1 max_depth is absent. Every requested parent must pass.
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["gpt-5.6-terra", "gpt-5.6-sol"], run=model_probe,
        )
        assert n["status"] == "ok" and "V2" in n["detail"] and n["sources"] == ["command:codex debug models"], n
        # Luna is V1: safe as a leaf reviewer, but not as a nested-spawner parent at
        # max_depth 1. A mixed V2+V1 parent set therefore also fails closed.
        for requested in (["gpt-5.6-luna"], ["gpt-5.6-terra", "gpt-5.6-luna"], ["gpt-5.5"]):
            n = classify_nesting(
                "codex", "subagents", root, [], {}, home,
                codex_models=requested, run=model_probe,
            )
            assert n["status"] == "hard_stop" and n["fix"] == _FIX_CODEX, (requested, n)
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["gpt-disabled"], run=model_probe,
        )
        assert n["status"] == "hard_stop" and "disable multi-agent" in n["detail"], n
        # An absent slug or failed/malformed capability probe is unverifiable, not a
        # stale config-only hard stop. The warning is surfaced by full preflight.
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["custom-model"], run=model_probe,
        )
        assert n["status"] == "warn" and "could not be verified" in n["detail"], n
        for bad_probe in (
            _fake_runner({}),
            _fake_runner({"codex debug models": (0, "not json", "")}),
        ):
            n = classify_nesting(
                "codex", "subagents", root, [], {}, home,
                codex_models=["gpt-5.6-sol"], run=bad_probe,
            )
            assert n["status"] == "warn" and "could not be verified" in n["detail"], n
        ...
        # Luna stays leaf-only even with an explicit depth-2 fallback. A legacy
        # non-Luna V1 parent may use that depth fallback.
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["gpt-5.6-luna"], run=model_probe,
        )
        assert n["status"] == "hard_stop" and "leaf-only" in n["detail"], n
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["gpt-5.6-luna"], run=_fake_runner({}),
        )
        assert n["status"] == "hard_stop" and "leaf-only" in n["detail"], n
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["gpt-5.5"], run=model_probe,
        )
        assert n["status"] == "ok" and "V1 depth fallback" in n["detail"], n
        # Project layer overrides the user layer key-by-key (max_depth back to 1 -> stop).
        (root / ".codex").mkdir()
        ...
        n = classify_nesting("codex", "subagents", root, [], {}, home)
        assert n["status"] == "ok" and "max_depth = 2" in n["detail"], n
        # The feature flag is irrelevant. The user layer still supplies
        # effective max_depth=2, so a failed probe takes the generic V1 fallback.
        n = classify_nesting(
            "codex", "subagents", root, [], {}, home,
            codex_models=["gpt-5.6-sol"], run=_fake_runner({}),
        )
        assert n["status"] == "ok" and "probe failed" in n["detail"] and "max_depth = 2" in n["detail"], n
        ...
        # Older callers that do not pass --codex-nesting-models still classify the
        # effective top-level configured model instead of relying on the stale flag.
        (home / ".codex" / "config.toml").write_text(
            'model = "gpt-5.6-sol"\n[agents]\nmax_depth = 1\n', encoding="utf-8"
        )
        n = classify_nesting("codex", "subagents", root, [], {}, home, run=model_probe)
        assert n["status"] == "ok" and "V2" in n["detail"], n
...
def main() -> int:
    ...
    parser.add_argument("--cli-phases", default="", help="CSV of delegation.cli_phases keys (build, followup_review and final_convergence all routed ⇒ nesting exempt).")
    parser.add_argument(
        "--codex-nesting-models",
        default="",
        help="CSV of resolved Codex models for in-tool build/followup_review delegates that spawn nested review agents.",
    )
    ...
    if args.central_config_only:
        extras = [
            n for n in ("host", "tier", "expected_branch", "require_skills", "skills_dirs", "cross_model_tool", "cli_phases", "codex_nesting_models", "spec_folder")
            if getattr(args, n)
    ...
    codex_nesting_models = [
        s.strip() for s in args.codex_nesting_models.split(",") if s.strip()
    ]
    if codex_nesting_models and args.host != "codex":
        return usage_error("--codex-nesting-models requires --host codex")
    ...
    result = preflight(
        ...
        codex_nesting_models=codex_nesting_models,
