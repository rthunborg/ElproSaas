...
``--adopt-legacy-spec --impl-dir DIR --story-key KEY --state-file PATH
--baseline-revision REV --created YYYY-MM-DD [--write]``
    Explicit, one-time pre-v0.30 adoption bridge. The default is a read-only
    plan. ``--write`` preserves the legacy body in
    ``legacy-v024-{story_key}.md``, writes the canonical
    ``spec-{story_key}.md`` with current build-auto frontmatter, and atomically
    migrates state to resume at Phase 7. Legacy review counters remain evidence
    only; current ``review_loop_iteration`` / ``followup_passes`` start at zero.
    Existing legacy+current ambiguity or mismatched generated content is a hard
    stop. Rerunning an exact adoption is idempotent.
...
def _stage_write(path, content):
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8", newline="") as fh:
            fh.write(content)
        # mkstemp creates 0600; carry an existing target's mode so a replace
        # doesn't silently drop group/other bits. New adoption targets have no
        # prior mode to copy and intentionally keep the safe mkstemp default.
...
_LEGACY_STORY_STATUS_RE = re.compile(
    r"^\s*(?:\*\*status:\*\*|status:)\s*([a-z-]+)\s*$", re.IGNORECASE | re.MULTILINE)
...
def read_spec(spec_path):
    ...
    if fm is not None:
        result["artifact_format"] = "bmad-build-auto"
        ...
        legacy_status = _LEGACY_STORY_STATUS_RE.search(text)
        if legacy_status:
            # Pre-v0.30 Auto-BMAD used the ordinary BMAD story artifact
            # (`{story_key}.md`) as its review ledger. It is evidence, not a
            # current bmad-build-auto spec: expose the status separately and
            # deliberately leave `status` null so callers cannot accidentally
            # dispatch the modern build-auto resume path against it.
            result["artifact_format"] = "legacy-story"
            result["legacy_story_status"] = legacy_status.group(1).lower()
...
def build_find_spec_result(impl_dir, story_key, sprint_status_path=None):
    ...
    modern_names = sorted(
        n for n in os.listdir(impl_dir)
        if mine.match(n) and os.path.isfile(os.path.join(impl_dir, n)))
    modern_names = [n for n in modern_names if not any(o.match(n) for o in others)]
    legacy_name = f"{story_key}.md"
    legacy_path = os.path.join(impl_dir, legacy_name)
    legacy_names = [legacy_name] if os.path.isfile(legacy_path) else []
    names = modern_names + legacy_names
    ...
    for n in names:
        ...
        cands.append({
            "path": p,
            "status": info["status"],
            "artifact_format": ("legacy-story" if n == legacy_name else info["artifact_format"]),
            "legacy_story_status": info["legacy_story_status"] if n == legacy_name else None,
    ...
    # A legacy story and a modern spec are two different resume contracts.
    # Never select between them by status/mtime: the caller must resolve which
    # artifact is authoritative, preserving the existing ambiguity safeguard.
    if modern_names and legacy_names:
        result["ambiguous"] = True
        result["hard_stop"] = True
        result["hard_stop_reason"] = (
            f"ambiguous legacy story and bmad-build-auto spec for {story_key}: "
    ...
    result["artifact_format"] = chosen["artifact_format"]
    result["legacy_story_status"] = chosen["legacy_story_status"]
    result["found"] = True
    return result, 0
...
def _load_state_update_module():
    """Load the sibling writer without making scripts/ a Python package."""
    import importlib.util
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "state_update.py")
    spec = importlib.util.spec_from_file_location("_auto_bmad_state_update", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load state writer: {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def _legacy_adoption_frontmatter(story_key, baseline_revision, created, source_name):
    slug = STORY_RE.match(story_key).group(4)
    ...
    return (
        ...
        f"baseline_revision: {json.dumps(baseline_revision)}\n"
        f"legacy_source: {json.dumps(source_name)}\n"
        "context: []\n"
        "warnings:\n"
        f"  - {json.dumps('Adopted from the pre-v0.30 Auto-BMAD story/review artifact; legacy review counters remain state evidence only.')}\n"
        "deferred: []\n"
        "---\n\n"
    )
...
def _verify_baseline_commit(impl_dir, baseline_revision, git_run=None):
    """Resolve ``baseline_revision`` as a commit without mutating the repository."""
    argv = [
        "git", "-C", os.path.abspath(impl_dir), "rev-parse", "--verify",
    ...
    try:
        if git_run is None:
            ...
            code, stdout, stderr = git_run(argv)
    except (OSError, subprocess.SubprocessError) as exc:
        return {"argv": argv, "resolved_commit": None}, f"git baseline probe failed: {exc}"
    resolved = str(stdout or "").strip().splitlines()
    commit = resolved[-1].strip() if resolved else ""
    if code != 0 or not re.fullmatch(r"[0-9a-fA-F]{40}", commit):
        detail = str(stderr or stdout or "not a commit").strip()
        return {"argv": argv, "resolved_commit": None}, (
            f"baseline revision {baseline_revision!r} does not resolve to a commit"
            + (f": {detail}" if detail else ""))
    return {"argv": argv, "resolved_commit": commit.lower()}, None


def build_legacy_adoption_result(impl_dir, story_key, state_file, baseline_revision,
                                 created, write=False, git_run=None):
    """Plan/apply the one-time legacy story + state adoption.

    The original body is copied byte-for-byte behind current frontmatter, then
    the legacy file is moved to ``legacy-v024-{story_key}.md`` so normal spec
    discovery has exactly one candidate. A crash between file operations is
    recoverable: rerunning verifies the exact generated target and completes
    the remaining archive/state step. Unknown legacy state fields are retained.
    """
    result = {
        "ok": False, "action": "adopt-legacy-spec", "write": bool(write),
        "story_key": story_key, "source": None, "archive": None, "target": None,
        "state_file": state_file, "baseline_revision": baseline_revision,
        "baseline_verification": None,
        "state_patch": None, "already_adopted": False, "verified": False,
        "error": None,
    }
    sm = STORY_RE.match(story_key or "")
    if not sm:
        result["error"] = f"invalid story key '{story_key}' (expected E-S[a-z]-slug)"
        return result, 1
    if not re.fullmatch(r"[0-9a-fA-F]{7,40}", baseline_revision or ""):
        result["error"] = "--baseline-revision must be a 7-40 character hexadecimal git revision"
        return result, 1
    if not impl_dir or not os.path.isdir(impl_dir):
        result["error"] = f"implementation_artifacts dir not found: {impl_dir}"
        return result, 1
    baseline_check, baseline_error = _verify_baseline_commit(
        impl_dir, baseline_revision, git_run=git_run)
    result["baseline_verification"] = baseline_check
    if baseline_error:
        result["error"] = baseline_error
        return result, 1
    source = os.path.abspath(os.path.join(impl_dir, f"{story_key}.md"))
    archive = os.path.abspath(os.path.join(impl_dir, f"legacy-v024-{story_key}.md"))
    target = os.path.abspath(os.path.join(impl_dir, f"spec-{story_key}.md"))
    result.update(source=source, archive=archive, target=target)
    if not os.path.isfile(state_file):
        result["error"] = f"state file not found: {state_file}"
        return result, 1

    evidence_path = source if os.path.isfile(source) else archive
    if not os.path.isfile(evidence_path):
        result["error"] = f"legacy story evidence not found: {source} (or archive {archive})"
        return result, 1
    try:
    ...
    if os.path.isfile(target):
        try:
            ...
            result["error"] = f"could not read existing adoption target: {exc}"
            return result, 1
        if existing != expected:
            result["error"] = (
                f"adoption target already exists with different content: {target}; "
                "legacy/current artifacts are ambiguous"
            )
            return result, 1

    su = _load_state_update_module()
    try:
        raw_state = su.load_state(su.Path(state_file))
        state = su.full_state(raw_state)
    except Exception as exc:
        result["error"] = f"could not read legacy state: {exc}"
        return result, 1
    if state.get("story_key") != story_key:
        result["error"] = f"state story_key {state.get('story_key')!r} does not match {story_key!r}"
        return result, 1
    phases = state.get("completed_phases")
    if not isinstance(phases, list) or not set(range(7)).issubset(set(phases)) or 7 in phases:
        result["error"] = (
            "legacy adoption requires completed_phases 0-6 with Phase 7 still incomplete"
        )
        return result, 1
    if not any(k in raw_state for k in (
            "code_review_iterations", "code_review_loop_done",
            "external_review_iterations", "convergence_unverified")):
        result["error"] = "state has no pre-v0.30 review evidence; refusing legacy adoption"
        return result, 1

    overrides = dict(state.get("overrides") or {})
    overrides.pop("legacy_adoption_pending", None)
    overrides.update(legacy_adoption=_LEGACY_ADOPTION_TAG, start_phase=7)
    state_patch = {
        "spec_path": target,
        "build": {**su.default_build(), "status": "done",
                  "followup_review_recommended": True},
        "followup_passes": 0,
        "review_unverified": True,
        "legacy_review_resume": False,
        "legacy_artifact_path": archive,
        "overrides": overrides,
    }
    result["state_patch"] = state_patch
    already_state = (
        state.get("spec_path") == target
    ...
    result["already_adopted"] = os.path.isfile(target) and not os.path.isfile(source) and already_state
    if not write:
        result["ok"] = True
        return result, 0
    ...
    try:
        if not os.path.isfile(target):
            staged = _stage_write(target, expected)
            os.replace(staged, target)
        # Preserve the old artifact separately before discovery verification.
    ...
    found, find_code = build_find_spec_result(impl_dir, story_key)
    parsed = read_spec(target)
    exact_one = (
        find_code == 0 and found.get("found") is True and not found.get("ambiguous")
        and len(found.get("candidates") or []) == 1 and found.get("spec_path") == target
    )
    parser_ok = (
        parsed.get("artifact_format") == "bmad-build-auto" and parsed.get("status") == "done"
        and parsed.get("frontmatter", {}).get("followup_review_recommended") is True
        and parsed.get("frontmatter", {}).get("review_loop_iteration") == 0
        and parsed.get("frontmatter", {}).get("baseline_revision") == baseline_revision
        and parsed.get("last_review_pass") is None
    )
    if not (exact_one and parser_ok):
        result["error"] = "post-adoption verification failed (exact candidate/current parser contract)"
        result["verification"] = {"find": found, "parsed": parsed}
        return result, 1
    result["verified"] = True
    result["ok"] = True
    result["already_adopted"] = result["already_adopted"] or already_state
    return result, 0
...
"""
    ...
    check("spec: current artifact format", s["artifact_format"] == "bmad-build-auto" and s["legacy_story_status"] is None)
    ...
    legacy_p = write("impl/10-6-tax-answer-reconciliation.md",
                     "# Story 10.6\n\n**Status:** review\n\n## Review Findings\n\n- preserved\n")
    s = read_spec(legacy_p)
    check("spec legacy: classified without pretending it is build-auto",
          s["artifact_format"] == "legacy-story" and s["legacy_story_status"] == "review"
          and s["status"] is None and s["auto_run_result"]["present"] is False)
    ...
    # Pre-v0.30 Auto-BMAD used `{story_key}.md` as the story/review ledger.
    # Adopt that exact artifact deterministically without presenting it as a
    # current build-auto spec; if a modern spec also exists, fail ambiguous.
    legacy = write("fs/10-6-tax-answer-reconciliation.md",
                   "# Story 10.6\n\nStatus: review\n\n## Review Findings\n\n- kept\n")
    r, code = build_find_spec_result(fs_dir, "10-6-tax-answer-reconciliation")
    check("find: legacy exact story adopted",
          code == 0 and r["found"] is True and r["spec_path"] == legacy
          and r["artifact_format"] == "legacy-story" and r["legacy_story_status"] == "review"
          and r["status"] is None)
    modern = touch("fs/spec-10-6-tax-answer-reconciliation.md", "done", t0)
    r, code = build_find_spec_result(fs_dir, "10-6-tax-answer-reconciliation")
    check("find: legacy + modern is ambiguous",
          code == 1 and r["ambiguous"] is True and r["hard_stop"] is True
          and {c["path"] for c in r["candidates"]} == {legacy, modern})
    os.unlink(legacy)
    os.unlink(modern)
    ...
    plan_adopt, code = build_legacy_adoption_result(
        ...
        git_run=adopt_git)
    check("legacy adoption: dry-run plans without writes",
          code == 0 and plan_adopt["ok"] is True and plan_adopt["verified"] is False
          and os.path.isfile(adopt_source) and not os.path.exists(plan_adopt["target"]))
    done_adopt, code = build_legacy_adoption_result(
        adopt_dir, adopt_key, adopt_state, "e90ec0e", "2026-08-31", write=True,
        git_run=adopt_git)
    check("legacy adoption: exact parser/discovery verification",
    ...
    check("legacy adoption: old counters are evidence, not current passes",
          adopted_state["code_review_iterations"] == 6
          and adopted_state["legacy_review_iteration"] == 6
          and adopted_state["followup_passes"] == 0
          and adopted_state["build"]["review_loop_iteration"] == 0
          and adopted_state["overrides"] == {
              "legacy_adoption": _LEGACY_ADOPTION_TAG, "start_phase": 7})
    again_adopt, code = build_legacy_adoption_result(
        adopt_dir, adopt_key, adopt_state, "e90ec0e", "2026-08-31", write=True,
        git_run=adopt_git)
    check("legacy adoption: exact rerun is an idempotent verified noop",
          code == 0 and again_adopt["already_adopted"] is True
          and again_adopt["verified"] is True)
    with open(done_adopt["target"], "a", encoding="utf-8", newline="") as fh:
        fh.write("different\n")
    refused_adopt, code = build_legacy_adoption_result(
        adopt_dir, adopt_key, adopt_state, "e90ec0e", "2026-08-31", write=False,
        git_run=adopt_git)
    ...
    code, _ = _main(["--adopt-legacy-spec", "--impl-dir", fs_dir, "--story-key", "10-6-x"])
    check("cli: adoption requires its explicit contract", code == 2)
    code, _ = _main(["--spec", spec_p, "--write"])
    check("cli: --write is adoption-only", code == 2)
...
def main(argv=None):
    ...
    parser.add_argument("--adopt-legacy-spec", action="store_true", help="plan/apply the explicit pre-v0.30 story/state adoption bridge")
    ...
    parser.add_argument("--state-file", metavar="PATH", help="with --adopt-legacy-spec: pre-v0.30 Auto-BMAD state file")
    parser.add_argument("--baseline-revision", metavar="REV", help="with --adopt-legacy-spec: confirmed 7-40 digit hexadecimal baseline revision")
    parser.add_argument("--created", metavar="DATE", help="with --adopt-legacy-spec: current spec created date (YYYY-MM-DD)")
    parser.add_argument("--write", action="store_true", help="with --adopt-legacy-spec: atomically apply the verified adoption plan")
    ...
    if args.adopt_legacy_spec:
        modes.append("adopt-legacy-spec")
    ...
    if len(modes) != 1:
        parser.error(
            ...
            "--mark-status | --find-spec | --adopt-legacy-spec | --spec | --retro-verdict | --self-test"
        )
    mode = modes[0]
    ...
    if args.impl_dir is not None and mode not in ("find-spec", "retro-verdict", "adopt-legacy-spec"):
        parser.error("--impl-dir is only valid with --find-spec/--retro-verdict/--adopt-legacy-spec")
    if args.story_key is not None and mode not in ("find-spec", "adopt-legacy-spec"):
        parser.error("--story-key is only valid with --find-spec/--adopt-legacy-spec")
    ...
    adoption_only = (args.state_file is not None or args.baseline_revision is not None
                     or args.created is not None or args.write)
    if adoption_only and mode != "adopt-legacy-spec":
        parser.error("--state-file/--baseline-revision/--created/--write are only valid with --adopt-legacy-spec")
    ...
    if mode == "adopt-legacy-spec":
        if not args.impl_dir or not args.story_key or not args.state_file or not args.baseline_revision or not args.created:
            parser.error(
                "--adopt-legacy-spec requires --impl-dir DIR --story-key KEY "
                "--state-file PATH --baseline-revision REV --created YYYY-MM-DD"
            )
        if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", args.created):
            parser.error("--created must be YYYY-MM-DD")
        return emit(*build_legacy_adoption_result(
            args.impl_dir, args.story_key, args.state_file, args.baseline_revision,
            args.created, write=args.write,
        ))
    if mode == "spec":
