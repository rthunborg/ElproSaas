...
``spec_folder``, ``story_id`` and the persisted ``selected_*`` route capsule /
``escalation_reason``) are read line by line —
...
_SCALAR_RE = {
    ...
    # Persisted delegate route capsule. A resume reuses these exact values;
    # state_update.py route-select rejects silent changes for an in-flight phase.
    "selected_phase": re.compile(r"^selected_phase:\s*(.*)$"),
    "selected_role": re.compile(r"^selected_role:\s*(.*)$"),
    "selected_profile": re.compile(r"^selected_profile:\s*(.*)$"),
    "selected_model": re.compile(r"^selected_model:\s*(.*)$"),
    "selected_effort": re.compile(r"^selected_effort:\s*(.*)$"),
    "selected_host": re.compile(r"^selected_host:\s*(.*)$"),
    "selected_tier": re.compile(r"^selected_tier:\s*(.*)$"),
    "selected_route": re.compile(r"^selected_route:\s*(.*)$"),
    "escalation_reason": re.compile(r"^escalation_reason:\s*(.*)$"),
}

# The scalars every record carries, in emission order.
_FIELDS = (
    "status", "updated_at", "branch", "story_source", "spec_folder", "story_id",
...
def read_state_file(path: str):
    """Return identity/status plus the deterministic route capsule.

    Every value may be None for old or unreadable state files.
    """
...
def _story_record(state_dir: str, filename: str):
    ...
    return {
        ...
        **{key: fields[key] for key in _FIELDS if key.startswith("selected_") or key == "escalation_reason"},
        "file": path,
