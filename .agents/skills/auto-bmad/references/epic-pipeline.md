...
- Review stays automatic: build-auto's own review layers (including the independent cross-model leaf when enabled) inside every build run, plus the Phase 7 primary follow-up/review-fix pass at Terra/high when its gate holds. The `pipeline.md` final-convergence selector still applies to a legacy-adoption or explicit conflict route (Sol/xhigh); it is not the ordinary epic review path. A story that still recommends a follow-up review after its last pass — or a run whose instructions skip the follow-up review — sets `review_unverified` on the epic anchor, and the epic **ships a draft**.
...
f. **Phase 7 Follow-up review** (**`followup-review`** → `followup_review`; or **`final-convergence`** → `final_convergence` only when `pipeline.md`'s selector fires): the same gate as per story; one pass when it holds (an E0-adopted `review` story enters under `pipeline.md` Phase 7's "Entry at Phase 7 without a Phase 5 result" rule — that is what the adopt choice means; a pre-v0.30 legacy-adoption marker takes the final-convergence route). Then `hitl_halt: "auto-continued (epic — no halt)"` — **no `AskUserQuestion`, no external-change check** (no human pause produced changes to review).
