          ...
          <ul data-testid={tid("file-panel-existing-list")} className="flex flex-col gap-1">
            {props.files.length === 0 ? (
              ...
              props.files.map((f) => (
                <li
                  key={f.linkId}
                  data-testid={tid("file-panel-file-item")}
                  className="flex items-center justify-between gap-3 rounded border border-zinc-100 px-2 py-1"
                >
                  <span className="truncate text-zinc-800">{f.displayName}</span>
