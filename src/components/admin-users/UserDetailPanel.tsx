...
export function UserDetailPanel({ detail }: { detail: AdminUserDetail }) {
  ...
  const submit = (form: FormData) => { const current = String(form.get("operationId") ?? ""); const next = operationIdForAdminUsersSubmit(state, current); if (next !== current) { form.set("operationId", next); const input = form.get("action") === "re_role" ? reRoleOperation : lifecycleOperation; if (input.current) { input.current.value = next; input.current.dataset.action = String(form.get("action") ?? ""); } } return action(form); };
  ...
  return <section className="p-6" aria-labelledby="user-events-heading"><h1 id="user-events-heading" className="text-2xl font-semibold">Händelser</h1>
    ...
    <form action={submit} className="mt-4 flex flex-wrap gap-2"><input type="hidden" name="membershipId" value={detail.id}/><input ref={lifecycleOperation} type="hidden" name="operationId" />
      ...
      {detail.status === "expired" && <><button name="action" value="resend" onClick={() => prepareOperation(lifecycleOperation, "resend")} disabled={pending}>Skicka ny inbjudan</button><button name="action" value="revoke" onClick={() => prepareOperation(lifecycleOperation, "revoke")} disabled={pending}>Återkalla inbjudan</button></>}
    </form>
