import { handleUnsubscribeRequest, trustedUnsubscribeIp } from "@/server/email/unsubscribe";
...
function documentFor(state: "active" | "inactive" | "unsubscribed" | "limited") {
  ...
  if (state === "unsubscribed") return '<!doctype html><main><h1>E-postinställningar</h1><p role="status">Du är avregistrerad.</p></main>';
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const result = await handleUnsubscribeRequest({ token, ip: trustedUnsubscribeIp(request.headers.get("x-vercel-forwarded-for")) });
