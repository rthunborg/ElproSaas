...
export default async function OperatorTenantPage({ params }: { readonly params: Promise<{ tenantId: string }> }) {
  ...
  try {
    identifier = decodeURIComponent(tenantId);
  } catch {
    return <p>Åtkomst saknas.</p>;
  }
  const result = await readOperatorConsole(identifier);
  const row = result.ok ? result.data[0] : null;
  if (!row) return <p>Åtkomst saknas.</p>;
  const resume = await readOperatorConsoleResumeState(identifier);
  const state = deriveWizardState({ lifecycle: row.provisioningState, firstAdminState: row.firstAdminState, attempt: resume?.attempt, invitationExpired: resume?.invitationExpired });
  return <section aria-labelledby="operator-tenant-heading"><h1 id="operator-tenant-heading">{row.tenantName}</h1><p>{row.canonicalOrganisationIdentity}</p><p role="status">{firstAdminLabel(row.firstAdminState)}</p>{resume && <p>Försök: {resume.attempt}</p>}{state.requiresReconciliation && <p>Inbjudan är okänd och måste kontrolleras innan ett nytt försök.</p>}{state.requiresFreshApprovalForAttempt === 4 && <p>En ny förhandsgranskning och ett nytt godkännande krävs före försök 4. Äldre länkar är ogiltiga.</p>}{resume && <HandoffRecovery tenantId={resume.tenantId} requiresReconciliation={Boolean(state.requiresReconciliation)} canRetry={Boolean(state.canRetry && !state.requiresFreshApprovalForAttempt)} requiresFreshApproval={Boolean(state.requiresFreshApproval || state.requiresFreshApprovalForAttempt)} />}</section>;
}
