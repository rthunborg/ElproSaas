...
export async function readPersonalNotifications() {
  ...
  const { data: run } = await db.from("job_runs").select("finished_at,outcome").eq("tenant_id", context.data.tenantId).eq("producer", "quotes.follow-up-reminders").order("finished_at", { ascending: false }).limit(1).maybeSingle();
