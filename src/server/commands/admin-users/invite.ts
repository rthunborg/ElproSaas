...
export async function inviteAdminUser(input: { email?: unknown; roles?: unknown; operationId?: unknown; actorId?: unknown; tenantId?: unknown }) {
  ...
  if (!fresh) return outcome === "succeeded" ? { ok: true as const, membershipId } : denied;
  ...
  return result.outcome === "succeeded" ? { ok: true as const, membershipId } : denied;
}
...
export async function resendAdminUserInvitation(input: { membershipId?: unknown; operationId?: unknown }) {
  ...
  try {
    ...
    const { data, error } = await client.from("tenant_memberships")
      .select("invited_email, role, status").eq("id", input.membershipId).maybeSingle();
    if (error || !data || data.status !== "invited" || typeof data.invited_email !== "string" || typeof data.role !== "string") return denied;
    const { data: roleRows, error: roleError } = await client.from("membership_roles")
      .select("role").eq("membership_id", input.membershipId);
