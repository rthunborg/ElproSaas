-- Story 12.1 / Decision 8A follow-up. The original authority migration was
-- already applied in local environments before this claim-based actor binding
-- landed. Replacing the function preserves the one normal-JWT + attestation
-- protocol while avoiding auth.uid() directly under the non-login owner.
...
begin
  begin
    -- This GUC is set by the authenticated caller's normal JWT before the
    -- SECURITY DEFINER boundary. is_platform_operator() independently checks
    -- that same live JWT identity against the allow-list.
